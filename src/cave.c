...
bool cave_valid_bold(int y, int x)
{
	...
	for (this_o_idx = c_ptr->o_idx; this_o_idx; this_o_idx = next_o_idx)
	{
		...
		/* Forbid artefact grids */
		if ((o_ptr->art_name) || artefact_p(o_ptr)) return (FALSE);
	}
...
* which means their colour changes, and "ATTR_CLEAR", which means they take
* the colour of whatever is under them, and "CHAR_CLEAR", which means that
...
* "monsters" and "objects" are really just "coloured ascii symbols".
*
...
* causing certain grids to be displayed using special colours.  If the
...
* any special colours for "dark" wall grids, since this would allow the
...
*
* Note that the "map_info()" function must return fully colourized
...
{
	int i, j, x, y,maxx,maxy,x_offset,y_offset;
	...
	for (x = 1; x <= maxx; x++) mc[0][x] = mc[maxy+1][x] = '-';

	/* Draw the vertical edges */
	for (y = 1; y <= maxy; y++) mc[y][0] = mc[y][maxx+1] = '|';
	...
	{
		/* Start a new line */
		Term_gotoxy(x_offset, y_offset+y);
	...
	/* Player location */
	(*cy) = y_offset + (py / RATIO + 1);
	(*cx) = x_offset + (px / RATIO + 1);
...
void do_cmd_view_map(void)
{
	...
	/* Wait for it */
	put_str("Hit any key to continue", 23, 28);
...
* to efficiently display the "torch lite" in a special colour.
*
*
...
*
* We optimize most lites (all non-artefact lites) by using "obvious"
...
void disturb(int stop_stealth, int unused_flag)
{
	...
	/* Cancel search mode if requested */
	if (stop_stealth && p_ptr->searching)
	{
		/* Cancel */
