...
static byte borg_magic_method[8][4][8] =
{
		...
		{/* 6 Trump Realm */
				...
				{/* Trumps of Doom (sval 2) */
						...
						BORG_MAGIC_NOP  /*   "T. Reptiles" */,
						BORG_MAGIC_NOP  /*   "T. Hounds" */,
				...
				{/* Five Aces (sval 3) */
						...
						BORG_MAGIC_NOP  /*   "T. Undead" */,
						BORG_MAGIC_NOP  /*   "T. Dragon" */,
						BORG_MAGIC_NOP  /*   "Mass Trump" */,
						BORG_MAGIC_NOP  /*   "T. Demon" */,
						BORG_MAGIC_NOP  /*   "T. Ancient Dragon " */,
						BORG_MAGIC_NOP  /*   "T. Greater Undead" */}
	}, /* end of Trump Realm */
...
{
	if ((take->tval == TV_SWORD) ||
	(take->tval == TV_POLEARM) ||
	(take->tval == TV_HAFTED) ||
	(take->tval == TV_DIGGING) ) return (INVEN_WIELD);

	if ((take->tval == TV_DRAG_ARMOR) ||
	(take->tval == TV_HARD_ARMOR) ||
	(take->tval == TV_SOFT_ARMOR) ) return (INVEN_BODY);

	if (take->tval == TV_SHIELD) return (INVEN_ARM);

	if ((take->tval == TV_CROWN) ||
	(take->tval == TV_HELM) ) return (INVEN_HEAD);

	if (take->tval == TV_BOW) return (INVEN_BOW);

	if (take->tval == TV_RING) return (INVEN_LEFT);

	if (take->tval == TV_AMULET) return (INVEN_NECK);

	if (take->tval == TV_LITE) return (INVEN_LITE);

	if (take->tval == TV_CLOAK) return (INVEN_OUTER);

	if (take->tval == TV_GLOVES) return (INVEN_HANDS);

	if (take->tval == TV_BOOTS) return (INVEN_FEET);
...
bool borg_object_star_id( void )
{
	...
	/* look in inventory and equiptment for something to *id* */
		...
		if (borg_items[i].needs_I_exam || item->quest ||
			(!borg_items[i].flags1 || !borg_items[i].flags2 || !borg_items[i].flags3))
				{
						if (!item->quest)
						{
						...
						if (!item->timeout && (item->name1 || item->name2 || item->name3 || item->quest ) &&
								strlen(item->desc) < 65 && /* some items have intrinsic names which are too long */
				(item->note == NULL || streq(item->note, ".") || streq(item->note, " ")  ||
				 streq(item->note, "")  || strstr(item->note, "cursed") || strstr(item->note, "uncursed") ||
...
static s32b borg_object_value_known(borg_item *item)
{
		...
		switch (item->tval)
		{
				...
				case TV_RING:
				{
						...
						if (item->flags1 & TR1_CHA) value += (item->pval * 200L);
 ...
 */
void borg_item_analyze(borg_item *item, object_type *real_item, cptr desc, int location)
{
	...
	/* Item has been ID'd (store, scroll, spell) */
		...
		((item->tval >= TV_MIRACLES_BOOK && item->tval <= TV_DEMONIC_BOOK) && item->aware) ||
		(item->tval == TV_ROD && item->aware) ||
		(item->tval == TV_POTION && item->aware) ||
		(item->tval == TV_FOOD && item->aware)) item->ident = TRUE;
	...
	if (item->tval == TV_STAFF || item->tval == TV_WAND)
	{
		...
		if (item->ident || strstr(item->desc, "charges")) item->pval = real_item->pval;
		if (item->ident || strstr(item->desc, "x")) item->pval = real_item->pval * item->iqty;
		...
		if (strstr(item->note, "empty")) item->pval = 0;


	}

	/* Phial is always lit, even unid'd one. */
	if (item->kind == 500 && !item->ident) item->pval = 1;
	...
	/* Check the Randart item flags. */
		...
		{
				/* Extract the artifact flags.
		 * This is not cheating.  If I know the name of the randart, then I know it's proporties.
		 */
...
bool borg_quaff_potion(int sval)
{
	...
	/* Safety check */
	if (sval <= SV_POTION_INFRAVISION) return (FALSE);
...
bool borg_equips_planar(/*int fail_allowed*/)
{
		...
		/* No item */
		if (!borg_items[i].iqty) return (FALSE);

		/* Not a Trump which is now a Planar */
		if (borg_items[i].name2 != EGO_PLANAR) return (FALSE);

	/* Not charged */
		if (borg_items[i].timeout) return (FALSE);
	...
	if (borg_items[i].cursed) chance = chance / 3;
...
bool borg_equips_artifact(int name1)
{
		...
		/* Cursed items make it much more difficult */
		if (item->cursed) chance = chance / 3;
...
bool borg_equips_activation(int activation, bool immediate)
{
		...
		for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
		{
				...
				if (item->xtra2 != activation) continue;

				/* Check charge, if requested. */
		if (immediate && item->timeout) continue;
...
bool borg_spell_okay(int realm, int book, int what)
{
	...
	if (borg_skill[BI_CRSNOMAGIC]) return (FALSE);
		...
		if (borg_skill[BI_REALM1] == REALM_TAROT) reserve_mana = 6;
		if (borg_skill[BI_REALM1] == REALM_CHARMS) reserve_mana = 15;
...
bool borg_spell(int realm, int book, int what)
{
	...
	if (borg_skill[BI_CRSNOMAGIC]) return (FALSE);
		...
		/* Debugging Info */
	borg_note(format("# Casting %s (%d,%d) fail rate: %d.", as->name, book, what, borg_spell_fail_rate(realm, book, what)));
...
bool borg_mindcr_legal(int spell, int level)
{
		...
		/* The borg must be able to "cast" spells this realm*/
	if (borg_class != CLASS_ORPHIC) return (FALSE);
...
bool borg_mindcr_okay(int spell, int level)
{
	...
	if (borg_skill[BI_CRSNOMAGIC]) return (FALSE);
		...
		if (borg_skill[BI_CLEVEL] >= 7) reserve_mana = 2;
...
bool borg_racial_check(int race, bool check_fail, int num)
{
	...
	if (borg_skill[BI_CURSP] <= cost) use_hp = TRUE;

	/* Too confused */
	if (borg_skill[BI_ISCONFUSED]) return FALSE;

	/* Cost -- dont go into debt */
	if (use_hp && (cost > borg_skill[BI_CURHP] * 5/10) && !borg_munchkin_mode) return (FALSE);

	/* Cost -- dont go into debt */
	if (use_hp && cost >= borg_skill[BI_CURHP]) return (FALSE);
	...
	if (borg_skill[BI_CURHP] < borg_skill[BI_MAXHP] * 5/10 &&
		borg_race != GNOME && !borg_munchkin_mode) return (FALSE);
...
extern bool borg_needed_deinscribe(void)
{
		...
		if (strstr(item->note, "!") || (strstr(item->note, "Quest") &&
			item->ident))
		{
...
void borg_cheat_store(int shop_num)
{
	...
	for (slot = 0; slot < 24; slot++)
	{
		...
		strcpy(borg_shops[shop_num].ware[slot].desc, buf);
				...
				/* Analyze the item */
				borg_item_analyze(&borg_shops[shop_num].ware[slot], &store[shop_num].stock[slot], buf, slot);
		...
		else
		{
			...
			x = price_item(j_ptr, owners[shop_num][st_ptr->owner].min_inflate, FALSE);
...
void borg_cheat_equip(void)
{
		...
		for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
		{
			...
			/* if (streq(borg_items[i].note, "special")) borg_items[i].fully_identified = FALSE; */
				}

		/* Make sure my trump weapon has inscribed the . to prevent teleportation */
		if ((borg_items[i].flags3 & TR3_TELEPORT) && !strstr(borg_items[i].note, ".")) borg_send_inscribe(i,".");
		}
}
...
static void prepare_book_info(int realm, int book)
{
	...
	/* Extract spells */
		...
		for (what = 0; what < num; what++)
		{
				...
				get_extended_spell_info( realm  , what , s_ptr );
...
/* Attempt to extract the exact effect of some mutations.  Some can effect our stats and some give us abilities
 * This is a copy from Mutation.c  if new mutations are added there (or effect altered) then the same change must
 * me made here.
 */
extern void borg_cheat_mutations(void)
{
	/* Muta1 is Activatible */
	if (p_ptr->muta1 & COR1_VAMPIRISM) borg_skill[BI_VAMPIRE] = TRUE;
 ...
 */
static bool borg_mutation_aux(s16b min_level, int cost, int use_stat, int difficulty, int fail_rate)
{
	bool use_hp = FALSE;
	int stat;
	int lev_adj = 0;
...
extern bool borg_mutation(u32b power, bool simulation, int fail_rate, bool legal)
{
	...
	/* Humans and others do not have an activatable power (slot A) on the 'U' list */
	if (borg_race != HUMAN && borg_race != ELF ) num++;

	/* Nobody has an extra racial command */
	/* if (borg_race == <>) num++; */

	/* Need to have some type of activatable mutation */
	if (!p_ptr->muta1) return (FALSE);

	/* Attempt to find the letter designation on the 'U'list.  Order is important. */
	if (p_ptr->muta1 & COR1_SPIT_ACID)
	{
		num++;
		if (power == COR1_SPIT_ACID) i = num;
	}
	if (p_ptr->muta1 & COR1_BR_FIRE)
	{
		num++;
		if (power == COR1_BR_FIRE) i = num;
	}
	if (p_ptr->muta1 & COR1_HYPN_GAZE)
	{
		num++;
		if (power == COR1_HYPN_GAZE) i = num;
	}
	if (p_ptr->muta1 & COR1_TELEKINES)
	{
		num++;
		if (power == COR1_TELEKINES) i = num;
	}
	if (p_ptr->muta1 & COR1_VTELEPORT)
	{
		num++;
		if (power == COR1_VTELEPORT) i = num;
	}
	if (p_ptr->muta1 & COR1_MIND_BLST)
	{
		num++;
		if (power == COR1_MIND_BLST) i = num;
	}
	if (p_ptr->muta1 & COR1_SLIME)
	{
		num++;
		if (power == COR1_SLIME) i = num;
	}
	if (p_ptr->muta1 & COR1_VAMPIRISM)
	{
		num++;
		if (power == COR1_VAMPIRISM) i = num;
	}
	if (p_ptr->muta1 & COR1_SMELL_MET)
	{
		num++;
		if (power == COR1_SMELL_MET) i = num;
	}
	if (p_ptr->muta1 & COR1_SMELL_MON)
	{
		num++;
		if (power == COR1_SMELL_MON) i = num;
	}
	if (p_ptr->muta1 & COR1_BLINK)
	{
		num++;
		if (power == COR1_BLINK) i = num;
	}
	if (p_ptr->muta1 & COR1_EAT_ROCK)
	{
		num++;
		if (power == COR1_EAT_ROCK) i = num;
	}
	if (p_ptr->muta1 & COR1_SWAP_POS)
	{
		num++;
		if (power == COR1_SWAP_POS) i = num;
	}
	if (p_ptr->muta1 & COR1_SHRIEK)
	{
		num++;
		if (power == COR1_SHRIEK) i = num;
	}
	if (p_ptr->muta1 & COR1_ILLUMINE)
	{
		num++;
		if (power == COR1_ILLUMINE) i = num;
	}
	if (p_ptr->muta1 & COR1_DET_CURSE)
	{
		num++;
		if (power == COR1_DET_CURSE) i = num;
	}
	if (p_ptr->muta1 & COR1_BERSERK)
	{
		num++;
		if (power == COR1_BERSERK) i = num;
	}
	if (p_ptr->muta1 & COR1_POLYMORPH)
	{
		num++;
		if (power == COR1_POLYMORPH) i = num;
	}
	if (p_ptr->muta1 & COR1_MIDAS_TCH)
	{
		num++;
		if (power == COR1_MIDAS_TCH) i = num;
	}
	if (p_ptr->muta1 & COR1_GROW_MOLD)
	{
		num++;
		if (power == COR1_GROW_MOLD) i = num;
	}
	if (p_ptr->muta1 & COR1_RESIST)
	{
		num++;
		if (power == COR1_RESIST) i = num;
	}
	if (p_ptr->muta1 & COR1_EARTHQUAKE)
	{
		num++;
		if (power == COR1_EARTHQUAKE) i = num;
	}
	if (p_ptr->muta1 & COR1_EAT_MAGIC)
	{
		num++;
		if (power == COR1_EAT_MAGIC) i = num;
	}
	if (p_ptr->muta1 & COR1_WEIGH_MAG)
	{
		num++;
		if (power == COR1_WEIGH_MAG) i = num;
	}
	if (p_ptr->muta1 & COR1_STERILITY)
	{
		num++;
		if (power == COR1_STERILITY) i = num;
	}
	if (p_ptr->muta1 & COR1_PANIC_HIT)
	{
		num++;
		if (power == COR1_PANIC_HIT) i = num;
	}
	if (p_ptr->muta1 & COR1_DAZZLE)
	{
		num++;
		if (power == COR1_DAZZLE) i = num;
	}
	if (p_ptr->muta1 & COR1_EYE_BEAM)
	{
		num++;
		if (power == COR1_EYE_BEAM) i = num;
	}
	if (p_ptr->muta1 & COR1_RECALL)
	{
		num++;
		if (power == COR1_RECALL) i = num;
	}
	if (p_ptr->muta1 & COR1_BANISH)
	{
		num++;
		if (power == COR1_BANISH) i = num;
	}
	if (p_ptr->muta1 & COR1_COLD_TOUCH)
	{
		num++;
		if (power == COR1_COLD_TOUCH) i = num;
	}
	if (p_ptr->muta1 & COR1_LAUNCHER)
	{
		num++;
		if (power == COR1_LAUNCHER) i = num;
	}
	...
	/* Do I have the mutation and do I have a reasonable fail rate? */
		...
		{
			if (!(p_ptr->muta1 & COR1_SPIT_ACID)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(9, 9, A_DEX, 15, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_BR_FIRE)
		{
			if (!(p_ptr->muta1 & COR1_BR_FIRE)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(20, borg_skill[BI_CLEVEL], A_CON, 18, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_HYPN_GAZE)
		{
			if (!(p_ptr->muta1 & COR1_HYPN_GAZE)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(12, 12, A_CHA, 18, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_VTELEPORT)
		{
			if (!(p_ptr->muta1 & COR1_VTELEPORT)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(7, 7, A_WIS, 15, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_MIND_BLST)
		{
			if (!(p_ptr->muta1 & COR1_MIND_BLST)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(5, 3, A_WIS, 15, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_SLIME)
		{
			if (!(p_ptr->muta1 & COR1_SLIME)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(15, 15, A_CON, 14, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_VAMPIRISM)
		{
			if (!(p_ptr->muta1 & COR1_VAMPIRISM)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(2, (1 + (borg_skill[BI_CLEVEL] / 3)), A_CON, 9, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_SMELL_MON)
		{
			if (!(p_ptr->muta1 & COR1_SMELL_MON)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(5, 4, A_INT, 15, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_BLINK)
		{
			if (!(p_ptr->muta1 & COR1_BLINK)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(5, 4, A_INT, 15, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		...
		{
			if (!(p_ptr->muta1 & COR1_EAT_ROCK)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(8, 12, A_CON, 18, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_SWAP_POS)
		{
			if (!(p_ptr->muta1 & COR1_SWAP_POS)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(15, 12, A_DEX, 16, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_SHRIEK)
		{
			if (!(p_ptr->muta1 & COR1_SHRIEK)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(20, 14, A_DEX, 16, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_ILLUMINE)
		{
			if (!(p_ptr->muta1 & COR1_ILLUMINE)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(3, 2, A_INT, 10, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_BERSERK)
		{
			if (!(p_ptr->muta1 & COR1_BERSERK)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(8, 8, A_STR, 14, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_MIDAS_TCH)
		{
			if (!(p_ptr->muta1 & COR1_MIDAS_TCH)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(10, 5, A_INT, 12, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_RESIST)
		{
			if (!(p_ptr->muta1 & COR1_RESIST)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(10, 12, A_CON, 12, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_EARTHQUAKE)
		{
			if (!(p_ptr->muta1 & COR1_EARTHQUAKE)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(12, 12, A_STR, 16, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_EAT_MAGIC)
		{
			if (!(p_ptr->muta1 & COR1_EAT_MAGIC)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(17, 1, A_WIS, 15, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_PANIC_HIT)
		{
			if (!(p_ptr->muta1 & COR1_PANIC_HIT)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(10, 12, A_DEX, 14, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_DAZZLE)
		{
			if (!(p_ptr->muta1 & COR1_DAZZLE)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(7, 15, A_CHA, 8, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_EYE_BEAM)
		{
			if (!(p_ptr->muta1 & COR1_EYE_BEAM)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(7, 15, A_WIS, 9, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_RECALL)
		{
			if (!(p_ptr->muta1 & COR1_RECALL)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(17, 50, A_INT, 16, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_BANISH)
		{
			if (!(p_ptr->muta1 & COR1_BANISH)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(25, 25, A_WIS, 18, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == COR1_COLD_TOUCH)
		{
			if (!(p_ptr->muta1 & COR1_COLD_TOUCH)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(2, 2, A_CON, 11, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
		if (power == 3) /* COR1_LAUNCHER */
		{
			if (!(p_ptr->muta1 & COR1_LAUNCHER)) return (FALSE);
			/* Return if just checking for legality */
			if (legal) return (TRUE);
			if (!borg_mutation_aux(1, borg_skill[BI_CLEVEL], A_STR, 6, fail_rate)) return (FALSE);
			if (simulation) return (TRUE);
		}
