    ...
    switch (o_ptr->tval)
    {
        ...
        case TV_SHOT:
        {
            ...
            if (o_ptr->name1 != j_ptr->name1 ||
            	o_ptr->name1 == ART_RANDART) return (FALSE);
...
bool borg_think_shop_sorcery(void)
{
    ...
    for (i = 0; i < INVEN_TOTAL; i++)
	{
		...
		if (item->tval == TV_STAFF && item->pval / item->iqty > 3) continue;
	...
	else if (recharge)
	{
		...
		charges = (k_info[borg_items[b_i].kind].pval * borg_items[b_i].iqty) - borg_items[b_i].pval;
...
bool borg_think_shop_trump(void)
{
	...
	/* if I am prepped for super deep but playing shallow, jump to the prepped depth */
...
bool borg_think_shop_fighter(void)
{
	...
	if (weapon_swap > 1 &&
		(borg_items[weapon_swap].iqty &&
	     borg_items[weapon_swap].ident &&
	    (borg_items[weapon_swap].to_h < max_pimp ||
	     borg_items[weapon_swap].to_d < max_pimp)))
	{
		b_i = weapon_swap;
		b_pimp = MAX(max_pimp - borg_items[weapon_swap].to_h, max_pimp - borg_items[weapon_swap].to_d);
	}

	/* Can I pimp my swap armor */
	if (armour_swap > 1 &&
		(borg_items[armour_swap].iqty &&
	     borg_items[armour_swap].ident &&
	...
	if (borg_items[INVEN_WIELD].iqty &&
	    borg_items[INVEN_WIELD].ident &&
	   (borg_items[INVEN_WIELD].to_h < max_pimp ||
	    borg_items[INVEN_WIELD].to_d < max_pimp))
	{
		b_i = INVEN_WIELD;
		b_pimp = MAX(max_pimp - borg_items[INVEN_WIELD].to_h, max_pimp - borg_items[INVEN_WIELD].to_d);
	}
    ...
    for (i = INVEN_BODY; i < INVEN_TOTAL; i++)
	{
		...
		if (item->to_a >= max_pimp) continue;
	...
	for (i = 1; i <= b_pimp; i++)
	{
		...
		if (b_i == INVEN_WIELD || b_i == weapon_swap) borg_keypress('w');
		else borg_keypress('a');
		if (b_i >= INVEN_WIELD)
		{
			borg_keypress('/');
			borg_keypress(I2A(b_i - 24));
		}
		else borg_keypress(I2A(b_i));
		borg_keypress(' ');
	}
...
bool borg_think_shop_bow(void)
{
	...
	if (b_i == -1)
	{
		for (i = 0; i < INVEN_PACK; i++)
		{
			...
			/* Can I afford it ? */
			if (item->iqty * cost > borg_gold) continue;
    ...
    *b_home_power = borg_power_home();
    ...
    for (n = 0; n < STORE_INVEN_MAX; n++)
    {
        ...
        for (i = 0; i < INVEN_PACK; i++)
        {
            ...
            bought_item_pval == item->pval && bought_item_store == BORG_HOME) continue;
...
static bool borg_think_home_sell_aux( bool save_best )
{
	...
	if (borg_skill[BI_TOWN_NUM] == 1 && !vanilla_town) return FALSE;
    ...
    *b_home_power = -1;
        ...
        for (i = 0; i < STORE_INVEN_MAX; i++)
        {
                ...
                (borg_shops[BORG_HOME].ware[i].iqty))
            {
				...
				if  ((item2->tval == TV_WAND || item2->tval == TV_STAFF) &&
					(sold_item_tval == item2->tval && sold_item_sval == item2->sval &&
					 sold_item_store == BORG_HOME)) return (FALSE);
				else if	(sold_item_tval == item2->tval && sold_item_sval == item2->sval &&
    ...
    for (i = 0; i < STORE_INVEN_MAX; i++)
    {
        ...
        if (best[i] != i && best[i] != 255)
        {
			...
			/* Full house */
		    if (borg_shops[BORG_HOME].ware[icky].iqty) return (FALSE);
    ...
    for (k = 0; k < (MAX_STORES) ; k++)
    {
        ...
        for (i = 0; i < INVEN_PACK; i++)
        {
			...
			if (item->tval == TV_WAND && item->sval == SV_WAND_STONE_TO_MUD &&
				item->iqty <= 4 && borg_skill[BI_ASTONE2MUD] < item->iqty * 5) continue;
			...
			if (borg_shops[k].ware[icky].tval == TV_WAND ||
				...
				borg_shops[k].ware[icky].tval == TV_SCROLL) b_q = 1;
...
static bool borg_good_buy(borg_item *item, int who)
{
    ...
    if  ((item->tval == TV_WAND || item->tval == TV_STAFF) &&
		(sold_item_tval == item->tval && sold_item_sval == item->sval &&
            sold_item_store == who)) return (FALSE);
    ...
    for (k = 0; k < (MAX_STORES); k++)
    {
        ...
        for (n = 0; n < STORE_INVEN_MAX; n++)
        {
			...
			if (borg_items[INVEN_LITE].pval == 0 &&
				(item->tval != TV_LITE &&
				 (item->tval != TV_FLASK && borg_items[INVEN_WIELD].sval == SV_LITE_LANTERN))) continue;


			/* Skip it if I just sold it */
            if  ((item->tval == TV_WAND || item->tval == TV_STAFF) &&
				(sold_item_tval == item->tval && sold_item_sval == item->sval &&
                 sold_item_store == k)) continue;
			else if	(sold_item_tval == item->tval && sold_item_sval == item->sval &&
    ...
    for (n = 0; n < STORE_INVEN_MAX; n++)
    {
        ...
        else
        {
			for (q = 1; q <= q_max; q++)
			{
				...
				/* Require one empty slot */
				if (stack == -1 && borg_items[INVEN_PACK-1].iqty) continue;
				if (stack == -1 && borg_items[INVEN_PACK-2].iqty) continue;
...
static bool borg_think_shop_grab_aux(void)
{
    ...
    if (borg_race_death[RACE_OBERON] != 0) return (FALSE);


	/* Wait until I can afford it */
	if (borg_skill[BI_CLEVEL] < 20) return (FALSE);
...
static bool borg_choose_shop(void)
{
    ...
    if (time_this_panel > 850) return (FALSE);

	
    /* If poisoned or bleeding -- flow to temple */
    if (borg_skill[BI_ISCUT] || borg_skill[BI_ISPOISONED]  && borg_gold >= 50) goal_shop = 3;

    /* If Starving or Out of Light -- flow to food */
    if (borg_skill[BI_ISWEAK] || (borg_skill[BI_CUR_LITE] == 0 &&
        borg_skill[BI_CLEVEL] >= 2 && borg_gold >= 5))
        {
    ...
    if ((borg_skill[BI_CUR_LITE] == 0 || borg_skill[BI_ISWEAK] || borg_skill[BI_ISCUT] || borg_skill[BI_ISPOISONED])  && borg_gold >= 5)
    {
       if (borg_think_shop_buy_aux())
       {
		...
		/* If no help in these shops, don't bother selling stuff, consider returning to dungeon */
		return (FALSE);
	}
    ...
    if (borg_think_shop_buy_aux())
    {
        ...
        borg_note(format("# Buying '%s' at '%s' (%d for player)",
                         borg_shops[goal_shop].ware[goal_ware].desc,
                         (f_name + f_info[FEAT_SHOP_HEAD+goal_shop].name), goal_qty));
...
static bool borg_think_shop_sell(void)
{
    ...
    if ((goal_shop == shop_num) && (goal_item >= 0))
    {
        ...
        if (goal_shop != BORG_HOME) borg_send_deinscribe(goal_item);
        ...
        if ((item->tval == TV_POTION &&
            item->sval != SV_POTION_STAR_HEALING &&
            item->sval != SV_POTION_LIFE) || item->tval != TV_POTION)
        {
...
bool borg_think_store(void)
{
    ...
    if (/* shop_num != BORG_HOME && */ borg_best_stuff()) return (TRUE);
	if (shop_num == BORG_HOME && borg_best_combo(TRUE)) return (TRUE);
...
bool borg_think_dungeon_light(void)
{
		...
		borg_skill[BI_CDEPTH] >= 1)
    {
		...
		for (i = 1; i < borg_kills_nxt; i++)
		{
			...
			/* delet all pets */
			if (kill->friendly || kill->pet) borg_delete_kill(i);
		}
		...
		for (ii = 0; ii < 8; ii++)
		{
				...
				(y == c_y && x == c_x))
			{
				...
				if (cave[c_y][c_x].info == CAVE_GLOW) ag->info |= BORG_GLOW;
				continue;
			}
...
bool borg_think_stair_scum(bool from_town)
{
	...
	if (borg_skill[BI_CDEPTH] >= 2)
	{
		...
		if (borg_flow_stair_both_dim(GOAL_FLEE)) return (TRUE);
...
bool borg_think_dungeon_lunal(void)
{
		...
		(borg_items[INVEN_LITE].tval == TV_LITE) && (borg_items[INVEN_LITE].sval == SV_LITE_LANTERN))
    {
        ...
        {
            /* Try to refuel the torch */
            if ((borg_items[INVEN_LITE].pval < 500) &&
                 borg_refuel_torch()) return (TRUE);
        }
        ...
        {
            /* Try to refill the lantern */
            if ((borg_items[INVEN_LITE].pval < 1000) && borg_refuel_lantern()) return (TRUE);
        }
...
/* XXX Should we allow him great flexibility in retreiving loot? (not always safe?)*/
    ...
    if (track_more_num && (safe_place || ag->feat == FEAT_MORE ||
         ag->feat == FEAT_LESS))
    {
        int y, x;
             ...
             ag->feat == FEAT_MORE)
		{
			...
			/* Flow to DownStair */
        	if (borg_flow_stair_more(GOAL_FLEE, TRUE, TRUE)) return (TRUE);
...
bool borg_think_dungeon_munchkin(bool prep_check)
{
	...
	/* Do not climb up if during daylight hours.  Stay deeper in the dungeon until night falls.  Otherwise,
	...
	if (borg_spell_okay_fail(REALM_NATURE, 2, 1, 50)) create_stair = TRUE;
	...
	if (track_more_num >= 1 || track_less_num >= 1)
	{
        ...
        for (i = 0; i < track_more_num; i++)
        {
			...
			if (!borg_projectable(y, x, c_y, c_x, TRUE, TRUE) && !borg_skill[BI_PASSWALL] && !borg_skill[BI_ADIMDOOR]) continue;
	...
	/* No Light at all */
	if (borg_skill[BI_CUR_LITE] == 0 && borg_items[INVEN_LITE].pval == 0 && borg_skill[BI_CDEPTH] == 1 &&
		(borg_mutation(MUT1_ILLUMINE, TRUE, 50, FALSE) || borg_spell_okay(REALM_ARCANE,0, 5) ||
         borg_spell_okay(REALM_CHAOS,0, 2) || borg_spell_okay(REALM_NATURE,0, 4) ||
         borg_spell_okay(REALM_SORCERY,0, 3) ||	borg_spell_okay(REALM_LIFE,0, 4) ||
	...
	{
		/* Heal if needed */
		if (borg_skill[BI_FOOD] >= 2 && borg_heal(borg_danger(c_y,c_x,1,TRUE))) return (TRUE);

		/* Can do a little attacking. */
		if (borg_skill[BI_FOOD] >= 2 && borg_munchkin_magic()) return (TRUE);
		if (borg_skill[BI_FOOD] >= 2 && borg_munchkin_melee()) return (TRUE);
		...
		/* Consume needed things */
		if (borg_skill[BI_ISHUNGRY] && borg_use_things()) return (TRUE);
		if (borg_skill[BI_ISWEAK] && (borg_eat_food_any() || borg_eat_vamp() || borg_eat_unknown() || borg_quaff_unknown())) return (TRUE);
		...
		if (safe_place && borg_crush_hole()) return (TRUE);
		...
		/* Find a (viewable) object */
		if (safe_place && safe_monsters && borg_flow_take_lunal(TRUE, 5 + borg_skill[BI_CLEVEL]/5)) return (TRUE);

		/* Recover from any nasty condition */
		if (safe_place && on_stair && borg_recover()) return (TRUE);
    ...
    /* Full of Items - Going up */
    if ((track_less_num || create_stair) && (borg_items[INVEN_PACK-2].iqty || !borg_items[INVEN_LITE].pval ||
        ...
        (safe_place || ag->feat == FEAT_LESS) && !fearing_sunlight)
    {
		...
		/* Higher level, deeper dungeon, use Recall instead of climbing all the way */
		if (borg_skill[BI_MAXCLEVEL] >= 10 && borg_skill[BI_CDEPTH] >= 8 && borg_recall())
		{
	...
	/* No value in bouncing if on a quest level.  Borg will need to stay and fight the quest monsters */
        ...
        ag->feat == FEAT_MORE)
    {
        ...
        /* if the downstair is close and path is safe, continue on */
        if (b_j != -1 && ((b_j < closeness && safe_place) || ag->feat == FEAT_MORE || borg_skill[BI_CDEPTH] == 1))
		{
        	/* Note */
			...
			/* Flow to DownStair */
			if (borg_flow_stair_more_dim(GOAL_FLEE)) return (TRUE);
        	if (borg_flow_stair_more(GOAL_FLEE, TRUE, TRUE)) return (TRUE);
    ...
    /* Going up */
    if ((((track_less_num || create_stair) && safe_place) || ag->feat == FEAT_LESS))
    {
...
static bool borg_think_dungeon_brave(void)
{
    ...
    if (goal_less)
    {
        ...
        /* Try to find some stairs */
		if ((borg_depth & DEPTH_SCARY) && !borg_skill[BI_CDEPTH] && borg_flow_stair_both_dim(GOAL_FLEE)) return (TRUE);
        ...
        if (borg_flow_stair_less(GOAL_FLEE, TRUE)) return (TRUE);
    }
...
bool borg_think_dungeon(void)
{
    ...
    if (borg_skill[BI_CDEPTH] &&
        time_this_panel >= 350 && time_this_panel <= 399 &&
	...
	/* Quick check on the inventory in case we need to ininscribe something and avoid loops. */
    ...
    /* Use things */
    if (borg_skill[BI_ISWEAK] && borg_use_things()) return (TRUE);
	...
	if (borg_flow_old(GOAL_UNREACH)) return (TRUE);
    ...
    if (borg_skill[BI_CUR_LITE] == 0 || borg_skill[BI_ISCUT] || borg_skill[BI_ISPOISONED] || borg_skill[BI_ISWEAK])
    {
		...
		if (when_waypoint[0] == 0 && borg_flow_town_exit(GOAL_TOWN)) return (TRUE);

		/* Vamps need to eat right away */
		if ((borg_skill[BI_VAMPIRE] || borg_spell_legal_fail(REALM_DEATH, 1, 3, 30)) && borg_skill[BI_ISWEAK])
		{
			if (borg_attack(FALSE, TRUE, -1, FALSE) >= 1) return (TRUE);
			if (borg_flow_old(GOAL_KILL)) return (TRUE);
			if (borg_flow_kill(FALSE, 50)) return (TRUE);
		}

		/* Try to get food from the Inn */
		if (borg_flow_shop_inn()) return (TRUE);
	...
	/* If fighting certain types of creatures, consider the swap gear for better damage. */
	if ((borg_fighting_summoner || borg_fighting_unique || borg_fighting_questor || borg_fighting_dragon || borg_fighting_demon) && borg_wear_swap()) return (TRUE);

	/* Dig an anti-summon corridor */
    if (borg_flow_kill_corridor_1(TRUE)) return (TRUE);

	/* Move to a safe grid and beat monsters with ranged attacks (play nanny-boo-boo) */
	if (borg_flow_kill_unreachable(4)) return (TRUE);
	...
	if (!borg_skill[BI_PASSWALL] && borg_grids[c_y][c_x].feat >= FEAT_DOOR_HEAD && borg_grids[c_y][c_x].feat <= FEAT_PERM_SOLID)
	{
		if (borg_flow_passwall()) return (TRUE);
	}
    ...
    if (borg_skill[BI_CLEVEL] < 10 && borg_skill[BI_MAXSP] >= 2 && borg_skill[BI_CURSP] <= 1)
    {
        if (borg_skill[BI_CDEPTH])
        {
            /* Check for an existing "up stairs" */
    ...
    if (goal_less)
    {
        ...
        /* Try to find some stairs */
		if ((borg_depth & DEPTH_SCARY) && !borg_skill[BI_CDEPTH] && borg_flow_stair_both_dim(GOAL_FLEE)) return (TRUE);
        if ((borg_depth & DEPTH_SCARY) && !borg_skill[BI_CDEPTH] && borg_flow_stair_both(GOAL_FLEE, FALSE, TRUE)) return (TRUE);
    ...
    if (goal_fleeing && !goal_recalling)
    {
		...
		/* Give a short reason */
		if (borg_verbose) borg_note("# Fleeing level but not Recalling.");
    ...
    if (goal_recalling && (borg_danger(c_y, c_x, 1, TRUE) <= 0)&& borg_check_rest(c_y, c_x))
    {
        /* Take note */
    ...
    /* Find a close object */
    if (borg_flow_take(FALSE, 20)) return (TRUE);

	/* Find a close mineral vein */
	if (borg_flow_vein(TRUE, 20)) return (TRUE);

	/* Find a mid-ranged monster */
    if (borg_flow_kill(TRUE, 35)) return (TRUE);

    /* Find an object */
    if (borg_flow_take(FALSE, 150)) return (TRUE);

	/* Find a mineral vein */
	if (borg_flow_vein(TRUE, 150)) return (TRUE);
    ...
    if (borg_pseudoid_stuff()) return (TRUE);
	...
	/* Flow to a different (favorite) town in order to take advantage of other shops */
	if (borg_skill[BI_ENCUMBERD] < 150 && borg_flow_demenager(borg_town_pref, GOAL_TOWN)) return (TRUE);

	/* Try to get to town location (town gate for now) */
	if (when_waypoint[0] == 0 && borg_flow_town_exit(GOAL_TOWN)) return (TRUE);

	/* Specialty shops (Restoration, ID, *ID*) */
	if (borg_flow_shop_special(TRUE)) return (TRUE);
	...
	if (borg_flow_shop_special(FALSE)) return (TRUE);
     ...
     */
     if ((borg_skill[BI_CDEPTH] == 0 && borg_t - borg_began > 800 &&
		 when_waypoint[1] == 0) || borg_t > 28000) old_depth = 126;
