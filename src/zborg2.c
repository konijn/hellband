	...
	if (borg_skill[BI_CURHP] < borg_skill[BI_MAXHP] / 2) {
		...
		/* If a non-LOS monster is attacking me, then it is probably has
		  ...
		  *priest.
			*
			*/
		if ((dist > MAX_RANGE) && (ag->feat == FEAT_NONE))
			return (FALSE);
	} else if (borg_detect_wall[y1 / PANEL_HGT][x1 / PANEL_WID] == TRUE) {
		/* This area has been magic mapped, so I should be able to see the unknown
		 * grids */
		return (TRUE);
	} else {
		/* Assume all unknown grids more than distance 3 from my light limit are
		 * walls. */
		if ((dist > borg_skill[BI_CUR_LITE] + 2) && (ag->feat == FEAT_NONE))
			return (FALSE);
	}
...
/* If a monster is sitting in a darkened area and the borg has ESP, he may not
 * realize that the monster
 * has full LOS to him.  If that monster casts a spell, then it surely has LOS
 * from it's current location.
 * This routine will force the unknown grids between the borg and the monster to
 * be floor grids and therefore
 * BORG_VIEW.
 */
...
bool borg_los(int y1, int x1, int y2, int x2) {
	...
	if (borg_projectable(y1, x1, y2, x2, TRUE, TRUE))
		return (TRUE);
	return (FALSE);
}
 ...
 * stop_door if true will effectively block vision, sometimes its nice to
 * pretend the door
 * is not there. so we set it as false
 */
bool borg_projectable(int y1, int x1, int y2, int x2, bool stop_doors,
							 bool stop_walls) {
	int dist, y, x;
	...
	for (dist = 0; dist <= MAX_RANGE; dist++) {
		...
		if (dist && (stop_walls || !borg_passwalling) &&
			 (ag->feat == FEAT_RUBBLE || ag->feat == FEAT_MAGMA ||
			  ag->feat == FEAT_MAGMA_K || ag->feat == FEAT_WALL_EXTRA ||
			  ag->feat == FEAT_WALL_INNER || ag->feat == FEAT_WALL_OUTER ||
			  ag->feat == FEAT_WALL_SOLID || ag->feat == FEAT_PERM_SOLID ||
			  ag->feat == FEAT_PERM_BUILDING || ag->feat == FEAT_PERM_INNER ||
			  ag->feat == FEAT_PERM_OUTER || ag->feat == FEAT_PERM_INNER ||
			  ag->feat == FEAT_PERM_SOLID || ag->feat == FEAT_WALL_EXTRA))
			break;

		/* Cheat the game code at the end.  Rarely the borg might not calculate
		 * the outside margin correctly. */
...
void borg_cave_lite_hack(int Y, int X) {
	if (Y >= 0 && Y < AUTO_MAX_Y && X >= 0 && X < AUTO_MAX_X) {
		borg_grids[Y][X].info |= BORG_LITE;
...
bool borg_cave_floor_view(borg_grid *ag) {
	if ((ag->feat == FEAT_NONE) || (ag->feat == FEAT_INVIS) ||
		 (ag->feat == FEAT_FLOOR) ||
		 (ag->feat >= FEAT_TRAP_HEAD && ag->feat <= FEAT_TRAP_TAIL) ||
		 (ag->feat == FEAT_LESS) || (ag->feat == FEAT_MORE) ||
		 (ag->feat == FEAT_BROKEN) || (ag->feat == FEAT_OPEN) ||
		 (ag->feat == FEAT_GLYPH) || (ag->feat == FEAT_WATER))
		return (TRUE);
	/*(ag->feat == FEAT_LAVA) ||*/
	return FALSE;
}
...
bool borg_cave_floor_bold(int Y, int X) {
	if (Y >= 0 && Y < AUTO_MAX_Y && X >= 0 && X < AUTO_MAX_X) {
		if ((borg_grids[Y][X].feat == FEAT_FLOOR) ||
			 (borg_grids[Y][X].feat >= FEAT_TRAP_HEAD &&
			  borg_grids[Y][X].feat <= FEAT_TRAP_TAIL) ||
			 ...
			 (borg_grids[Y][X].feat == FEAT_WATER))
			return (TRUE);
...
#define borg_cave_empty_bold(Y, X)                                             \
	(borg_cave_floor_bold(Y, X) && !(borg_grids[Y][X].kill) &&                  \
	 !(((Y) == c_y) && ((X) == c_x)) && !(borg_grids[Y][X].take))
...
void borg_update_lite(void) {
	...
	for (i = 0; i < borg_lite_n; i++) {
		...
		if (borg_grids[y][x].info & BORG_LITE)
			borg_grids[y][x].info &= ~BORG_LITE;
	}
...
static bool borg_update_view_aux(int y, int x, int y1, int x1, int y2, int x2) {
	...
	if (!borg_cave_floor_bold(y, x)) {
		if (distance(c_y, c_x, y, x) > 2 + borg_skill[BI_CUR_LITE])
			wall = TRUE;
		else
			wall = FALSE;
	}
...
void borg_update_view(void) {
	...
	/* Scan south-east */
	for (d = 1; d <= z; d++) {
		if ((y + d > AUTO_MAX_Y - 1) || (x + d > AUTO_MAX_X - 1))
			continue;
		ag = &borg_grids[y + d][x + d];
		ag->info |= BORG_XTRA;
		borg_cave_view_hack(ag, y + d, x + d);
		if (!borg_dynamic_view(y + d, x + d, d))
			break;
		if (!borg_cave_floor_view(ag))
			break;
	}

	/* Scan south-west */
	for (d = 1; d <= z; d++) {
		if ((y + d > AUTO_MAX_Y - 1) || (x - d < 0))
			continue;
		ag = &borg_grids[y + d][x - d];
		ag->info |= BORG_XTRA;
		borg_cave_view_hack(ag, y + d, x - d);
	...
	/* Scan south */
	for (d = 1; d <= full; d++) {
		if ((y + d > AUTO_MAX_Y - 1) || (x > AUTO_MAX_X - 1))
			continue;
		ag = &borg_grids[y + d][x];
		ag->info |= BORG_XTRA;
		borg_cave_view_hack(ag, y + d, x);
		if (!borg_dynamic_view(y + d, x, d))
			break;
		if (!borg_cave_floor_view(ag))
			break;
	}
	...
	/* Scan east */
	for (d = 1; d <= full; d++) {
		if ((y > AUTO_MAX_Y - 1) || (x + d > AUTO_MAX_X - 1))
			continue;
		ag = &borg_grids[y][x + d];
		ag->info |= BORG_XTRA;
		borg_cave_view_hack(ag, y, x + d);
		if (!borg_dynamic_view(y, x + d, d))
			break;
		if (!borg_cave_floor_view(ag))
			break;
	}
	...
	for (n = 1; n <= over / 2; n++) {
		...
		if (xpn < AUTO_MAX_X - 1) {
			...
			if ((ypn <= AUTO_MAX_Y - 1) && (n < es)) {
				...
				for (k = n, d = 1; d <= m; d++) {
					/* Check grid "d" in strip "n", notice "blockage" */
					if (borg_update_view_aux(ypn, xpn + d, ypn - 1, xpn + d - 1, ypn,
													 xpn + d - 1)) {
						if (n + d >= es)
							break;
					}
			...
			if ((ymn >= 0) && (n < en)) {
				...
				for (k = n, d = 1; d <= m; d++) {
					/* Check grid "d" in strip "n", notice "blockage" */
					if (borg_update_view_aux(ymn, xpn + d, ymn + 1, xpn + d - 1, ymn,
													 xpn + d - 1)) {
						if (n + d >= en)
							break;
					}
		...
		if (xmn > 0) {
			...
			if ((ypn <= AUTO_MAX_Y - 1) && (n < ws)) {
				...
				for (k = n, d = 1; d <= m; d++) {
					/* Check grid "d" in strip "n", notice "blockage" */
					if (borg_update_view_aux(ypn, xmn - d, ypn - 1, xmn - d + 1, ypn,
													 xmn - d + 1)) {
						if (n + d >= ws)
							break;
					}
			...
			if ((ymn >= 0) && (n < wn)) {
				...
				for (k = n, d = 1; d <= m; d++) {
					/* Check grid "d" in strip "n", notice "blockage" */
					if (borg_update_view_aux(ymn, xmn - d, ymn + 1, xmn - d + 1, ymn,
													 xmn - d + 1)) {
						if (n + d >= wn)
							break;
					}
...
sint borg_project_path(coord *gp, int range, int y1, int x1, int y2, int x2,
							  u16b flg) {
	int y, x;
	...
	if (ay > ax) {
		...
		while (1) {
			...
			if ((n > 0) && !borg_cave_floor_bold(y, x))
				break;

			/* Sometimes stop at non-initial monsters/players */
			/* Sometimes stop at non-initial monsters/players */
			if (flg & (PROJECT_STOP)) {
				if ((n > 0) && (borg_grids[y][x].kill != 0))
					break;
			}
