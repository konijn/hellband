...
*
* Hack -- all of the "dragons" in a "dragon" pit must be the same "colour",
...
static void build_type7(int yval, int xval)
{
	...
	/* Message */
	if (debug_room) msg_print("Lesser Vault");
...
static void build_type8(int yval, int xval)
{
	...
	/* Message */
	if (debug_room) msg_print("Greater Vault");
	...
	*/
	...
	for (i = 0; i < DUN_ROOMS; i++)
	{
		...
		if (rand_int(DUN_UNUSUAL) < dun_level)
		{
			...
			if (rand_int(DUN_UNUSUAL) < dun_level || 1)
			{
#ifdef FORCE_V_IDX
				...
				if (k < 10)
				{
					...
					{
						if (debug_room) msg_print("Refusing a greater vault.");
					}
				}
				...
				if (k < 25)
				{
					...
					{
						if (debug_room) msg_print("Refusing a lesser vault.");
					}
				}
...
static void town_gen(void)
{
	...
	for (y = 6; y < cur_hgt-6; y++)
	{
		for (x = 4; x < cur_wid-4; x++)
		{
			cave_type *c_ptr = &cave[y][x];
...
void generate_cave(void)
{
	...
	/* HACK - store player's entry point in case we rebuild level */
	old_px = px;
	old_py = py;
	...
	for (num = 0; TRUE; num++)
	{
		...
		if (auto_scum && (num < 100))
		{
				...
				((dun_level >= 40) && (feeling > 5)))
			{
				...
				if (debug_room || debug_hear ||
					debug_peek || debug_xtra)
				{
					/* Message */
					why = "boring level";
				}
