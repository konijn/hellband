...
bool borg_item_icky(borg_item *item)
{
	...
	if (!borg_mindcr_legal(MIND_PSYCHOMETRY, 15)) return (TRUE);
...
bool borg_use_things(void)
{
    ...
    if (borg_skill[BI_ISHUNGRY])
    {
	    ...
	    if (borg_spell(REALM_LIFE, 0, 7) ||
	        borg_spell(REALM_ARCANE, 2, 6) ||
...
bool borg_check_lite(void)
{
    ...
    if (!when_detect_traps || (borg_skill[BI_CURSP] >= borg_skill[BI_MAXSP] * 9 / 10 &&
    ...
    if (!when_detect_doors || (borg_skill[BI_CURSP] >= borg_skill[BI_MAXSP] * 9 / 10 &&
    ...
    if (!when_detect_walls || (borg_skill[BI_CURSP] >= borg_skill[BI_MAXSP] * 9 / 10 &&
    ...
    /* Hack -- check evil every few turns anyway- more fq if low level or passwalling */
    if (!when_detect_evil || (borg_skill[BI_CURSP] >= borg_skill[BI_MAXSP] * 9 / 10 &&
		borg_t - when_detect_evil  >= 75 - (20 - borg_skill[BI_MAXCLEVEL]))) do_evil = TRUE;

    if (borg_skill[BI_PASSWALL] && borg_grids[c_y][c_x].feat >= FEAT_MAGMA && borg_grids[c_y][c_x].feat <= FEAT_WALL_SOLID &&
		(borg_t - when_detect_evil  >= 8)) do_evil = TRUE;

	/* Dont bother if I have ESP */
    if (borg_skill[BI_ESP] || borg_esp) do_evil = FALSE;

    /* Do not do these if monsters near.  Save mana */
    if (!borg_check_rest(c_y, c_x) && (!borg_skill[BI_PASSWALL] ||
		(borg_skill[BI_PASSWALL] && borg_grids[c_y][c_x].feat < FEAT_MAGMA || borg_grids[c_y][c_x].feat > FEAT_WALL_SOLID)))
    {
        do_trap = FALSE;
        do_door = FALSE;
        do_wall = FALSE;
    ...
    if (do_evil)
    {
        ...
        if (borg_spell_fail(REALM_LIFE,0, 0, 20) ||
			...
			borg_mutation(MUT1_SMELL_MON, FALSE, 20, FALSE))
        {
        ...
        (rand_int(100) < 90))
    {
        ...
        for (y = c_y - 2; y <= c_y + 2; y++)
        {
            ...
            for (x = c_x - 2; x <= c_x + 2; x++)
            {
                ...
                if (ag->info & BORG_GLOW && cave[y][x].info ) floors --;
        ...
        for (y = c_y - 2; y <= c_y + 2; y++)
        {
            ...
            for (x = c_x - 2; x <= c_x + 2; x++)
            {
                ...
                if ((ag->info & BORG_GLOW) ||
					(cave[y][x].info & CAVE_GLOW)) floors --;
...
bool borg_on_safe_grid(int y, int x)
{
	...
	if (ag->feat == FEAT_SHAL_LAVA ||
		ag->feat == FEAT_DEEP_LAVA)
	{
		/* Immunity helps */
		if (borg_skill[BI_IFIRE]) return (TRUE);

		/* Invulnerability helps */
		if (borg_goi || borg_wraith) return (TRUE);
	...
	if (ag->feat == FEAT_DEEP_WATER)
	{
		...
		if (borg_skill[BI_ENCUMBERD] <= 0) return (TRUE);
	...
	if (/* (borg_skill[BI_PASSWALL] || borg_race == RACE_SPECTRE) && */
		ag->feat >= FEAT_DOOR_HEAD && ag->feat <= FEAT_PERM_SOLID) return (FALSE);
...
static bool borg_brand_weapon(void)
{
	...
	if (borg_skill[BI_CURSP] <= borg_skill[BI_MAXSP] * 9/10) return (FALSE);
	...
	*/
    if (!borg_spell_okay_fail(REALM_SORCERY, 2, 0, 30) &&
        ...
        for (i = INVEN_WIELD; i < INVEN_BOW; i++)
        {
			...
			/* Check Sorcery */
			if (borg_spell_okay_fail(REALM_SORCERY, 2, 0, 30) &&
				(item->name3 != TEMP_ELEC &&
				 item->name3 != TEMP_COLD && item->name3 != TEMP_FIRE))
			{
				/* Save the info  */
				b_i = i;
	...
	/* Make sure the borg has enough mana to make the spell worth while. Some brands are very expensive. */
	if (borg_skill[BI_CURSP] < min_mana) return (FALSE);
...
static bool borg_bless_weapon(void)
{
	...
	/* Unable to brand */
    if (!borg_spell_okay_fail(REALM_LIFE, 3, 2, 30)) return (FALSE);
	...
	for (i = 0; i < INVEN_BOW; i++)
	{
		...
		if (item->flags3 & TR3_BLESSED) continue;

		/* Skip non weapons (melee/bow) and non-missiles */
		if (borg_wield_slot(item) != INVEN_WIELD &&
			borg_wield_slot(item) != INVEN_BOW &&
			item->tval != TV_SHOT &&
			item->tval != TV_BOLT &&
			item->tval != TV_ARROW) continue;
...
static bool borg_decurse_armour(void)
{
    ...
    if (decurse_armour_swap == 0 || borg_wearing_cursed)
    {
        ...
        else
        {
			...
			if (borg_wield_slot(item) < INVEN_WIELD) return (FALSE);
...
bool borg_enchanting(void)
{
    ...
    if ((borg_t - borg_began > 750 && borg_skill[BI_CDEPTH]) ||
        (borg_t-borg_began > 1350 && !borg_skill[BI_CDEPTH])) return (FALSE);
    ...
    if (borg_bless_weapon()) return (TRUE);
...
bool borg_recharging(void)
{
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        if (item->tval == TV_ROD && item->pval < item->iqty && !borg_munchkin_mode &&
            (borg_spell_okay(REALM_SORCERY,0, 7) ||
			 borg_spell_okay(REALM_CHAOS,2, 2) ||
			 borg_spell_okay(REALM_ARCANE,3,0) ||
			 borg_equips_artifact(ART_THINGOL, INVEN_OUTER)))
        {
            charge = TRUE;
        }
...
static bool borg_consume(int i)
{
    ...
    switch (item->tval)
    {
        ...
        switch (item->sval)
        {
			...
			if (borg_race == RACE_SKELETON && item->sval >= SV_FOOD_MIN_FOOD) return (FALSE);
...
bool borg_crush_junk(void)
{
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
		...
		if (!item->kind && !strstr(item->note, "cursed")) continue;
        ...
        if (item->quest && !item->ident && item->value) continue;
		...
		if (borg_wield_slot(item) >= INVEN_WIELD && !borg_items[borg_wield_slot(item)].iqty) continue;

        /* dont crush our spell books */
        if (item->tval == REALM1_BOOK || item->tval == REALM2_BOOK) continue;

		/* Do not potential speed items */
		if ((item->tval == TV_BOOTS || item->tval == TV_RING) && !item->ident && item->value) continue;

		/* Never crush Rod of Healing or Annihilation*/
		if (item->tval == TV_ROD && item->sval == SV_ROD_HEALING) continue;
        if (item->tval == TV_WAND && item->sval == SV_WAND_ANNIHILATION) continue;

		/* Special fake value added to certain types of missile potions while low clevel.
		 * Override the value of some take items in order for the borg to flow to it and carry it around.
		 * These values will allow the borg to avoid crushing (junk) these items.  He can use them as
	     * thrown weapons
		 */
		if (borg_skill[BI_CLEVEL] < 10 &&
			(item->tval == TV_POTION &&
			 (item->sval == SV_POTION_SLEEP ||
			 item->sval == SV_POTION_SLOWNESS ||
		...
		if (item->kind == K_FIGURINE) continue;

		/* Do not crush the item if it has a resistance which I am lacking */
		if (item->value)
		{
			if ((item->flags2 & TR2_RES_POIS) && !borg_skill[BI_RPOIS]) continue;
			if ((item->flags2 & TR2_RES_SOUND) && !borg_skill[BI_RSND]) continue;
			if ((item->flags2 & TR2_RES_LITE) && !borg_skill[BI_RLITE]) continue;
			if ((item->flags2 & TR2_RES_DARK) && !borg_skill[BI_RDARK]) continue;
			if ((item->flags2 & TR2_RES_CHAOS) && !borg_skill[BI_RKAOS]) continue;
			if ((item->flags2 & TR2_RES_CONF) && !borg_skill[BI_RCONF]) continue;
			if ((item->flags2 & TR2_RES_BLIND) && !borg_skill[BI_RBLIND]) continue;
			if ((item->flags2 & TR2_RES_DISEN) && borg_skill[BI_MAXDEPTH]  >= 50 && !borg_skill[BI_RDIS]) continue;
			if ((item->flags2 & TR2_RES_SHARDS) && borg_skill[BI_MAXDEPTH] >= 50 && !borg_skill[BI_RSHRD]) continue;
			if ((item->flags2 & TR2_RES_NEXUS) && borg_skill[BI_MAXDEPTH] >= 50 && !borg_skill[BI_RNXUS]) continue;
			if ((item->flags2 & TR2_RES_NETHER) && borg_skill[BI_MAXDEPTH] >= 50 && !borg_skill[BI_RNTHR]) continue;
			if ((item->flags2 & TR2_REFLECT) && borg_skill[BI_MAXDEPTH] >= 50 && !borg_skill[BI_REFLECT]) continue;
		}
		...
		if (borg_gold > 200000) value -= 3000;
		if (borg_gold > 300000) value -= 5000;
		if (borg_gold > 500000 && item->tval >= TV_LIFE_BOOK) value -= 25000;
		...
		if (borg_skill[BI_NOEAT] &&
			...
			(item->sval == SV_FOOD_WAYBREAD ||
             ...
             item->sval == SV_FOOD_JERKY ||
             item->sval == SV_FOOD_BISCUIT ||
			 item->sval == SV_FOOD_SLIME_MOLD)) value = 0L;


        /* Skip non "worthless" items */
        if (item->tval >= TV_CORPSE)
        {
			...
			(item->tval == TV_ROD && item->sval == SV_ROD_ILLUMINATION && borg_skill[BI_ALITE] <= 1000) ||
			...
			if (!borg_skill[BI_NO_MELEE])
			{
				if (item->tval == TV_SCROLL &&
					(item->sval == SV_SCROLL_STAR_ENCHANT_WEAPON ||
					 ...
					 (enchant_weapon_swap_to_h >= 1 || enchant_weapon_swap_to_h >=1)) continue;
				if (item->tval == TV_SCROLL &&
					(item->sval == SV_SCROLL_STAR_ENCHANT_WEAPON ||
					 item->sval == SV_SCROLL_ENCHANT_WEAPON_TO_DAM) &&
					 (enchant_weapon_swap_to_d >= 1 || enchant_weapon_swap_to_d >=1)) continue;
			}
			if (item->tval == TV_SCROLL &&
			...
			if (item->tval == TV_SCROLL &&
				(item->sval == SV_SCROLL_ACQUIREMENT ||
				 ...
				 item->sval == SV_SCROLL_ARTIFACT)) continue;
            ...
            if ((borg_skill[BI_MAXDEPTH] < 5 /* || borg_munchkin_mode */)&& value > 0)
                continue;
        ...
        if (strstr(item->note, "special") && !item->fully_identified) continue;
        if (strstr(item->desc, "terrible")&& !item->fully_identified) continue;
        if (strstr(item->desc, "indestructible")&& !item->fully_identified) continue;
		...
		if ((item->tval == TV_ROD || item->tval == TV_STAFF || item->tval == TV_WAND) &&
			(item->pval) && borg_mutation(MUT1_EAT_MAGIC, FALSE, 30, FALSE))
		{
			/* Note and drain */
	        borg_keypress(I2A(i));
			borg_note(format("# Draining charges from %s prior to junking", item->desc));
			return (TRUE);
		}
        ...
        if (!item->name1)
		{
			...
			if (item->value >= 5 && (borg_mutation(MUT1_MIDAS_TCH, FALSE, 30, FALSE) ||
        ...
        else
        {
            ...
            borg_note(format("# Crappy artifact at %d,%d",bad_obj_x[bad_obj_num],bad_obj_y[bad_obj_num]));
			bad_obj_num++;
        }
        borg_keypress(I2A(i));
...
bool borg_crush_hole(void)
{
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        if (item->quest && !item->ident) continue;
		...
		if ((item->tval == TV_BOOTS || item->tval == TV_RING) && !item->ident) continue;
		...
		if (item->tval == TV_LITE && item->pval >=1 && borg_skill[BI_CUR_LITE] == 0) continue;
        ...
        if (item->tval == TV_ROD && item->sval == SV_ROD_TELEPORT_AWAY) continue;
        if (item->tval == TV_ROD && item->sval == SV_ROD_HEALING) continue;
		if (item->tval == TV_ROD && item->sval == SV_ROD_RECALL && borg_skill[BI_RECALL] <= 1000) continue;
		if (item->tval == TV_ROD && item->sval == SV_ROD_ILLUMINATION && borg_skill[BI_ALITE] <= 1000) continue;
		...
		if (item->tval == my_ammo_tval) ammo_slots++;
...
bool borg_crush_slow(void)
{
    ...
    if (borg_skill[BI_SPEED] >= 110 && !borg_skill[BI_ENCUMBERD]) return (FALSE);
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
		...
		/* Skip wands */
		if (item->tval == TV_WAND && borg_skill[BI_CLEVEL] <= 30) continue;

		/* Skip "good" unknown items (unless "icky") */
        if (!item->aware && !borg_item_icky(item)) continue;
        ...
        if (item->tval == TV_SCROLL && item->sval == SV_SCROLL_TELEPORT) continue;
        if (item->tval == TV_ROD && item->sval == SV_ROD_TELEPORT_AWAY) continue;
		if (item->tval == TV_ROD && item->sval == SV_ROD_ILLUMINATION && borg_skill[BI_ALITE] <= 1000) continue;
...
extern bool borg_eat_magic(bool cursed_only, int fail_allowed)
{
	...
	/* Have the capacity? */
	if (!borg_mutation(MUT1_EAT_MAGIC, TRUE, fail_allowed, FALSE)) return (FALSE);
	...
	for (i = 0; i < INVEN_PACK; i++)
    {
		...
		if (item->tval != TV_ROD &&
			item->tval != TV_STAFF &&
			item->tval != TV_WAND) continue;

		/* only checking crap items */
		if (cursed_only && !item->value ) continue;
		...
		if ((item->tval == TV_STAFF && item->sval == SV_STAFF_TELEPORTATION) ||
			(item->tval == TV_ROD && item->sval == SV_ROD_RECALL)) continue;
	...
	/* Drain the Charges */
	if (borg_mutation(MUT1_EAT_MAGIC, FALSE, fail_allowed, FALSE))
	{
...
bool borg_pseudoid_stuff(void)
{
    ...
    if (borg_mindcr_legal(MIND_PSYCHOMETRY, 25)) return (FALSE);
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
		...
		if (item->tval < TV_SHOT ||
			item->tval > TV_RING ||
			item->tval == TV_LITE) continue;

        /* skip weak pseudo id */
        if (strstr(item->desc, "{average")) continue;
        else if (strstr(item->desc, "{curse")) continue;
        else if (strstr(item->desc, "{good")) continue;
        else if (strstr(item->desc, "{excellent")) continue;
        else if (strstr(item->desc, "Quest")) continue;
        else if (strstr(item->desc, "special")) continue;
        else if (strstr(item->desc, "{terrible")) continue;
        else if (strstr(item->desc, "{indestructible")) continue;
...
/* This is used to determine if certain items should be *ID*d.  It is called from test_stuff as well as
...
bool borg_starid_item(borg_item *item)
{
	...
	if (strstr(item->note, "special")) return (TRUE);

	/* Skip *ID*'d items */
    if (item->fully_identified) return (FALSE);
...
bool borg_test_stuff(bool star_id)
{
    ...
    if (star_id)
	{
		for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
		{
			...
			if (borg_starid_item(item)) v = item->value + 100000L;
		...
		for (i = 0; i < INVEN_PACK; i++)
		{
			...
			if (strstr(item->desc, "{good") && !star_id &&
				(borg_class == CLASS_MAGE || borg_class == CLASS_PRIEST || borg_class == CLASS_RANGER)) v = item->value + 10000L;
			/* heavy pseudo id */
			else if (strstr(item->desc, "{good") && !star_id && borg_gold < 10000) v = item->value +1000L;
			else if (strstr(item->desc, "{excellent") && !star_id) v = item->value + 20000L;
			else if (strstr(item->desc, "Quest") && !star_id) v = item->value + 20000L;
	...
	if (!star_id)
	{
		...
		for (i = 0; i < INVEN_PACK; i++)
		{
			...
			if (strstr(item->note, "average") && !star_id && item->tval != TV_RING && item->tval != TV_AMULET) continue;

			/* Try to ID specials/ego right away. */
			if (strstr(item->note, "special")) OK_toID = TRUE;
			if (strstr(item->note, "excellent") && !star_id) OK_toID = TRUE;
			if (strstr(item->note, "Quest") && !star_id) OK_toID = TRUE;

			/* Identify "good" items */
			if (strstr(item->note, "good")) OK_toID = TRUE;

			/* ID the Phial */
			if (strstr(item->desc, "Phial") && !star_id && !item->ident) OK_toID = TRUE;

			/* Hack -- assume no value */
			v = -1;

			/* Ego and Artifacts have intrinsic value */
			if (strstr(item->note, "special") || strstr(item->desc, "Quest") || strstr(item->desc, "{excellent}") ) v = (borg_skill[BI_MAXDEPTH] * 5000L);

			/* {good} have intrinsic value */
			if (strstr(item->note, "good")) v = (borg_skill[BI_MAXDEPTH] * 500L);
			...
			switch (item->tval)
			{
				...
				if (borg_spell_legal(REALM_SORCERY, 1, 1) || borg_spell_legal(REALM_ARCANE,3, 2) ||
            		borg_equips_artifact(ART_ERIRIL, INVEN_WIELD) || borg_spell_legal(REALM_LIFE, 3, 5) ||
            		borg_spell_legal(REALM_DEATH, 3, 2) || borg_mindcr_legal(MIND_PSYCHOMETRY, 25) ||
					borg_equips_activation(ACT_ID_PLAIN, TRUE))
				{
					v += item->value;
				}
				...
				else if ((borg_class == CLASS_ORPHIC) && (rand_int(5000) < borg_skill[BI_CLEVEL]))
				{
...
bool borg_swap_rings(void)
{
    ...
    if (borg_t - borg_began > 350) return (FALSE);
	...
	if (borg_swapped_rings) return (FALSE);
...
bool borg_wear_rings(void)
{
	...
	/* Don't do this during temp effects that significantly alter our power and fear */
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
        ...
        if (!item->aware && !strstr(item->note, "average")) continue;
...
bool borg_backup_swap(int p)
{
	...
	for (i = 0; i <= 1; i++)
	{
		...
		if ((borg_goi || borg_wraith) && i == 1) continue;
		...
		if (borg_fighting_unique && borg_verbose)
		{
		   ...
		   borg_note(format("Swap: Trying Item %s (new power %ld)",borg_items[slot].desc, p));
		   borg_note(format("Swap: Against Item  %s  (old power %ld)",safe_items[slot].desc, b_p));
		}
		...
		/* Examine the critical skills with new gear on.  If i lose a critical skill, then boost the danger */
		if ((save_rconf) && borg_threat_conf && borg_skill[BI_RCONF] == 0) p = 9999;
		if ((save_rblind) && borg_threat_blind &&
			(!borg_skill[BI_RBLIND] &&
			 !borg_skill[BI_RLITE] &&
			 !borg_skill[BI_RDARK] &&
			  borg_skill[BI_SAV] < 100)) p = 9999;
		if ((save_fract) &&
			(!borg_skill[BI_FRACT] &&
			  borg_skill[BI_SAV] < 100 && borg_threat_para)) p = 9999;
		if ((save_seeinvis || save_esp) && borg_threat_invis &&
			(!borg_skill[BI_ESP] && ! borg_skill[BI_SINV])) p = 9999;
    ...
    if ((borg_goi || borg_wraith) && damage2 > damage1 && !borg_skill[BI_NO_MELEE])
    {
        swap = weapon_swap;
...
bool borg_remove_stuff(void)
{
	...
	if (borg_skill[BI_ENCUMBERD]) return (FALSE);
...
bool borg_wear_stuff(void)
{
    ...
    /*    Just come back and work through the loop later */
    if (borg_t - borg_began > 2000) return (FALSE);
	if (borg_t - time_last_swap < 15) return (FALSE);
    ...
    for (i = 0; i < INVEN_PACK; i++)
    {
		...
		if (borg_skill[BI_ENCUMBERD])
		{
			...
			if (((borg_items[slot].flags1 & TR1_STR) &&
				!(item->flags1 & TR1_STR)) ||
			   ((borg_items[slot].flags1 & TR1_STR) &&
				(item->flags1 & TR1_STR) &&
				borg_items[slot].pval > item->pval))
			{
				...
				if ((slot == INVEN_LEFT && (borg_items[INVEN_RIGHT].flags1 & TR1_STR) &&
				!(item->flags1 & TR1_STR)) ||
				(slot == INVEN_LEFT && (borg_items[INVEN_RIGHT].flags1 & TR1_STR) &&
				(item->flags1 & TR1_STR) &&
            ...
            (!borg_items[INVEN_LEFT].tval || !borg_items[INVEN_RIGHT].tval))
        {
			...
			if (slot == INVEN_LEFT && borg_items[INVEN_LEFT].iqty && !borg_items[INVEN_RIGHT].iqty) slot = INVEN_RIGHT;
			...
			/*
			 * If the borg happens to be holding some ammo which does not match his bow (bolts vs shot),
			 * and he finds a new bow to match the ammo, he will mistakenly swap out the good bow for
			 * the new one becase the ammo matches.  After doing this simulated check, he just defined a
			 * new my_ammo_type.  So we compare this new value to the original tval.
    ...
    if (b_i >= 0)
    {
		...
		{
			/* Log */
            borg_note(format("# Removing %s to go as unarmed monk.", &borg_items[INVEN_WIELD].desc));
...
static void borg_best_stuff_aux2(int n, byte *test, byte *best, s32b *vp)
{
    ...
    for (i = 0; i < ((shop_num == BORG_HOME) ? (INVEN_PACK + STORE_INVEN_MAX) : INVEN_PACK); i++)
    {
		...
		/* Reset our recently worn flag.  This item shouldnt inherit the flag from the previous item. */
		recently_worn = FALSE;
...
{
	int hole = INVEN_PACK-1;
	int weapon, bow, ring_r, ring_l, neck, outer, shield, helm, glove, boot, lite, body, i;
    s32b value;
    s32b b_value;
	int track, k;
	...
	char b_bow_desc[80];
	char b_ring_r_desc[80];
	char b_ring_l_desc[80];
	char b_neck_desc[80];
	...
	/* Hack -- Anti-loop */
    if (time_this_panel >= 300) return (FALSE);
    if (borg_t - borg_began >= 600) return (FALSE);

	/* Not if stats are drained at all */
	if (borg_skill[BI_CURHP] < borg_skill[BI_MAXHP]) return (FALSE);
	...
	/*
	 * Nest the equipment checks.  This routine will check every permutation of equipment combinations.
	 * It is a bit slow, but will positively identify the best set up for maximum power.
	...
	if (borg_verbose)
	{
		borg_note(format("# Best Combo: Attempting every permutation of %d weapons and", weapon_cnt));
		borg_note(format("#             Attempting every permutation of %d bows and", bow_cnt));
		borg_note(format("#             Attempting every permutation of %d rings and", ring_l_cnt));
		borg_note(format("#             Attempting every permutation of %d amulets and", neck_cnt));
		borg_note(format("#             Attempting every permutation of %d lite and", lite_cnt));
		borg_note(format("#             Attempting every permutation of %d armors and", body_cnt));
		borg_note(format("#             Attempting every permutation of %d shields and", shield_cnt));
		borg_note(format("#             Attempting every permutation of %d helms and", helm_cnt));
		borg_note(format("#             Attempting every permutation of %d gloves and", glove_cnt));
		borg_note(format("#             Attempting every permutation of %d boots and", boot_cnt));
	}
	...
	for (weapon = 0; weapon < weapon_cnt; weapon++)
	{
		...
		{
			COPY(&borg_items[INVEN_WIELD], &borg_items[n_weapon[weapon]], borg_item);
		}
		else
		{
			COPY(&borg_items[INVEN_WIELD], &borg_shops[BORG_HOME].ware[n_weapon[weapon]-INVEN_TOTAL], borg_item);
		}

		/* Move the original item into the new items orignal inventory spot */
		if (n_weapon[weapon] < INVEN_TOTAL)
		{
			COPY(&borg_items[n_weapon[weapon]], &safe_items[INVEN_WIELD], borg_item);
		}
		else
		{
			COPY(&borg_shops[BORG_HOME].ware[n_weapon[weapon]-INVEN_TOTAL], &safe_items[INVEN_WIELD], borg_item);
		}
		...
		if (borg_verbose)
		{
			...
			for (bow = 0; bow < bow_cnt; bow++)
			{
				...
				{
					COPY(&borg_items[INVEN_BOW], &borg_shops[BORG_HOME].ware[n_bow[bow]-INVEN_TOTAL], borg_item);
				}
				...
				{
					COPY(&borg_shops[BORG_HOME].ware[n_bow[bow]-INVEN_TOTAL], &safe_items[INVEN_WIELD], borg_item);
				}
				...
				borg_note(format(" BOW: %s",borg_items[INVEN_BOW].desc));
					...
					for (neck = 0; neck < neck_cnt; neck++)
					{
						...
						{
							COPY(&borg_items[INVEN_NECK], &borg_items[n_neck[neck]], borg_item);
						}
						else
						{
							COPY(&borg_items[INVEN_NECK], &borg_shops[BORG_HOME].ware[n_neck[neck]-INVEN_TOTAL], borg_item);
						}

						/* Move the original item into the new items orignal inventory spot */
						if (n_neck[neck] < INVEN_TOTAL)
						{
							COPY(&borg_items[n_neck[neck]], &safe_items[INVEN_NECK], borg_item);
						}
						else
						{
							COPY(&borg_shops[BORG_HOME].ware[n_neck[neck]-INVEN_TOTAL], &safe_items[INVEN_NECK], borg_item);
						}
						...
						{
							/* Move my current item to a safe place */
							COPY(&safe_items[INVEN_OUTER], &borg_items[INVEN_OUTER], borg_item);

							/* Move the new item into the weild spot */
							if (n_outer[outer] < INVEN_TOTAL)
							{
								COPY(&borg_items[INVEN_OUTER], &borg_items[n_outer[outer]], borg_item);
							}
							else
							{
								COPY(&borg_items[INVEN_OUTER], &borg_shops[BORG_HOME].ware[n_outer[outer]-INVEN_TOTAL], borg_item);
							}

							/* Move the original item into the new items orignal inventory spot */
							if (n_outer[outer] < INVEN_TOTAL)
							{
								COPY(&borg_items[n_outer[outer]], &safe_items[INVEN_OUTER], borg_item);
							}
							else
							{
								COPY(&borg_shops[BORG_HOME].ware[n_outer[outer]-INVEN_TOTAL], &safe_items[INVEN_OUTER], borg_item);
							}

							if (borg_verbose)
							{
							/* dump list and power...  for debugging */
							borg_note(format("   Outer: %s",borg_items[INVEN_OUTER].desc));
							}
							...
							{
								/* Move my current item to a safe place */
								COPY(&safe_items[INVEN_ARM], &borg_items[INVEN_ARM], borg_item);

								/* Move the new item into the weild spot */
								if (n_shield[shield] < INVEN_TOTAL)
								{
									COPY(&borg_items[INVEN_ARM], &borg_items[n_shield[shield]], borg_item);
								}
								else
								{
									COPY(&borg_items[INVEN_ARM], &borg_shops[BORG_HOME].ware[n_shield[shield]-INVEN_TOTAL], borg_item);
								}

								/* Move the original item into the new items orignal inventory spot */
								if (n_shield[shield] < INVEN_TOTAL)
								{
									COPY(&borg_items[n_shield[shield]], &safe_items[INVEN_ARM], borg_item);
								}
								else
								{
									COPY(&borg_shops[BORG_HOME].ware[n_shield[shield]-INVEN_TOTAL], &safe_items[INVEN_ARM], borg_item);
								}

								if (borg_verbose)
								{
								/* dump list and power...  for debugging */
								borg_note(format("    Shield: %s",borg_items[INVEN_ARM].desc));
								}
								...
								{
									/* Move my current item to a safe place */
									COPY(&safe_items[INVEN_HEAD], &borg_items[INVEN_HEAD], borg_item);

									/* Move the new item into the weild spot */
									if (n_helm[helm] < INVEN_TOTAL)
									{
										COPY(&borg_items[INVEN_HEAD], &borg_items[n_helm[helm]], borg_item);
									}
									else
									{
										COPY(&borg_items[INVEN_HEAD], &borg_shops[BORG_HOME].ware[n_helm[helm]-INVEN_TOTAL], borg_item);
									}

									/* Move the original item into the new items orignal inventory spot */
									if (n_helm[helm] < INVEN_TOTAL)
									{
										COPY(&borg_items[n_helm[helm]], &safe_items[INVEN_HEAD], borg_item);
									}
									else
									{
										COPY(&borg_shops[BORG_HOME].ware[n_helm[helm]-INVEN_TOTAL], &safe_items[INVEN_HEAD], borg_item);
									}

									if (borg_verbose)
									{
									/* dump list and power...  for debugging */
									borg_note(format("     Helm: %s",borg_items[INVEN_HEAD].desc));
									}
									...
									{
										/* Move my current item to a safe place */
										COPY(&safe_items[INVEN_HANDS], &borg_items[INVEN_HANDS], borg_item);

										/* Move the new item into the weild spot */
										if (n_glove[glove] < INVEN_TOTAL)
										{
											COPY(&borg_items[INVEN_HANDS], &borg_items[n_glove[glove]], borg_item);
										}
										else
										{
											COPY(&borg_items[INVEN_HANDS], &borg_shops[BORG_HOME].ware[n_glove[glove]-INVEN_TOTAL], borg_item);
										}

										/* Move the original item into the new items orignal inventory spot */
										if (n_glove[glove] < INVEN_TOTAL)
										{
											COPY(&borg_items[n_glove[glove]], &safe_items[INVEN_HANDS], borg_item);
										}
										else
										{
											COPY(&borg_shops[BORG_HOME].ware[n_glove[glove]-INVEN_TOTAL], &safe_items[INVEN_HANDS], borg_item);
										}

										if (borg_verbose)
										{
										/* dump list and power...  for debugging */
										borg_note(format("      Glove: %s",borg_items[INVEN_HANDS].desc));
										}
										...
										{
											/* Move my current item to a safe place */
											COPY(&safe_items[INVEN_FEET], &borg_items[INVEN_FEET], borg_item);

											/* Move the new item into the weild spot */
											if (n_boot[boot] < INVEN_TOTAL)
											{
												COPY(&borg_items[INVEN_FEET], &borg_items[n_boot[boot]], borg_item);
											}
											else
											{
												COPY(&borg_items[INVEN_FEET], &borg_shops[BORG_HOME].ware[n_boot[boot]-INVEN_TOTAL], borg_item);
											}

											/* Move the original item into the new items orignal inventory spot */
											if (n_boot[boot] < INVEN_TOTAL)
											{
												COPY(&borg_items[n_boot[boot]], &safe_items[INVEN_FEET], borg_item);
											}
											else
											{
												COPY(&borg_shops[BORG_HOME].ware[n_boot[boot]-INVEN_TOTAL], &safe_items[INVEN_FEET], borg_item);
											}

											if (borg_verbose)
											{
											/* dump list and power...  for debugging */
											borg_note(format("       Boots: %s",borg_items[INVEN_FEET].desc));
											}
											...
											{
												/* Move my current item to a safe place */
												COPY(&safe_items[INVEN_LITE], &borg_items[INVEN_LITE], borg_item);

												/* Move the new item into the weild spot */
												if (n_lite[lite] < INVEN_TOTAL)
												{
													COPY(&borg_items[INVEN_LITE], &borg_items[n_lite[lite]], borg_item);
												}
												else
												{
													COPY(&borg_items[INVEN_LITE], &borg_shops[BORG_HOME].ware[n_lite[lite]-INVEN_TOTAL], borg_item);
												}

												/* Move the original item into the new items orignal inventory spot */
												if (n_lite[lite] < INVEN_TOTAL)
												{
													COPY(&borg_items[n_lite[lite]], &safe_items[INVEN_LITE], borg_item);
												}
												else
												{
													COPY(&borg_shops[BORG_HOME].ware[n_lite[lite]-INVEN_TOTAL], &safe_items[INVEN_LITE], borg_item);
												}

												if (borg_verbose)
												{
												/* dump list and power...  for debugging */
												borg_note(format("        Lite: %s",borg_items[INVEN_LITE].desc));
												}
												...
												{
													/* Move my current item to a safe place */
													COPY(&safe_items[INVEN_BODY], &borg_items[INVEN_BODY], borg_item);

													/* Move the new item into the weild spot */
													if (n_body[body] < INVEN_TOTAL)
													{
														COPY(&borg_items[INVEN_BODY], &borg_items[n_body[body]], borg_item);
													}
													else
													{
														COPY(&borg_items[INVEN_BODY], &borg_shops[BORG_HOME].ware[n_body[body]-INVEN_TOTAL], borg_item);
													}

													/* Move the original item into the new items orignal inventory spot */
													if (n_body[body] < INVEN_TOTAL)
													{
														COPY(&borg_items[n_body[body]], &safe_items[INVEN_BODY], borg_item);
													}
													else
													{
														COPY(&borg_shops[BORG_HOME].ware[n_body[body]-INVEN_TOTAL], &safe_items[INVEN_BODY], borg_item);
													}
													...
													{
														/* dump list and power...  for debugging */
														borg_note(format("         Armor: %s (power %ld)",borg_items[INVEN_BODY].desc, value));
													}
													/* Restore original item to the place holder */
													if (n_body[body] < INVEN_TOTAL)
													{
														COPY(&safe_items[INVEN_BODY], &borg_items[n_body[body]], borg_item);
													}
													else
													{
														COPY(&safe_items[INVEN_BODY], &borg_shops[BORG_HOME].ware[n_body[body]-INVEN_TOTAL], borg_item);
													}

													/* Restore tested item back into its original inventory place */
													if (n_body[body] < INVEN_TOTAL)
													{
														COPY(&borg_items[n_body[body]], &borg_items[INVEN_BODY], borg_item);
													}
													else
													{
														COPY(&borg_shops[BORG_HOME].ware[n_body[body]-INVEN_TOTAL], &borg_items[INVEN_BODY], borg_item);
													}

													/* Restore original item back into its original place. */
													COPY(&borg_items[INVEN_BODY], &safe_items[INVEN_BODY], borg_item);

													/* Ignore "bad" swaps */
													if (value <= b_value) continue;

													/* Maintain the "best" */
													 b_value = value;
													 b_weapon = borg_best_stuff_order[INVEN_WIELD] = weapon;
													 b_bow = borg_best_stuff_order[INVEN_BOW] = bow;
													 /* b_ring_r =  borg_best_stuff_order[INVEN_LEFT] = ring_r; */
													 /* b_ring_l =  borg_best_stuff_order[INVEN_RIGHT] = ring_l; */
													 b_neck =  borg_best_stuff_order[INVEN_NECK] = neck;
													 b_outer =  borg_best_stuff_order[INVEN_OUTER] = outer;
													 b_shield =  borg_best_stuff_order[INVEN_ARM] = shield;
													 b_helm =  borg_best_stuff_order[INVEN_HEAD] = helm;
													 b_glove =  borg_best_stuff_order[INVEN_HANDS] = glove;
													 b_boot =  borg_best_stuff_order[INVEN_FEET] = boot;
													 b_lite =  borg_best_stuff_order[INVEN_LITE] = lite;
													 b_body =  borg_best_stuff_order[INVEN_BODY] = body;

													/* Store descriptions for later */
													if (n_weapon[b_weapon] < INVEN_TOTAL)
														strcpy(b_weapon_desc, borg_items[n_weapon[b_weapon]].desc);
													else
														strcpy(b_weapon_desc, borg_shops[BORG_HOME].ware[n_weapon[b_weapon] - INVEN_TOTAL].desc);

													if (n_bow[b_bow] < INVEN_TOTAL)
														strcpy(b_bow_desc, borg_items[n_bow[b_bow]].desc);
													else
														strcpy(b_bow_desc, borg_shops[BORG_HOME].ware[n_bow[b_bow] - INVEN_TOTAL].desc);

													if (n_neck[b_neck] < INVEN_TOTAL)
														strcpy(b_neck_desc, borg_items[n_neck[b_neck]].desc);
													else
														strcpy(b_neck_desc, borg_shops[BORG_HOME].ware[n_neck[b_neck] - INVEN_TOTAL].desc);

													if (n_outer[b_outer] < INVEN_TOTAL)
														strcpy(b_outer_desc, borg_items[n_outer[b_outer]].desc);
													else
														strcpy(b_outer_desc, borg_shops[BORG_HOME].ware[n_outer[b_outer] - INVEN_TOTAL].desc);

													if (n_shield[b_shield] < INVEN_TOTAL)
														strcpy(b_shield_desc, borg_items[n_shield[b_shield]].desc);
													else
														strcpy(b_shield_desc, borg_shops[BORG_HOME].ware[n_shield[b_shield] - INVEN_TOTAL].desc);

													if (n_helm[b_helm] < INVEN_TOTAL)
														strcpy(b_helm_desc, borg_items[n_helm[b_helm]].desc);
													else
														strcpy(b_helm_desc, borg_shops[BORG_HOME].ware[n_helm[b_helm] - INVEN_TOTAL].desc);

													if (n_glove[b_glove] < INVEN_TOTAL)
														strcpy(b_glove_desc, borg_items[n_glove[b_glove]].desc);
													else
														strcpy(b_glove_desc, borg_shops[BORG_HOME].ware[n_glove[b_glove] - INVEN_TOTAL].desc);

													if (n_boot[b_boot] < INVEN_TOTAL)
														strcpy(b_boot_desc, borg_items[n_boot[b_boot]].desc);
													else
														strcpy(b_boot_desc, borg_shops[BORG_HOME].ware[n_boot[b_boot] - INVEN_TOTAL].desc);

													if (n_lite[b_lite] < INVEN_TOTAL)
														strcpy(b_lite_desc, borg_items[n_lite[b_lite]].desc);
													else
														strcpy(b_lite_desc, borg_shops[BORG_HOME].ware[n_lite[b_lite] - INVEN_TOTAL].desc);

													if (n_body[b_body] < INVEN_TOTAL)
														strcpy(b_body_desc, borg_items[n_body[b_body]].desc);
													else
														strcpy(b_body_desc, borg_shops[BORG_HOME].ware[n_body[b_body] - INVEN_TOTAL].desc);

													if (borg_verbose)
													{
														/* dump list and power...  for debugging */
														borg_note(format("          Recommend change:  %ld, %ld",b_value, value));
													}
												...
												if (n_lite[lite] < INVEN_TOTAL)
												{
													COPY(&safe_items[INVEN_LITE], &borg_items[n_lite[lite]], borg_item);
												}
												else
												{
													COPY(&safe_items[INVEN_LITE], &borg_shops[BORG_HOME].ware[n_lite[lite]-INVEN_TOTAL], borg_item);
												}

												/* Restore tested item back into its original inventory place */
												if (n_lite[lite] < INVEN_TOTAL)
												{
													COPY(&borg_items[n_lite[lite]], &borg_items[INVEN_LITE], borg_item);
												}
												else
												{
													COPY(&borg_shops[BORG_HOME].ware[n_lite[lite]-INVEN_TOTAL], &borg_items[INVEN_LITE], borg_item);
												}

												/* Restore original item back into its original place. */
												COPY(&borg_items[INVEN_LITE], &safe_items[INVEN_LITE], borg_item);
											}
											...
											if (n_boot[boot] < INVEN_TOTAL)
											{
												COPY(&safe_items[INVEN_FEET], &borg_items[n_boot[boot]], borg_item);
											}
											else
											{
												COPY(&safe_items[INVEN_FEET], &borg_shops[BORG_HOME].ware[n_boot[boot]-INVEN_TOTAL], borg_item);
											}

											/* Restore tested item back into its original inventory place */
											if (n_boot[boot] < INVEN_TOTAL)
											{
												COPY(&borg_items[n_boot[boot]], &borg_items[INVEN_FEET], borg_item);
											}
											else
											{
												COPY(&borg_shops[BORG_HOME].ware[n_boot[boot]-INVEN_TOTAL], &borg_items[INVEN_FEET], borg_item);
											}

											/* Restore original item back into its original place. */
											COPY(&borg_items[INVEN_FEET], &safe_items[INVEN_FEET], borg_item);
										}
										...
										if (n_glove[glove] < INVEN_TOTAL)
										{
											COPY(&safe_items[INVEN_HANDS], &borg_items[n_glove[glove]], borg_item);
										}
										else
										{
											COPY(&safe_items[INVEN_HANDS], &borg_shops[BORG_HOME].ware[n_glove[glove]-INVEN_TOTAL], borg_item);
										}

										/* Restore tested item back into its original inventory place */
										if (n_glove[glove] < INVEN_TOTAL)
										{
											COPY(&borg_items[n_glove[glove]], &borg_items[INVEN_HANDS], borg_item);
										}
										else
										{
											COPY(&borg_shops[BORG_HOME].ware[n_glove[glove]-INVEN_TOTAL], &borg_items[INVEN_HANDS], borg_item);
										}

										/* Restore original item back into its original place. */
										COPY(&borg_items[INVEN_HANDS], &safe_items[INVEN_HANDS], borg_item);
									}

									/* Restore original item to the place holder */
									if (n_helm[helm] < INVEN_TOTAL)
									{
										COPY(&safe_items[INVEN_HEAD], &borg_items[n_helm[helm]], borg_item);
									}
									else
									{
										COPY(&safe_items[INVEN_HEAD], &borg_shops[BORG_HOME].ware[n_helm[helm]-INVEN_TOTAL], borg_item);
									}

									/* Restore tested item back into its original inventory place */
									if (n_helm[helm] < INVEN_TOTAL)
									{
										COPY(&borg_items[n_helm[helm]], &borg_items[INVEN_HEAD], borg_item);
									}
									else
									{
										COPY(&borg_shops[BORG_HOME].ware[n_helm[helm]-INVEN_TOTAL], &borg_items[INVEN_HEAD], borg_item);
									}

									/* Restore original item back into its original place. */
									COPY(&borg_items[INVEN_HEAD], &safe_items[INVEN_HEAD], borg_item);
								}
								...
								if (n_shield[shield] < INVEN_TOTAL)
								{
									COPY(&safe_items[INVEN_ARM], &borg_items[n_shield[shield]], borg_item);
								}
								else
								{
									COPY(&safe_items[INVEN_ARM], &borg_shops[BORG_HOME].ware[n_shield[shield]-INVEN_TOTAL], borg_item);
								}

								/* Restore tested item back into its original inventory place */
								if (n_shield[shield] < INVEN_TOTAL)
								{
									COPY(&borg_items[n_shield[shield]], &borg_items[INVEN_ARM], borg_item);
								}
								else
								{
									COPY(&borg_shops[BORG_HOME].ware[n_shield[shield]-INVEN_TOTAL], &borg_items[INVEN_ARM], borg_item);
								}

								/* Restore original item back into its original place. */
								COPY(&borg_items[INVEN_ARM], &safe_items[INVEN_ARM], borg_item);
							}

							/* Restore original item to the place holder */
							if (n_outer[outer] < INVEN_TOTAL)
							{
								COPY(&safe_items[INVEN_OUTER], &borg_items[n_outer[outer]], borg_item);
							}
							else
							{
								COPY(&safe_items[INVEN_OUTER], &borg_shops[BORG_HOME].ware[n_outer[outer]-INVEN_TOTAL], borg_item);
							}

							/* Restore tested item back into its original inventory place */
							if (n_outer[outer] < INVEN_TOTAL)
							{
								COPY(&borg_items[n_outer[outer]], &borg_items[INVEN_OUTER], borg_item);
							}
							else
							{
								COPY(&borg_shops[BORG_HOME].ware[n_outer[outer]-INVEN_TOTAL], &borg_items[INVEN_OUTER],  borg_item);
							}

							/* Restore original item back into its original place. */
							COPY(&borg_items[INVEN_OUTER], &safe_items[INVEN_OUTER], borg_item);
						}

						/* Restore original item to the place holder */
						if (n_neck[neck] < INVEN_TOTAL)
						{
							COPY(&safe_items[INVEN_NECK], &borg_items[n_neck[neck]], borg_item);
						}
						else
						{
							COPY(&safe_items[INVEN_NECK], &borg_shops[BORG_HOME].ware[n_neck[neck]-INVEN_TOTAL], borg_item);
						}

						/* Restore tested item back into its original inventory place */
						if (n_neck[neck] < INVEN_TOTAL)
						{
							COPY(&borg_items[n_neck[neck]], &borg_items[INVEN_NECK], borg_item);
						}
						else
						{
							COPY(&borg_shops[BORG_HOME].ware[n_neck[neck]-INVEN_TOTAL], &borg_items[INVEN_NECK], borg_item);
						}
				...
				{
					COPY(&safe_items[INVEN_BOW], &borg_shops[BORG_HOME].ware[n_bow[bow]-INVEN_TOTAL], borg_item);
				}
				...
				{
					COPY(&borg_shops[BORG_HOME].ware[n_bow[bow]-INVEN_TOTAL], &borg_items[INVEN_BOW], borg_item);
				}
		...
		if (n_weapon[weapon] < INVEN_TOTAL)
		{
			COPY(&safe_items[INVEN_WIELD], &borg_items[n_weapon[weapon]], borg_item);
		}
		else
		{
			COPY(&safe_items[INVEN_WIELD], &borg_shops[BORG_HOME].ware[n_weapon[weapon]-INVEN_TOTAL], borg_item);
		}

		/* Restore tested item back into its original inventory place */
		if (n_weapon[weapon] < INVEN_TOTAL)
		{
			COPY(&borg_items[n_weapon[weapon]], &borg_items[INVEN_WIELD], borg_item);
		}
		else
		{
			COPY(&borg_shops[BORG_HOME].ware[n_weapon[weapon]-INVEN_TOTAL], &borg_items[INVEN_WIELD], borg_item);
		}
	...
	/* Report best combo */
		if (b_weapon >= 0) borg_note(format("# Best Weapon address: %d, %s", b_weapon, b_weapon_desc));
		if (b_bow >= 0) borg_note(format("# Best Bow address: %d, %s", b_bow,b_bow_desc));
		if (b_neck >= 0) borg_note(format("# Best Neck address: %d, %s", b_neck, b_neck_desc));
		if (b_outer >= 0) borg_note(format("# Best Outer address: %d, %s", b_outer, b_outer_desc));
		if (b_shield >= 0) borg_note(format("# Best Shield address: %d, %s", b_shield, b_shield_desc));
		if (b_helm >= 0) borg_note(format("# Best Helm address: %d, %s", b_helm, b_helm_desc));
		if (b_glove >= 0) borg_note(format("# Best Glove address: %d, %s", b_glove, b_glove_desc));
		if (b_boot >= 0) borg_note(format("# Best Boot address: %d, %s", b_boot, b_boot_desc));
		if (b_lite >= 0) borg_note(format("# Best Lite address: %d, %s", b_lite, b_lite_desc));
		if (b_body >= 0) borg_note(format("# Best Body address: %d, %s", b_body, b_body_desc));
#if 0
	/* Make first change. */
    for (k = 0; k < 12; k++)
    {
    ...
    if (borg_verbose)
	{
		for (k = 0; k < 12; k++)
		{
			...
			else
			{
				i-=100;
				item = &borg_shops[BORG_HOME].ware[i];
				borg_note(format("# Best Combo %d: %s. (home item)", k, item->desc));
			}
	...
	for (realm = MAX_REALM; realm > 0; realm --)
	{
	    ...
	    for (book = 3; book >= 0; book--)
	    {
	        ...
	        for (spell = 8 - 1; spell >= 0; spell--)
	        {
				...
				if (as->method == BORG_MAGIC_ICK) continue;

	            /* Make sure I have enough mana */
	            if (borg_skill[BI_CURSP] < as->power) continue;

				/* If in munchkin mode, do not test spells which cause me to move off my stair */
				if (borg_munchkin_mode)
				{
					/* Check the different realms for Phase Door */
					if (realm == REALM_SORCERY && book == 0 && spell == 1) continue;
					if (realm == REALM_CHAOS && book == 0 && spell == 7) continue;
					if (realm == REALM_TRUMP && book == 0 && spell == 0) continue;
					if (realm == REALM_TRUMP && book == 0 && spell == 4) continue;
					if (realm == REALM_TRUMP && book == 0 && spell == 5) continue;
					if (realm == REALM_ARCANE && book == 0 && spell == 4) continue;
				}
...
bool borg_leave_level(bool bored)
{
	...
	/* Not bored if I have seen Morgoth recently or a questor */
	if ((borg_skill[BI_CDEPTH] == MORGOTH_DEPTH && (borg_depth & DEPTH_QUEST) &&
    ...
    if (!borg_skill[BI_CDEPTH])
    {
		...
		if (borg_gold > 1000 &&
			((borg_skill[BI_RECALL] <= 2) ||
			 ...
			 (borg_skill[BI_MAXDEPTH] == 100 && borg_skill[BI_ATELEPORTLVL] == 0) ||
			 ((cptr)NULL != borg_prepared[borg_skill[BI_MAXDEPTH]*6/10])))
		{
			/* Flow to Trump Tower */
			if (borg_flow_shop_trump()) return (TRUE);
		}
        ...
        if (borg_skill[BI_MAXDEPTH] == 100 && !borg_plays_risky)
        {
            ...
            {
                /* Note */
                borg_note("# Fleeing.  Borg must crawl to deep dungeon- no recall to 100.");
        ...
        else
        {
            ...
            else
			{
				...
				else if (borg_skill[BI_FEAR_LITE]) borg_note("# Need RLight before I can recall down.");
				else
                {
	...
	/* Avoid sitting on level 1 during daylight hours.  Try to stay deeper in the dungeon until night approaches. */
    if (borg_skill[BI_FEAR_LITE] &&
	    (borg_skill[BI_HRTIME] >= 5) &&
		(borg_skill[BI_HRTIME] <= 18)) fearing_sunlight = TRUE;
    ...
    if ((cptr)NULL == borg_prepared[borg_skill[BI_CDEPTH] + 1] && k < 13) g = 1;
    ...
    if ((cptr)NULL != borg_prepared[borg_skill[BI_CDEPTH]] && !(borg_depth & DEPTH_QUEST))
    {
     ...
     * the final battle
     */
    if (borg_skill[BI_CDEPTH] == 99 && borg_race_death[RACE_OBERON] == 1 &&
    ...
    if ((borg_worships_gold && borg_skill[BI_MAXCLEVEL] <= 20) && (k >= 12) && !fearing_sunlight)
    {
    ...
    if (bored && borg_skill[BI_ISFIXEXP] && borg_skill[BI_CLEVEL] !=50 && !fearing_sunlight &&
    ...
    if ((!goal_rising && bored && !(borg_depth & DEPTH_QUEST) && !(borg_depth & DEPTH_VAULT) &&
         !borg_fighting_unique  && !fearing_sunlight &&
    ...
    if (g < 0)
    {
        ...
        if (borg_flow_stair_less(GOAL_BORE, FALSE)) return (TRUE);
