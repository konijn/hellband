 ...
 *
 * This version of the "APWBorg" is designed for use with Hellband
 * It is meant to get modified as Hellband gets modified
 *
 ...
 * when asked if you really want to use the Borg.  This will
 ...
 *
 * There is a minor cheat on features, monsters, and objects where the borg
 * asks the game instead of using the 'l'ook command, this speeds up the borg
 * tremendously
 *
 ...
 *
 * While this will remain true, the borg will be borrowing more  Hellband code
 * to reduce double maintenance.
 *
 ...
 *
 * Note that the Borg can play any class, that is, he can make
 ...
 *
 * The Borg assumes that the "maximise" flag is off, and that the
 * "preserve" flag is on, since he cannot actually set those flags.
 * If the "maximise" flag is on, the Borg may not work correctly.
 * If the "preserve" flag is off, the Borg may miss artefacts.
 */
 ...
 *
 * Cheats that the Borg would love, but will never happen:
...
static void borg_hidden(void) {
	...
	for (i = 0; i < 6; i++) {
		...
		if (!my_need_stat_check[i])
			continue;

		/* Reverse known bonuses to get the base stat value */
		modify_stat_value(borg_stat[i], -my_stat_add[i]);
...
static bool borg_think(void) {
	...
	if (borg_do_spell) {
		...
		for (ii = 1; ii < MAX_REALM + 1; ii++) {
			/* skip non my realms */
			if (ii != borg_skill[BI_REALM1] && ii != borg_skill[BI_REALM2])
				continue;
	...
	if ((0 == borg_what_text(3, 5, 16, &t_a, buf)) &&
		...
		if (0 == borg_what_text(21, 5, -6, &t_a, buf)) {
			if (prefix(buf, "Page 1"))
				borg_shops[shop_num].page = 0;
			if (prefix(buf, "Page 2"))
				borg_shops[shop_num].page = 1;
		}
	...
	if ((0 == borg_what_text(1, 2, 5, &t_a, buf)) &&
		 ((streq(buf, "Eldor")) || (streq(buf, "Crysa")) ||
		  (streq(buf, "Kabir")))) {
		/* Note */
	...
	if ((0 == borg_what_text(1, 2, 5, &t_a, buf)) &&
		 ((streq(buf, "Lardb")) || (streq(buf, "Palir")) ||
		  (streq(buf, "Molvr")))) {
		/* Note */
	...
	if ((0 == borg_what_text(1, 2, 5, &t_a, buf)) &&
		 ((streq(buf, "Astin")) || (streq(buf, "Mirim")) ||
		  (streq(buf, "Vaner")))) {
		/* Note */
	...
	if ((0 == borg_what_text(1, 2, 5, &t_a, buf)) && (streq(buf, "Xeno "))) {
		/* Note */
	...
	if ((0 == borg_what_text(1, 2, 5, &t_a, buf)) && (streq(buf, "Celeb"))) {
		/* Note */
	...
	if ((0 == borg_what_text(1, 2, 5, &t_a, buf)) && (streq(buf, "Pello"))) {
		/* Note */
	...
	if ((0 == borg_what_text(1, 2, 5, &t_a, buf)) &&
		 (streq(buf, "Sonda") || streq(buf, "Otick") || streq(buf, "Uldri") ||
		  streq(buf, "Draxl") || streq(buf, "Bulin") || streq(buf, "Garad"))) {
		/* Note our mistake */
	...
	if ((0 == borg_what_text(0, 2, 8, &t_a, buf)) && (streq(buf, "Interact"))) {
		/* Note our mistake */
...
static cptr suffix_pain[] = {
	 " is unharmed."
	 " barely notices.",
	 " flinches.", " squelches.", " quivers in pain.", " writhes about.",
	 " writhes in agony.", " jerks limply.",

	 " roars thunderously.", " rumbles.", " grunts.", " hesitates.",
	 " crumples.",

	 " hisses.", " rears up in anger.", " hisses in pain.", " hisses with pain.",
	 " growls angrily.", " hisses furiously.", " roars.", " mewls in pain.",
	 " mewls pitifully.",

	 " chitters.", " scuttles about.", " twitters.", " twitches.", " chirps.",
	 " squawks.", " chatters.", " jeers.", " flutters about.", " squeaks.",

	 " snarls with pain.", " roars with pain.", " gasps.", " snarls feebly.",
	 " rattles.", " stumbles.", " staggers.", " clatters.", " groans.",
	 " moans.", " hesitates.", " grunts.", " wails.", " howls.",
	 " moans softly.", " sighs.", " shrieks in pain.", " shrieks in agony.",

	 " spawns!", " looks healthier.", " starts moving faster.",
	 " starts moving slower.",

	 " is unaffected!", " resists!", " is immune.", " is immune!",
	 " resists a lot.", " resists.", " resists somewhat.",

	 " shrugs off the attack.", " snarls with pain.", " yelps in pain.",
	 " howls in pain.", " howls in agony.",
	 /* xxx */
	 " yelps feebly.",

	 " ignores the attack.", " grunts with pain.", " squeals in pain.",
	 " shrieks in pain.", " shrieks in agony.",
	 /* xxx */
	 " cries out feebly.",

	 /* xxx */
	 /* xxx */
	 " cries out in pain.", " screams in pain.", " screams in agony.",
	 /* xxx */
	 " cringes from the light!", " loses some skin!",

	 " is suspended!",

	 " is hit hard.", " is dazed.", " is more dazed.",

	 " is suddenly very hot!", " gets zapped!",

	 " is tamed!", " hates you too much!", " is in your thrall!",
 ...
 */
static cptr prefix_kill[] = {"You have killed ", "You have slain ",
									  "You have destroyed ", NULL};
...
static cptr suffix_died[] = {" dies.",
									  ...
									  " evaporates!",
									  NULL};
static cptr suffix_blink[] = {" disappears!",	  /* from teleport other */
 ...
 */
static cptr suffix_hit_by[] = {" hits you.",
										 " touches you.",
										 " punches you.",
										 " kicks you.",
										 " claws you.",
...
static cptr suffix_mon_mon[] = {
	 ...
	 " touches the", " punches the", " kicks the", " claws the", " bites the",
	 " stings the", " butts the", " crushes the", " engulfs the", " charges the",
	 " crawls on the", " drools on the", " spits on the", " gazes at the",
	 " wails at the", " releases spores at the", NULL};
 ...
 */
static cptr prefix_feeling[] = {"Looks like any other level",
...
static void borg_parse_aux(cptr msg, int len) {
	...
	/* Log (if needed) */
	if (borg_fff)
		borg_info(format("& Msg <%s>", msg));

	/* Hack -- Notice death */
	if (prefix(msg, "You die.")) {
		/* Abort (unless cheating) */
		if (!(debug_mode || borg_cheat_death)) {
			/* Abort */
			borg_oops("death");
	...
	if (prefix(msg, "You are wielding") ||
		 ...
		 prefix(msg, "Your light source is") || prefix(msg, "You are wearing")) {
		/* Done */
		borg_note("# Parsed my equipment change.");
		return;
	}

	/* Hack -- Notice Quest complete, and turn off recall so we can grab our
	 * reward */
	...
	/* Hack -- Notice "failure" */
	if (prefix(msg, "You failed ") || prefix(msg, "You've failed ")) {
	...
	if (prefix(msg, "Your memories fade") ||
		...
		for (i = INVEN_WIELD; i <= INVEN_FEET; i++) {
			...
			if ((item->tval == TV_RING) && ((item->sval == SV_RING_FREE_ACTION) ||
													  (item->sval == SV_RING_SEE_INVIS) ||
													  (item->sval <= SV_RING_SUSTAIN_CHA)))
				continue;
		...
		for (i = 0; i <= INVEN_PACK; i++) {
			...
			/* skip certain easy know items */
			if ((item->tval == TV_RING) && (item->flags3 & TR3_EASY_KNOW))
				continue;
	...
	if (prefix(msg, "You feel opaque.") || prefix(msg, "The light forces you")) {
		borg_wraith = 0;
		return;
	}
	...
	if (prefix(msg, "You hear a door burst open!")) {
		...
		if (borg_skill[BI_CDEPTH] <= 3 && borg_skill[BI_CLEVEL] <= 5)
	...
	if (prefix(msg, "You are drowning")) {
		borg_note(format("# Help! I can't swim.  (borg carry cap:%d, real carry "
							  "cap:%d, real weight:%d)",
		...
		borg_grids[c_y][c_x].feat = FEAT_WATER;
	...
	*/
	...
	/* Trump weapons can induce random teleports.  They need to be inscribed with
	 * a . */
...
static bool borg_load_formula(char *string) {
	...
	while (string && *string) {
		...
		case '_':
			...
			if (-1000 != (value = borg_getval(&string, "_artefact"))) {
				formula[formula_num][iformula++] = value + max_k_idx + max_k_idx;
				break;
			}
			...
			if (x == BI_MAX) {
				...
				formula[formula_num][1] = 0;
				borg_note(
					 format("bad item in formula%03d %s", formula_num, string));
				return FALSE;
			}
		...
		default:
			...
			if (*string == '-') {
				...
				if (iformula + 1 > MAX_FORMULA_ELEMENTS) {
					borg_note(
						 format("too many elements in formula %03d", formula_num));
					formula[formula_num][0] = BFO_NUMBER;
					formula[formula_num][1] = 0;
					return FALSE;
				}
...
static bool add_power_item(int class_num, int depth_num, int cnd_num,
									int range_to, int range_from, bool each,
									int item_num, int power) {
	if ((class_num >= MAX_CLASS && class_num != 999) || depth_num >= MAX_DEPTH ||
...
static bool borg_load_power(char *string) {
	...
	if (-1000 != (item_num = borg_getval(&string, "_FORMULA"))) {
		...
		return add_power_item(class_num, depth_num, cnd_num, range_to, range_from,
									 each, -1, item_num);
	}
	...
	for (x = 0; x < BI_MAX; x++) {
		...
		if (prefix(string, prefix_pref[x])) {
			...
			if (strstr(string, "EACH"))
				each = TRUE;
			return add_power_item(class_num, depth_num, cnd_num, range_to,
										 range_from, each, item_num, power);
		}
	}
...
static bool borg_load_requirement(char *string) {
	...
	if (-1000 != (item_num = borg_getval(&string, "_WITEM"))) {
		string++;
		sscanf(string, "%d", &number_items);
		return add_required_item(class_num, depth_num, max_k_idx + item_num,
										 number_items);
	}
	if (-1000 != (item_num = borg_getval(&string, "_artefact"))) {
		string++;
		sscanf(string, "%d", &number_items);
		return add_required_item(class_num, depth_num,
										 max_k_idx + max_k_idx + item_num, number_items);
	}
...
void init_borg_txt_file(void) {
	...
	/* 2*k_max to a_max  = artefacts worn */
	...
	for (i = 0; i < MAX_CLASS; i++) {
		...
		/* TODO SHOULD externalize the 400 later */
		C_MAKE(borg_required_item[i], 400, req_item);
		n_req[i] = 0;
		C_MAKE(borg_power_item[i], 400, power_item);
		n_pwr[i] = 0;
	}
	...
	while (0 == my_fgets(fp, buf, sizeof(buf) - 1)) {
		...
		if (prefix(buf, "borg_worships_speed =")) {
			if (buf[strlen("borg_worships_speed =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_worships_speed =") + 1] == 't')
		...
		if (prefix(buf, "borg_worships_hp =")) {
			if (buf[strlen("borg_worships_hp =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_worships_hp =") + 1] == 't')
				borg_worships_hp = TRUE;
			else
				borg_worships_hp = FALSE;
			continue;
		}
		...
		if (prefix(buf, "borg_worships_mana =")) {
			if (buf[strlen("borg_worships_mana =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_worships_mana =") + 1] == 't')
				borg_worships_mana = TRUE;
			else
				borg_worships_mana = FALSE;
			continue;
		}
		...
		if (prefix(buf, "borg_worships_ac =")) {
			if (buf[strlen("borg_worships_ac =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_worships_ac =") + 1] == 't')
				borg_worships_ac = TRUE;
			else
				borg_worships_ac = FALSE;
			continue;
		}
		...
		if (prefix(buf, "borg_worships_gold =")) {
			if (buf[strlen("borg_worships_gold =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_worships_gold =") + 1] == 't')
				borg_worships_gold = TRUE;
			else
				borg_worships_gold = FALSE;
			continue;
		}
		...
		if (prefix(buf, "borg_plays_risky =")) {
			if (buf[strlen("borg_plays_risky =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_plays_risky =") + 1] == 't')
		...
		if (prefix(buf, "borg_scums_uniques =")) {
			if (buf[strlen("borg_scums_uniques =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_scums_uniques =") + 1] == 't')
				borg_scums_uniques = TRUE;
			else
		...
		if (prefix(buf, "borg_uses_swaps =")) {
			if (buf[strlen("borg_uses_swaps =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_uses_swaps =") + 1] == 't')
				borg_uses_swaps = TRUE;
			else
		...
		if (prefix(buf, "borg_slow_optimizehome =")) {
			if (buf[strlen("borg_slow_optimizehome =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_slow_optimizehome =") + 1] == 't')
				borg_slow_optimizehome = TRUE;
			else
				borg_slow_optimizehome = FALSE;
			continue;
		}
		...
		if (prefix(buf, "borg_uses_dynamic_calcs =")) {
			if (buf[strlen("borg_uses_dynamic_calcs =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_uses_dynamic_calcs =") + 1] == 't')
				borg_uses_calcs = TRUE;
			else
				borg_uses_calcs = FALSE;
			continue;
		}
		...
		if (prefix(buf, "borg_respawn_winners =")) {
			if (buf[strlen("borg_respawn_winners =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_respawn_winners =") + 1] == 't')
				borg_respawn_winners = TRUE;
			else
				borg_respawn_winners = FALSE;
			continue;
		}
		...
		if (prefix(buf, "borg_respawn_realm_1 =")) {
			sscanf(buf + strlen("borg_respawn_realm_1 =") + 1, "%d",
					 &borg_respawn_realm_1);
			continue;
		}
		if (prefix(buf, "borg_respawn_realm_2 =")) {
			sscanf(buf + strlen("borg_respawn_realm_2 =") + 1, "%d",
					 &borg_respawn_realm_2);
			continue;
		}
		...
		if (prefix(buf, "borg_money_scum_amount =")) {
			sscanf(buf + strlen("borg_money_scum_amount =") + 1, "%ld",
		...
		if (prefix(buf, "borg_scums_money =")) {
			if (buf[strlen("borg_scums_money =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_scums_money =") + 1] == 't')
				borg_scums_money = TRUE;
			else
				borg_scums_money = FALSE;
			continue;
		}
		...
		if (prefix(buf, "borg_delay_factor =")) {
			sscanf(buf + strlen("borg_delay_factor =") + 1, "%d",
					 &borg_delay_factor);
			if (borg_delay_factor <= 1)
				borg_delay_factor = 1;
			if (borg_delay_factor >= 9)
				borg_delay_factor = 9;
			continue;
		}
		...
		if (prefix(buf, "borg_munchkin_start =")) {
			if (buf[strlen("borg_munchkin_start =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_munchkin_start =") + 1] == 't')
		...
		if (prefix(buf, "borg_munchkin_level =")) {
			sscanf(buf + strlen("borg_munchkin_level =") + 1, "%d",
					 &borg_munchkin_level);
			if (borg_munchkin_level <= 1)
				borg_munchkin_level = 1;
			if (borg_munchkin_level >= 50)
				borg_munchkin_level = 50;
			continue;
		}

		if (prefix(buf, "borg_munchkin_depth =")) {
			sscanf(buf + strlen("borg_munchkin_depth =") + 1, "%d",
					 &borg_munchkin_depth);
			if (borg_munchkin_depth <= 1)
				borg_munchkin_depth = 8;
			if (borg_munchkin_depth >= 100)
				borg_munchkin_depth = 100;
			continue;
		}

		if (prefix(buf, "borg_enchant_limit =")) {
			sscanf(buf + strlen("borg_enchant_limit =") + 1, "%d",
					 &borg_enchant_limit);
			if (borg_enchant_limit <= 8)
				borg_enchant_limit = 8;
			if (borg_enchant_limit >= 15)
				borg_enchant_limit = 15;
			continue;
		}
		...
		if (prefix(buf, "borg_lunal_mode =")) {
			if (buf[strlen("borg_lunal_mode =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_lunal_mode =") + 1] == 't')
		...
		if (prefix(buf, "borg_self_lunal =")) {
			if (buf[strlen("borg_self_lunal =") + 1] == 'T' ||
				 ...
				 buf[strlen("borg_self_lunal =") + 1] == 't')
				borg_self_lunal = TRUE;
			else
...
static int adjust_stat_borg(int value, int amount, int borg_roll) {
	/* Negative amounts or maximise mode */
	if ((amount < 0) || maximise_mode) {
...
static void get_extra_borg(void) {
	...
	percent =
		 (int)(((long)player_hp[PY_MAX_LEVEL - 1] * 200L) /
				 (2 * p_ptr->hitdie + ((PY_MAX_LEVEL - 1) * (p_ptr->hitdie + 1))));
...
static hist_type bg[] = {
	 ...
	 {"In life you were a simple peasant, the victim of a powerful Vampire "
	  "Lord.  ",
	  40, 113, 114, 50},
	 ...
	 {"In your childhood, you were stupid enough to stick your head in raw "
	  "Logrus.  ",
	  50, 129, 130, 50},
...
static void player_outfit_borg(void) {
	...
	for (i = 0; i < 3; i++) {
		...
		if (tv == TV_SWORD && p_ptr->pclass == CLASS_ROGUE &&
			 p_ptr->realm1 ==
				  REALM_DEATH) /* Only assassins get a poisoned weapon */
		{
...
void resurrect_borg(void) {
	int i, ii, j;
	int n;
	/*bool borg_cheat_live; Not used in the end*/
...
*/
	...
	borg_note("# Reset artefacts and Quests.");
	...
	for (i = 1; i < max_k_idx; i++) {
		...
		/* skip some stuff */
		if (k_ptr->tval <= TV_AMULET || k_ptr->tval >= TV_FLASK)
			continue;
	...
	if (borg_respawn_race == -1) {
		while (1) {
			/* Race */
			p_ptr->prace = rand_int(COUNT_SUBRACES);

			rp_ptr = &race_info[p_ptr->prace];

			/* Try again if not a legal choice */
			if (!(rp_ptr->choice & (1L << p_ptr->pclass)))
				continue;
			break;
		}
	} else {
	...
	p_ptr->evil_patron = (s16b)rand_int(MAX_PATRON);

	/* Hack -- enter the world */
	if ((p_ptr->prace == VAMPIRE) || (p_ptr->prace == SKELETON) ||
		 /*p_ptr->prace == ZOMBIE) ||*/ (p_ptr->prace == SPECTRE)) {
	...
	case CLASS_PRIEST:
		...
		{
			p_ptr->realm1 = ((rand_int(50) > 25) ? REALM_LIFE : REALM_DEATH);
		}
		...
		while (1) {
			...
			if (borg_respawn_realm_2 == 0 ||
				 borg_respawn_realm_2 == p_ptr->realm1 ||
				 (p_ptr->realm1 == REALM_LIFE &&
				  borg_respawn_realm_2 == REALM_DEATH) ||
				 (p_ptr->realm1 == REALM_DEATH &&
				  borg_respawn_realm_2 == REALM_LIFE)) {
				p_ptr->realm2 = rand_int(MAX_REALM) + 1;
			} else {
				p_ptr->realm2 = borg_respawn_realm_2;
			}
			...
			if (p_ptr->realm1 == REALM_LIFE && p_ptr->realm2 == REALM_DEATH)
				continue;
			if (p_ptr->realm2 == REALM_LIFE && p_ptr->realm1 == REALM_DEATH)
				continue;
			if (p_ptr->realm2 == p_ptr->realm1)
				continue;
	...
	case CLASS_MONK:
		...
		while (1) {
			...
			/* Certain Class Restrictions: */
			if (p_ptr->realm1 == REALM_ARCANE)
				continue;
			if (p_ptr->realm1 == REALM_SORCERY)
				continue;
	...
	for (i = 0; i < MAX_CLASS; i++) {
		...
		C_WIPE(borg_required_item[i], 400,
				 req_item); /* externalize the 400 later */
		C_WIPE(borg_power_item[i], 400,
				 power_item); /* externalize the 400 later */
	}
	C_WIPE(borg_has, size_obj, int);
	for (i = 0; i < 1000; i++) {
		if (formula[i])
			C_WIPE(formula[i], MAX_FORMULA_ELEMENTS, int);
	}
...
static char borg_inkey_hack(int flush_first) {
	 ...
	 */
	if (borg_prompt && !inkey_flag && streq(buf, "Atte")) {
	...
	if ((0 == borg_what_text(0, 0, 4, &t_a, buf)) && (t_a != TERM_DARK) &&
		 (*((u32b *)(buf)) != 0x20202020)) {
		/* Assume a prompt/message is available */
		borg_prompt = TRUE;
	}
	...
	if (borg_prompt && !inkey_flag && (y == 0) && (x >= 4) &&
		 (streq(buf, "Die?") || streq(buf, "Dump")) && borg_cheat_death) {
		/* Flush messages */
		borg_parse(NULL);
...
static void borg_display_item(object_type *item2) {
	...
	prt(format("kind = %-5d  level = %-4d  tval = %-5d  sval = %-5d", item->kind,
				  item->level, item->tval, item->sval),
		 4, j);
	...
	prt(format("name1 = %-4d  name2 = %-4d  name3 = %-5d value = %-2d   cursed "
				  "= %-2ld",
				  item->name1, item->name2, item->name3, (long)item->value,
				  item->cursed),
		 7, j);

	prt(format(
			  "aware = %-1d     id'd = %-1d      fully_id = %-1d  timeout = %-1d",
			  item->aware, item->ident, item->fully_identified, item->timeout),
		 8, j);
 ...
 */
#define object_kind_attr(T) ((T)->x_attr)
 ...
 * Default to user definitions.
 */
#define object_kind_char(T) ((T)->x_char)
...
void init_translate_visuals(void) {
	...
	for (i = 0; i < max_f_idx; i++) {
		...
		translate_visuals[(byte)f_ptr->f_attr][(byte)f_ptr->f_char].d_attr =
			 f_ptr->f_attr;
		translate_visuals[(byte)f_ptr->f_attr][(byte)f_ptr->f_char].d_char =
			 f_ptr->f_char;

		/* Boring grids (floors, etc) */
		if (view_special_lite && (f_ptr->z_attr == TERM_WHITE) &&
			 (i <= FEAT_INVIS)) {
			translate_visuals[TERM_YELLOW][(byte)f_ptr->z_char].d_attr =
				 f_ptr->f_attr;
			translate_visuals[TERM_YELLOW][(byte)f_ptr->z_char].d_char =
				 f_ptr->f_char;
			translate_visuals[TERM_L_DARK][(byte)f_ptr->z_char].d_attr =
				 f_ptr->f_attr;
			translate_visuals[TERM_L_DARK][(byte)f_ptr->z_char].d_char =
				 f_ptr->f_char;
			translate_visuals[TERM_SLATE][(byte)f_ptr->z_char].d_attr =
				 f_ptr->f_attr;
			translate_visuals[TERM_SLATE][(byte)f_ptr->z_char].d_char =
				 f_ptr->f_char;
		} else if (view_granite_lite && (f_ptr->f_attr == TERM_WHITE) &&
					  (i >= FEAT_SECRET)) {
			translate_visuals[TERM_L_DARK][(byte)f_ptr->z_char].d_attr =
				 f_ptr->f_attr;
			translate_visuals[TERM_L_DARK][(byte)f_ptr->z_char].d_char =
	...
	for (i = 0; i < max_k_idx; i++) {
		...
		/* Store the underlying values */
		translate_visuals[(byte)object_kind_attr(k_ptr)]
							  [(byte)object_kind_char(k_ptr)]
									.d_attr = k_ptr->d_attr;
		translate_visuals[(byte)object_kind_attr(k_ptr)]
							  [(byte)object_kind_char(k_ptr)]
									.d_char = k_ptr->d_char;
	...
	for (i = 0; i < max_r_idx; i++) {
		...
		/* Store the underlying values */
		translate_visuals[(byte)r_ptr->x_attr][(byte)r_ptr->x_char].d_attr =
			 r_ptr->d_attr;
		translate_visuals[(byte)r_ptr->x_attr][(byte)r_ptr->x_char].d_char =
			 r_ptr->d_char;
...
void borg_write_map(bool ask) {
	...
	store_type *st_ptr = &store[BORG_HOME];
...
/*bool *okay;*/
	...
	if (ask && get_string("Borg map File: ", buf2, 70)) {
		...
		/* Failure */
		if (!borg_map_file)
			msg_print("Cannot open that file.");
	} else if (!ask)
		borg_map_file = my_fopen(buf2, "w");
	...
	for (i = 1; i < borg_kills_nxt; i++) {
		...
		fprintf(borg_map_file, "monster '%s' (%d) at (%d,%d) speed %d \n",
				  (r_name + r_info[kill->r_idx].name), kill->r_idx, kill->y,
				  kill->x, kill->speed);
	}
	...
	fprintf(borg_map_file, "Time on this level; %ld\n", borg_t - borg_began);
	fprintf(borg_map_file, "Time since left town; %ld\n",
	...
	if ((turn % (10L * TOWN_DAWN)) < ((10L * TOWN_DAWN) / 2))
		...
		fprintf(borg_map_file, "It is night-time in town.\n");
	fprintf(borg_map_file, "\n\n");
	fprintf(borg_map_file, "\n\n");
	...
	if (borg_skill[BI_REALM1]) {
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 8; j++) {
				...
				if (as->level < 99) {
					legal = (borg_spell_legal(borg_skill[BI_REALM1], i, j) ? "Yes"
																							 : "No ");
					fprintf(borg_map_file, "%-30s   %s   %ld\n", as->name, legal,
							  (long)as->times);
				}
			}
			fprintf(borg_map_file, "\n");
	...
	if (borg_skill[BI_REALM2]) {
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 8; j++) {
				...
				if (as->level < 99) {
					legal = (borg_spell_legal(borg_skill[BI_REALM2], i, j) ? "Yes"
																							 : "No ");
					fprintf(borg_map_file, "%-30s   %s   %ld\n", as->name, legal,
							  (long)as->times);
				}
	...
	for (; itemm < to; itemm++) {
		fprintf(borg_map_file, "skill %d (%s) value= %d.\n", itemm,
				  prefix_pref[itemm - max_k_idx - max_k_idx - max_a_idx],
				  borg_has[itemm]);
	}

#if 0
    /*** Dump the Uniques and artefact Lists ***/
    ...
    fprintf(borg_map_file, "   [artefact Info] \n\n");
    ...
    for (k = 0; k < max_a_idx; k++)
    {
        ...
        /* Paranoia */
        strcpy(o_name, "Unknown artefact");
		...
		if (window_flag[j] & (PW_BORG_2)) {
			...
			if (unique_on_level)
				Term_putstr(58, 2, -1, attr,
								format("(%s)", r_name + r_info[unique_on_level].name));
			else
				Term_putstr(58, 2, -1, attr, "                                   ");
			...
			/*if (borg_depth & DEPTH_QUEST)
				Term_putstr(55, 3, -1, attr, format("%s (%d)", name, q_num));
			else*/
			...
			Term_putstr(1, 20, -1, attr,
							format("Borer on Level.  Last seen:%d       ",
									 borg_t - borg_t_questor));
			if (borg_depth & (DEPTH_BORER | DEPTH_SUMMONER))
				attr = TERM_BLUE;
			else
				attr = TERM_SLATE;
			if (borg_skill[BI_CDEPTH] == 100)
				Term_putstr(1, 20, -1, attr,
								format("Bor/Sum on Level.  Last seen:%d       ",
										 borg_t - borg_t_questor));
...
void do_cmd_borg(void) {
	...
	case 'F': {
		...
		case 'D': {
			...
			msg_format("Borg -- borg_flag_dump is now %d.", borg_flag_dump);
			break;
		}
		...
		case 'S': {
			...
			msg_format("Borg -- borg_flag_save is now %d.", borg_flag_save);
			break;
		}
	...
	case 'c': {
		...
		case 'D': {
			...
			msg_format("Borg -- borg_cheat_death is now %d.", borg_cheat_death);
			break;
		}
		}
		break;
	...
	case 'G': {
		...
		/*		case 'm':
				...
				case 'a':
					...
					break;
		*/
		/*TODO SHOULD Totally not sure about this??*/
		case 'A':
			low = high = FEAT_WATER;
			break;
		case 'k':
	...
	case 'A': {
		...
		/* Get keypress */
		msg_format("(%d,%d of %d,%d) Avoidance value %d.", c_y, c_x,
					  panel_row_min / PANEL_HGT, panel_col_min / PANEL_WID,
					  avoidance);
		msg_print(NULL);
	...
	case '#': {
		...
		/* Danger of grid */
		msg_format("Danger(%d,%d,%d) is %d", target_col, target_row, n,
		...
		for (y = w_y; y < w_y + SCREEN_HGT; y++) {
			for (x = w_x; x < w_x + SCREEN_WID; x++) {
				...
				if (ag->info & BORG_VIEW)
					print_rel('*', TERM_ORANGE, y, x);

				/* Projectable/ LOS routine */
				else if (borg_los(c_y, c_x, y, x))
					print_rel('*', TERM_RED, y, x);
			}
		}
	...
	case 'm': {
		...
		for (y = 0; y < MAX_HGT; y++) {
			for (x = 0; x < MAX_WID; x++) {
				/* Obtain */
				if (borg_detect_wall[y / 11][x / 33] == FALSE)
					continue;
	...
	case '1': {
		...
		new_borg_depth =
			 get_quantity("Enter new Max Depth: ", MAX_DEPTH - 1, FALSE);

		/* Allow user abort */
		if (new_borg_depth >= 0) {
			p_ptr->max_dun_level = new_borg_depth;
	...
	case 'q': {
		...
		new_borg_stop_dlevel =
			 get_quantity("Enter new auto-stop dlevel: ", MAX_DEPTH - 1, FALSE);
		new_borg_stop_clevel =
			 get_quantity("Enter new auto-stop clevel: ", 51, FALSE);
	...
	case '3': {
		...
		/* Feature of grid */
		msg_format("Feature of (%d,%d) %d", target_col, target_row, ag->feat);
		break;
	}
	...
	case 'd': {
		...
		for (k = 1; k < MAX_REALM + 1; k++) {
			...
			/* skip wrong realms */
			if (k != borg_skill[BI_REALM1] && k != borg_skill[BI_REALM2])
				continue;
			...
			for (i = 0; i < 4; i++) {
				...
				Term_putstr(
					 1, ii++, -1, TERM_WHITE,
					 format("[ Realm %s ]:", borg_magics[k][0][0].realm_name));
				for (j = 0; j < 8; j++) {
					...
					if (as->level < 99) {
						if (k == borg_skill[BI_REALM1])
							legal = (borg_spell_legal(borg_skill[BI_REALM1], i, j)
											 ? "legal"
											 : "Not Legal ");
						if (k == borg_skill[BI_REALM2])
							legal = (borg_spell_legal(borg_skill[BI_REALM2], i, j)
											 ? "legal"
											 : "Not Legal ");
					}
					Term_putstr(1, ii++, -1, TERM_WHITE,
									format("%-25s %-10s %-2d attempts.", as->name, legal,
											 as->times));
				}
				Term_putstr(
					 1, ii++, -1, TERM_WHITE,
					 format("Book located at inventory slot %d", borg_book[k][i]));
		...
		if (borg_class == CLASS_ORPHIC) {
			...
			for (i = 0; i < MAX_MINDCRAFT_POWERS; i++) {
				...
				if (as->level < 99) {
					legal =
						 (borg_mindcr_legal(i, borg_skill[BI_CLEVEL]) ? "legal"
																					 : "Not Legal ");
				}

				Term_putstr(1, ii++, -1, TERM_WHITE,
								format("%s, %s, attempted %d times", as->name, legal,
										 as->times));
			}
	...
	case 'h': {
		...
		/* Get a "Borg command", or abort */
		if (!get_com("Dynamic Borg Has What: ((i)nv/(w)orn/(a)rtifact/(s)kill) ",
						 &cmd))
			return;
		...
		for (; item < to; item++) {
			...
			case 'w':
			case 'W':
				borg_note(format("WItem%03d value= %d.", item - max_k_idx,
									  borg_has[item]));
				break;
			case 'a':
			case 'A':
				borg_note(format("artefact%03d value= %d.",
									  item - max_k_idx - max_k_idx, borg_has[item]));
				break;
			default:
				borg_note(
					 format("skill %d (%s) value= %d.", item,
							  prefix_pref[item - max_k_idx - max_k_idx - max_a_idx],
							  borg_has[item]));
				break;
			}
		}
	...
	case 'H': {
		...
		/* Get keypress */
		msg_format("Grids that are Happy and pass borg_check_rest (%d).",
					  borg_temp_n - 1);
		msg_print(NULL);
...
struct {
	...
	long data;
	 ...
	 {"borg_flag_save", "Save the game when the dungeon level changes", NULL,
	  (long)&borg_flag_save},
	 {"borg_stop_king", "Stop the borg on win", NULL, (long)&borg_stop_king},
	 {"borg_worships_damage", "Borg worships damage", NULL,
	  (long)&borg_worships_damage},
	 {"borg_worships_speed", "Borg worships speed", NULL,
	  (long)&borg_worships_speed},
	 {"borg_worships_hp", "Borg worshops hitpoints", NULL,
	  (long)&borg_worships_hp},
	 {"borg_worships_mana", "Borg worships mana", NULL,
	  (long)&borg_worships_mana},
	 {"borg_worships_gold", "Borg worships gold", NULL,
	  (long)&borg_worships_gold},
	 {"borg_worships_ac", "Borg worships armor class", NULL,
	  (long)&borg_worships_ac},
	 {"borg_plays_risky", "Borg plays risky", NULL, (long)&borg_plays_risky},
	 {"borg_slow_optimizehome", "Slow home optimize", NULL,
	  (long)&borg_slow_optimizehome},
	 {"borg_uses_swaps", "Borg uses Swap Items", NULL, (long)&borg_uses_swaps},
	 {"borg_uses_calcs", "Borg uses Dynamic Calculations (slow)", NULL,
	  (long)&borg_uses_calcs},
	 {"delay_factor", "xyz", NULL, (long)&borg_delay_factor},
	 {NULL, NULL, NULL, 0L},
};

void *Borg_GetSettings(void) { return settings; }
