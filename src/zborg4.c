...
static void borg_notice_aux1(void) {
	...
	/* borg_skill[BI_THN] += add_special_melee_skill(borg_class,
	 * borg_items[INVEN_WIELD].weight, &inventory[INVEN_WIELD]); */

	/* Base skill -- combat (shooting) */
	borg_skill[BI_THB] = rb_ptr->r_thb + cb_ptr->c_thb;
	/* = rp_ptr->r_thb + cp_ptr->c_thb + bsp_ptr->r_thb; */
	/* borg_skill[BI_THB] += add_special_missile_skill(borg_class,
	 * borg_items[INVEN_WIELD].weight, &inventory[INVEN_BOW]); */
	...
	/* Bad flags */
	if (f1 & (TR1_IMPACT))
		borg_skill[BI_W_IMPACT] = TRUE;
	if (f3 & (TR3_AGGRAVATE))
		borg_skill[BI_CRSAGRV] = TRUE;
	if (f3 & (TR3_TELEPORT))
		borg_skill[BI_CRSTELE] = TRUE;
	if (f3 & (TR3_NO_TELE))
		borg_skill[BI_CRSNOTELE] = TRUE;
	if (f3 & (TR3_NO_MAGIC))
	...
	if (f2 & (TR2_RES_POIS))
		borg_skill[BI_RPOIS] = TRUE;
	if (f2 & (TR2_RES_FEAR))
		borg_skill[BI_RFEAR] = TRUE;
	if (f2 & (TR2_RES_LITE))
		borg_skill[BI_RLITE] = TRUE;
	if (f2 & (TR2_RES_DARK))
		borg_skill[BI_RDARK] = TRUE;
	if (f2 & (TR2_RES_BLIND))
		borg_skill[BI_RBLIND] = TRUE;
	if (f2 & (TR2_RES_CONF))
		borg_skill[BI_RCONF] = TRUE;
	if (f2 & (TR2_RES_SOUND))
		borg_skill[BI_RSND] = TRUE;
	if (f2 & (TR2_RES_SHARDS))
		borg_skill[BI_RSHRD] = TRUE;
	if (f2 & (TR2_RES_NEXUS))
		borg_skill[BI_RNXUS] = TRUE;
	if (f2 & (TR2_RES_NETHER))
		borg_skill[BI_RNTHR] = TRUE;
	if (f2 & (TR2_RES_CHAOS))
		borg_skill[BI_RKAOS] = TRUE;
	if (f2 & (TR2_RES_DISEN))
		borg_skill[BI_RDIS] = TRUE;
	if (f2 & (TR2_REFLECT)) {
	...
	/* Sustain flags */
	if (f2 & (TR2_SUST_STR))
		borg_skill[BI_SSTR] = TRUE;
	if (f2 & (TR2_SUST_INT))
		borg_skill[BI_SINT] = TRUE;
	if (f2 & (TR2_SUST_WIS))
		borg_skill[BI_SWIS] = TRUE;
	if (f2 & (TR2_SUST_DEX))
		borg_skill[BI_SDEX] = TRUE;
	if (f2 & (TR2_SUST_CON))
		borg_skill[BI_SCON] = TRUE;
	if (f2 & (TR2_SUST_CHA))
		borg_skill[BI_SCHR] = TRUE;
	...
	for (i = INVEN_WIELD; i < INVEN_TOTAL; i++) {
		...
		if (borg_class == CLASS_MYSTIC && i == INVEN_WIELD)
			continue;
		...
		if (item->flags1 & TR1_STR)
			my_stat_add[A_STR] += item->pval;
		if (item->flags1 & TR1_INT)
			my_stat_add[A_INT] += item->pval;
		if (item->flags1 & TR1_WIS)
			my_stat_add[A_WIS] += item->pval;
		if (item->flags1 & TR1_DEX)
			my_stat_add[A_DEX] += item->pval;
		if (item->flags1 & TR1_CON)
			my_stat_add[A_CON] += item->pval;
		if (item->flags1 & TR1_CHA)
			my_stat_add[A_CHA] += item->pval;

		/* various slays */
		if (item->flags1 & TR1_SLAY_ANIMAL)
			borg_skill[BI_WS_ANIMAL] = TRUE;
		if (item->flags1 & TR1_SLAY_EVIL)
			borg_skill[BI_WS_EVIL] = TRUE;

		if (item->flags1 & TR1_SLAY_UNDEAD)
			borg_skill[BI_WS_UNDEAD] = TRUE;
		if (item->flags1 & TR1_SLAY_DEMON)
			borg_skill[BI_WS_DEMON] = TRUE;
		if (item->flags1 & TR1_SLAY_GIANT)
			borg_skill[BI_WS_GIANT] = TRUE;
		if (item->flags1 & TR1_SLAY_DRAGON)
			borg_skill[BI_WS_DRAGON] = TRUE;
		if (item->flags1 & TR1_KILL_DRAGON)
			borg_skill[BI_WK_DRAGON] = TRUE;
		if (item->flags1 & TR1_IMPACT)
			borg_skill[BI_W_IMPACT] = TRUE;
		if (item->flags1 & TR1_BRAND_ACID)
			borg_skill[BI_WB_ACID] = TRUE;
		if (item->flags1 & TR1_BRAND_ELEC)
			borg_skill[BI_WB_ELEC] = TRUE;
		if (item->flags1 & TR1_BRAND_FIRE)
			borg_skill[BI_WB_FIRE] = TRUE;
		if (item->flags1 & TR1_BRAND_COLD)
			borg_skill[BI_WB_COLD] = TRUE;
		if (item->flags1 & TR1_BRAND_POIS)
			borg_skill[BI_WB_POIS] = TRUE;
		if (item->flags1 & TR1_VORPAL)
			borg_skill[BI_WB_VORPAL] = TRUE;
		if (item->flags1 & TR1_VAMPIRIC)
			borg_skill[BI_WB_VAMPIRIC] = TRUE;
		if (item->flags1 & TR1_CHAOTIC)
			borg_skill[BI_WB_CHAOTIC] = TRUE;

		/* Affect infravision */
		if (item->flags1 & TR1_INFRA)
			borg_skill[BI_INFRA] += item->pval;

		/* Affect stealth */
		if (item->flags1 & TR1_STEALTH)
			borg_skill[BI_STL] += item->pval;

		/* Affect searching ability (factor of five) */
		if (item->flags1 & TR1_SEARCH)
			borg_skill[BI_SRCH] += (item->pval * 5);

		/* Affect searching frequency (factor of five) */
		if (item->flags1 & TR1_SEARCH)
			borg_skill[BI_SRCHFREQ] += (item->pval * 5);

		/* Affect digging (factor of 20) */
		if (item->flags1 & TR1_TUNNEL)
			borg_skill[BI_DIG] += (item->pval * 20);

		/* Affect speed */
		if (item->flags1 & TR1_SPEED)
			borg_skill[BI_SPEED] += item->pval;

		/* Affect blows */
		if (item->flags1 & TR1_BLOWS)
			extra_blows += item->pval;

		/* Boost shots */
		if (item->flags3 & TR3_XTRA_SHOTS)
			extra_shots++;

		/* Boost might */
		if (item->flags3 & TR3_XTRA_MIGHT)
			extra_might++;

		/* Various flags */
		if (item->flags3 & TR3_SLOW_DIGEST)
			borg_skill[BI_SDIG] = TRUE;
		if (item->flags3 & TR3_AGGRAVATE)
			borg_skill[BI_CRSAGRV] = TRUE;
		if (item->flags3 & TR3_TY_CURSE)
			borg_skill[BI_CRSTY] = TRUE;
		if ((item->flags3 & TR3_TELEPORT) && !strstr(item->desc, "."))
			borg_skill[BI_CRSTELE] = TRUE;
		if (item->flags3 & TR3_NO_TELE)
			borg_skill[BI_CRSNOTELE] = TRUE;
		if (item->flags3 & TR3_NO_MAGIC)
			borg_skill[BI_CRSNOMAGIC] = TRUE;

		if (item->flags3 & TR3_REGEN)
			borg_skill[BI_REG] = TRUE;
		if (item->flags3 & TR3_TELEPATHY)
			borg_skill[BI_ESP] = TRUE;
		if (item->flags3 & TR3_LITE)
			borg_skill[BI_LITE] = TRUE;
		if (item->flags3 & TR3_SEE_INVIS)
			borg_skill[BI_SINV] = TRUE;
		if (item->flags3 & TR3_FEATHER)
			borg_skill[BI_FEATH] = TRUE;
		...
		if (item->flags3 & TR3_SH_ELEC)
			borg_skill[BI_ELECSH] = TRUE;
		if (item->flags3 & TR3_WRAITH) {
			borg_skill[BI_WRAITH] = TRUE;
			/* Its too dangerous for the borg to passwall without ESP; he exits the
			 * walls with
		...
		/* Resistance flags */
		if (item->flags2 & TR2_RES_ACID)
			borg_skill[BI_RACID] = TRUE;
		if (item->flags2 & TR2_RES_ELEC)
			borg_skill[BI_RELEC] = TRUE;
		if (item->flags2 & TR2_RES_FIRE)
			borg_skill[BI_RFIRE] = TRUE;
		if (item->flags2 & TR2_RES_COLD)
			borg_skill[BI_RCOLD] = TRUE;
		...
		if (item->flags2 & TR2_RES_NETHER)
			borg_skill[BI_RNTHR] = TRUE;
		if (item->flags2 & TR2_REFLECT)
			borg_skill[BI_REFLECT] = TRUE;
		if (item->flags2 & TR2_RES_FEAR)
			borg_skill[BI_RFEAR] = TRUE;

		/* Sustain flags */
		if (item->flags2 & TR2_SUST_STR)
			borg_skill[BI_SSTR] = TRUE;
		if (item->flags2 & TR2_SUST_INT)
			borg_skill[BI_SINT] = TRUE;
		if (item->flags2 & TR2_SUST_WIS)
			borg_skill[BI_SWIS] = TRUE;
		if (item->flags2 & TR2_SUST_DEX)
			borg_skill[BI_SDEX] = TRUE;
		if (item->flags2 & TR2_SUST_CON)
			borg_skill[BI_SCON] = TRUE;
		if (item->flags2 & TR2_SUST_CHA)
			borg_skill[BI_SCHR] = TRUE;
#if 0
		...
		if (item->name2 == EGO_PLANAR)
			borg_skill[BI_AESCAPE]++;
	...
	if (!rp_ptr->rations)
		borg_skill[BI_NOEAT] = TRUE;
	if (borg_race == SPECTRE /* && borg_skill[BI_ESP] */)
		borg_skill[BI_PASSWALL] = TRUE;
	if ((borg_class == CLASS_ORPHIC && borg_skill[BI_MAXCLEVEL] >= 5) ||
		 (borg_class == CLASS_MAGE && borg_skill[BI_MAXCLEVEL] >= 5) ||
		 (borg_class == CLASS_HIGH_MAGE && borg_skill[BI_MAXCLEVEL] >= 5))
		borg_skill[BI_NO_MELEE] = TRUE;
	...
	borg_skill[BI_HP_ADJ] =
		 (adj_stat[ADJ_HP][my_stat_ind[A_CON]] - 128) * borg_skill[BI_CLEVEL] / 2;

	/* 'Mana' is actually the 'mana adjustment' */
	if (borg_skill[BI_WISMANA]) {
		borg_skill[BI_SP_ADJ] =
			 adj_stat[ADJ_MANA][my_stat_ind[A_WIS]] * borg_skill[BI_CLEVEL] / 2;
	...
	if (borg_skill[BI_INTMANA]) {
		borg_skill[BI_SP_ADJ] =
			 adj_stat[ADJ_MANA][my_stat_ind[A_INT]] * borg_skill[BI_CLEVEL] / 2;
	...
	if (item->iqty && (hold >= item->weight / 10)) {
		...
		if ((borg_class == CLASS_RANGER) &&
			 (my_ammo_tval == TV_ARROW || my_ammo_tval == TV_BOLT)) {
			/* Extra shot at level 20 */
			if (borg_skill[BI_CLEVEL] >= 20 && my_ammo_tval == TV_ARROW)
				my_num_fire++;

			/* Extra shot at level 30 */
			if (borg_skill[BI_CLEVEL] >= 30 && my_ammo_tval == TV_BOLT)
				my_num_fire++;

			/* Extra shot at level 40 if using a bow */
			if (borg_skill[BI_CLEVEL] >= 40 && my_ammo_tval == TV_ARROW)
				my_num_fire++;
		}
		...
		if (borg_class == CLASS_WARRIOR && (my_ammo_tval <= TV_BOLT) &&
			 (my_ammo_tval >= TV_SHOT)) {
			/* Extra shot at level 40 */
			if (borg_skill[BI_CLEVEL] >= 40)
				my_num_fire++;
	...
	if (item->iqty && (hold >= item->weight / 10)) {
		...
		/* Maximal value */
		if (borg_skill[BI_BLOWS] > num)
			borg_skill[BI_BLOWS] = num;

		/* Add in the "bonus blows" */
		borg_skill[BI_BLOWS] += extra_blows;

		/* Require at least one blow */
		if (borg_skill[BI_BLOWS] < 1)
			borg_skill[BI_BLOWS] = 1;
	...
	/* priest weapon penalty for non-blessed edged weapons if I cannot bless it
	 * myself */
	...
	/* Wraith form from spell grants powers.  Wraith_form from item also counts
	 * bonuses here. */
	if (borg_wraith) {
		borg_skill[BI_ARMOR] += 100;
		borg_skill[BI_PASSWALL] = TRUE;
	...
	if (borg_skill[BI_STL] > 30)
		borg_skill[BI_STL] = 30;
	if (borg_skill[BI_STL] < 0)
		borg_skill[BI_STL] = 0;

	/* Limit Skill -- digging from 1 up */
	if (borg_skill[BI_DIG] < 1)
		borg_skill[BI_DIG] = 1;
	...
	if (borg_class == CLASS_MYSTIC) {
		...
		if (!(borg_items[INVEN_WIELD].iqty)) {
			...
			if (borg_skill[BI_CLEVEL] > 9)
				borg_skill[BI_BLOWS]++;
			if (borg_skill[BI_CLEVEL] > 14)
				borg_skill[BI_BLOWS]++;
			if (borg_skill[BI_CLEVEL] > 24)
				borg_skill[BI_BLOWS]++;
			if (borg_skill[BI_CLEVEL] > 34)
				borg_skill[BI_BLOWS]++;
			if (borg_skill[BI_CLEVEL] > 44)
				borg_skill[BI_BLOWS]++;
			if (borg_skill[BI_CLEVEL] > 49)
				borg_skill[BI_BLOWS]++;
	...
	for (i = INVEN_WIELD; i <= INVEN_BOW; i++) {
		...
		if (i == INVEN_WIELD && !item->name3 &&
			 (borg_spell_legal_fail(REALM_SORCERY, 2, 0, 30) ||
			  ...
			  borg_spell_legal_fail(REALM_TAROT, 2, 4, 30)))
			my_need_brand_weapon += 10;
		...
		if ((borg_spell_legal_fail(REALM_SORCERY, 3, 4, 40) ||
			...
			if (item->to_h < borg_enchant_limit) {
				...
				if (item->to_h < 0 && my_need_enchant_to_h > 10)
					my_need_enchant_to_h = 1;
			}

			/* Enchant all weapons (to damage) */
			if (item->to_d < borg_enchant_limit) {
				my_need_enchant_to_d += (borg_enchant_limit - item->to_d);
				if (item->to_d < 0 && my_need_enchant_to_d > 10)
					my_need_enchant_to_d = 1;
			}
		} else {
			if (item->to_h < 8) {
	...
	if (item->tval == TV_LITE) {
		/* Torches -- radius one */
		if (item->sval == SV_LITE_TORCH && item->pval != 0)
			borg_skill[BI_CUR_LITE] = 1;

		/* Lanterns -- radius two */
		if (item->sval == SV_LITE_LANTERN && item->pval != 0)
			borg_skill[BI_CUR_LITE] = 2;

		/* No fuel means no radius */
		if (!item->pval && !borg_skill[BI_LITE])
			borg_skill[BI_CUR_LITE] = 0;
	...
	for (i = INVEN_WIELD; i < INVEN_TOTAL; i++) {
		...
		if (item->xtra2 == ACT_SUNLIGHT || item->xtra2 <= ACT_DISP_GOOD ||
			 item->xtra2 == ACT_VAMPIRE_1 || item->xtra2 == ACT_VAMPIRE_2 ||
...
static void borg_notice_aux2(void) {
	...
	for (i = 0; i < INVEN_PACK; i++) {
		...
		if (item->flags3 & TR3_WRAITH) {
			borg_oops("# Borg unhooked.  Andrew is looking for an item with "
						 "Wraith.  Send him this savefile.");
		}
		...
		case TV_ROD:
			...
			case SV_ROD_HEALING:
				...
				break;
			/* Special cascade order of the rods to keep them in a certain order
			 * and iqty */
			case SV_ROD_ACID_BOLT: {
				if (item->iqty <= 5)
					borg_skill[BI_AROD1] += item->iqty;
				else
					borg_skill[BI_AROD1] += 5;
				borg_skill[BI_NSRANGED]++;
			} break;
			case SV_ROD_ELEC_BOLT: {
				if (item->iqty + borg_skill[BI_AROD1] <= 5)
					borg_skill[BI_AROD1] += item->iqty;
				else
					borg_skill[BI_AROD1] += 5;
				borg_skill[BI_NSRANGED]++;
			...
			case SV_ROD_LITE: {
				if (item->iqty <= 5)
					...
					borg_skill[BI_AROD2] += 5;
			} break;
			case SV_ROD_DRAIN_LIFE: {
				if (item->iqty + borg_skill[BI_AROD2] <= 10)
					borg_skill[BI_AROD2] += item->iqty;
				else
					borg_skill[BI_AROD2] += 10;
				borg_skill[BI_NSRANGED]++;
			} break;
		...
		case TV_FLASK:
			...
			if (borg_items[INVEN_LITE].tval == TV_LITE &&
				...
				borg_skill[BI_AFUEL] += (item->iqty * 2);
		...
		case TV_LITE:
			...
			if ((item->sval == SV_LITE_TORCH && borg_items[INVEN_LITE].iqty &&
		...
		case TV_DIGGING:
			...
			if (borg_gold >= 350000 && borg_class != CLASS_MYSTIC)
				break;
		...
		case TV_BOLT:
			...
			/* enchant missiles */
			if (borg_spell_legal_fail(REALM_MIRACLES, 7, 3, 40) &&
				 item->iqty >= 5) {
				if (item->to_h < 10) {
	...
	if (borg_spell_legal_fail(REALM_MIRACLES, 0, 7, 10) ||
		 ...
		 (borg_skill[BI_VAMPIRE] && borg_skill[BI_MAXCLEVEL] >= 20 &&
	...
	if (borg_spell_legal(REALM_MIRACLES, 0, 5) ||
		 ...
		 borg_mindcr_legal(MIND_PRECOGNIT, 2) || borg_racial_check(DWARF, 10)) {
	...
	if (borg_spell_legal(REALM_SORCERY, 1, 0) ||
		 borg_spell_legal(REALM_NATURE, 1, 2) || borg_equips_artifact(ART_EOS) ||
	...
	if ((borg_mindcr_legal(MIND_PRECOGNIT, 24) ||
		  ...
		  borg_equips_activation(ACT_ESP, FALSE)) &&
		 !borg_munchkin_mode) {
	...
	if (borg_spell_legal(REALM_CHARMS, 0, 5) ||
		 ...
		 /* borg_equips_activation(ACT_MAP_LIGHT, FALSE) || Timeout too long for
			 illumination */
	...
	if (borg_equips_artifact(ART_AMULET_MICHAEL) ||
	...
	if (borg_spell_legal(REALM_MIRACLES, 1, 7) ||
		 ...
		 */
		if (borg_magics[REALM_MIRACLES][1][7].power < 30 &&
			 borg_magics[REALM_MIRACLES][1][7].power > 1)
			borg_skill[BI_CHEAPGLYPH] = 1;
		if (borg_magics[REALM_SORCERY][3][2].power < 30 &&
			 borg_magics[REALM_SORCERY][3][2].power > 1)
			borg_skill[BI_CHEAPGLYPH] = 1;
	}

	/* Genocide */
	if (borg_equips_activation(ACT_GENOCIDE, FALSE) ||
		 borg_spell_legal(REALM_DEATH, 1, 6))
		borg_skill[BI_AGENOCIDE] += 1000;
	...
	if (borg_spell_legal_fail(REALM_CHARMS, 3, 6, 40) ||
		 ...
		 (borg_skill[BI_CDEPTH] == 100 &&
		  (borg_spell_legal(REALM_SORCERY, 2, 7) ||
			borg_spell_legal(REALM_TAROT, 1, 6))))
	...
	if ((borg_spell_legal_fail(REALM_MIRACLES, 3, 7, 4) ||
		 ...
		 borg_skill[BI_RBLIND] && borg_skill[BI_RCONF] && borg_skill[BI_ESP] &&
	...
	if (borg_equips_artifact(ART_LIFE) ||
		 ...
		 borg_spell_legal(REALM_DEATH, 1, 7) || borg_racial_check(SKELETON, 10) ||
	...
	/* Handle "recharge" */
	if (borg_equips_artifact(ART_BARD) || borg_spell_legal(REALM_CHARMS, 3, 0) ||
	...
	if (borg_spell_legal(REALM_CHARMS, 2, 4) ||
		 ...
		 borg_mutation(COR1_EAT_ROCK, TRUE, 40,
							TRUE) || /* He should be standing right next to it here */
	...
	if (my_stat_cur[A_STR] >=
		 (18 + 100) +
			  10 * maximise_mode * (rp_ptr->r_adj[A_STR] + cp_ptr->c_adj[A_STR]))
		amt_add_stat[A_STR] += 1000;

	if (my_stat_cur[A_INT] >=
		 (18 + 100) +
			  10 * maximise_mode * (rp_ptr->r_adj[A_INT] + cp_ptr->c_adj[A_INT]))
		amt_add_stat[A_INT] += 1000;

	if (my_stat_cur[A_WIS] >=
		 (18 + 100) +
			  10 * maximise_mode * (rp_ptr->r_adj[A_WIS] + cp_ptr->c_adj[A_WIS]))
		amt_add_stat[A_WIS] += 1000;

	if (my_stat_cur[A_DEX] >=
		 (18 + 100) +
			  10 * maximise_mode * (rp_ptr->r_adj[A_DEX] + cp_ptr->c_adj[A_DEX]))
		amt_add_stat[A_DEX] += 1000;

	if (my_stat_cur[A_CON] >=
		 (18 + 100) +
			  10 * maximise_mode * (rp_ptr->r_adj[A_CON] + cp_ptr->c_adj[A_CON]))
		amt_add_stat[A_CON] += 1000;

	if (my_stat_cur[A_CHA] >=
		 (18 + 100) +
			  10 * maximise_mode * (rp_ptr->r_adj[A_CHA] + cp_ptr->c_adj[A_CHA]))
	...
	if (!borg_skill[BI_ISFIXCHR])
		amt_fix_stat[A_CHA] += 1000;
	...
	if (borg_skill[BI_ENCUMBERD] > carry_capacity / 2)
		borg_skill[BI_SPEED] -=
			 ((borg_skill[BI_ENCUMBERD] - (carry_capacity / 2)) /
			  (carry_capacity / 10));
...
void borg_notice_swap_weapon(void) {
	...
	for (i = 0; i < INVEN_PACK; i++) {
		...
		if (borg_skill[BI_MAXCLEVEL] < 50 && !(item->flags1 & TR1_TUNNEL))
			continue;

		/* Require ID, "known" (or average, good, etc) */
		if (!item->ident && !strstr(item->desc, "{good") &&
			 !strstr(item->desc, "{excellent") &&
			 !strstr(item->desc, "{terrible") && !strstr(item->desc, "Quest") &&
			 !strstr(item->desc, "special"))
			continue;
		...
		case TV_DIGGING: {
			...
			if (borg_class == CLASS_MYSTIC && !weapon_swap_digger)
				continue;

			/* various slays */
			if (item->flags1 & TR1_SLAY_ANIMAL)
				weapon_swap_slay_animal = TRUE;
			if (item->flags1 & TR1_SLAY_EVIL)
				weapon_swap_slay_evil = TRUE;
			if (item->flags1 & TR1_SLAY_UNDEAD)
				weapon_swap_slay_undead = TRUE;
			if (item->flags1 & TR1_SLAY_DEMON)
				weapon_swap_slay_demon = TRUE;
			if (item->flags1 & TR1_SLAY_GIANT)
				weapon_swap_slay_giant = TRUE;
			if (item->flags1 & TR1_SLAY_DRAGON)
				weapon_swap_slay_dragon = TRUE;
			if (item->flags1 & TR1_KILL_DRAGON)
				weapon_swap_kill_dragon = TRUE;
			if (item->flags1 & TR1_IMPACT)
			...
			/* Affect infravision */
			if (item->flags1 & TR1_INFRA)
				weapon_swap_see_infra += item->pval;
			...
			if (item->cursed)
				decurse_weapon_swap = 0;
			if (item->flags3 & TR3_HEAVY_CURSE)
				decurse_weapon_swap = 1;
			if (item->flags3 & TR3_TY_CURSE)
				decurse_weapon_swap = -1;
			...
			if (!borg_skill[BI_WS_ANIMAL] && !borg_skill[BI_WB_ELEC] &&
				...
				v += (dam * 2) / 2;
			if (!borg_skill[BI_WS_EVIL] && weapon_swap_slay_evil)
				v += (dam * 7) / 2;

			/* assume 3x base damage for x% of creatures */
			dam = damage * 3 * borg_skill[BI_BLOWS];

			if (!borg_skill[BI_WS_UNDEAD] && weapon_swap_slay_undead)
				v += (dam * 5) / 2;
			if (!borg_skill[BI_WS_DEMON] && weapon_swap_slay_demon)
				v += (dam * 3) / 2;
			if (!borg_skill[BI_WS_GIANT] && weapon_swap_slay_giant)
				v += (dam * 4) / 2;
			if (!borg_skill[BI_WS_DRAGON] && !borg_skill[BI_WK_DRAGON] &&
				 weapon_swap_slay_dragon)
				v += (dam * 6) / 2;
			if (!borg_skill[BI_WB_ACID] && weapon_swap_brand_acid)
				v += (dam * 4) / 2;
			if (!borg_skill[BI_WB_ELEC] && weapon_swap_brand_elec)
				v += (dam * 5) / 2;
			if (!borg_skill[BI_WB_FIRE] && weapon_swap_brand_fire)
				v += (dam * 3) / 2;
			if (!borg_skill[BI_WB_COLD] && weapon_swap_brand_cold)
				v += (dam * 3) / 2;

			/* assume 5x base damage for x% of creatures */
			dam = damage * 5 * borg_skill[BI_BLOWS];
			if (!borg_skill[BI_WK_DRAGON] && weapon_swap_kill_dragon)
				v += (dam * 5) / 2;
			...
			/* Other Skills */
			if (!borg_skill[BI_SDIG] && weapon_swap_slow_digest)
				v += 10L;
			if (weapon_swap_aggravate)
				v -= 900000L;
			if (weapon_swap_teleport)
				v -= 900000L;
			if (decurse_weapon_swap != -1)
				v -= 5000L;
			if (!borg_skill[BI_REG] && weapon_swap_regenerate)
				v += 2000L;
			if (!borg_skill[BI_ESP] && weapon_swap_telepathy)
				v += 5000L;
			if (!borg_skill[BI_LITE] && weapon_swap_lite)
				v += 2000L;
			if (!borg_skill[BI_SINV] && weapon_swap_see_invis)
				v += 50000L;
			if (!borg_skill[BI_FEATH] && weapon_swap_ffall)
				v += 10L;
			if (!borg_skill[BI_FRACT] && weapon_swap_free_act)
				v += 10000L;
			if (!borg_skill[BI_HLIFE] && (borg_skill[BI_MAXCLEVEL] < 50) &&
				 weapon_swap_hold_life)
				v += 2000L;
			if (!borg_skill[BI_IFIRE] && weapon_swap_immune_fire)
				v += 70000L;
			if (!borg_skill[BI_IACID] && weapon_swap_immune_acid)
				v += 30000L;
			if (!borg_skill[BI_ICOLD] && weapon_swap_immune_cold)
				v += 50000L;
			if (!borg_skill[BI_IELEC] && weapon_swap_immune_elec)
				v += 25000L;
			if (!borg_skill[BI_RFIRE] && weapon_swap_resist_fire)
				v += 8000L;
			if (!borg_skill[BI_RACID] && weapon_swap_resist_acid)
				v += 6000L;
			...
			if (!borg_skill[BI_RFIRE] && !borg_skill[BI_RELEC] &&
				 !borg_skill[BI_RCOLD] && !borg_skill[BI_RACID] &&
				 (weapon_swap_resist_fire && weapon_swap_resist_acid &&
			...
			if (!borg_skill[BI_RCONF] && weapon_swap_resist_conf)
				v += 5000L;
			if (!borg_skill[BI_RSND] && weapon_swap_resist_sound)
				v += 2000L;
			if (!borg_skill[BI_RLITE] && weapon_swap_resist_lite)
				v += 800L;
			if (!borg_skill[BI_RDARK] && weapon_swap_resist_dark)
				v += 800L;
			if (!borg_skill[BI_RKAOS] && weapon_swap_resist_chaos)
				v += 8000L;
			if (!borg_skill[BI_RDIS] && weapon_swap_resist_disen)
				v += 5000L;
			if (!borg_skill[BI_RSHRD] && weapon_swap_resist_shard)
				v += 100L;
			if (!borg_skill[BI_RNXUS] && weapon_swap_resist_nexus)
				v += 100L;
			if (!borg_skill[BI_RBLIND] && weapon_swap_resist_blind)
				v += 5000L;
			if (!borg_skill[BI_RNTHR] && weapon_swap_resist_neth)
				v += 5500L;
			if (!borg_skill[BI_RFEAR] && weapon_swap_resist_fear)
				v += 5500L;
			...
			if (borg_skill[BI_MAXDEPTH] >= 75 &&
				...
				{
						if (!borg_skill[BI_ICOLD] && weapon_swap_immune_cold) v  +=
				90000L;
						if (!borg_skill[BI_IFIRE] && weapon_swap_immune_fire) v  +=
				90000L;
				}
				*/
			}
	...
	if ((borg_spell_legal_fail(REALM_SORCERY, 3, 4, 40) ||
		...
		if (item->to_h < 10) {
			...
			if (item->to_h < 0 && enchant_weapon_swap_to_h > 10)
		...
		if (item->to_d < 10) {
			...
			if (item->to_d < 0 && enchant_weapon_swap_to_d > 10)
				enchant_weapon_swap_to_d = 1;
		}
	} else {
		if (item->to_h < 8) {
			enchant_weapon_swap_to_h += (8 - item->to_h);
			if (item->to_h < 0 && enchant_weapon_swap_to_h > 8)
				enchant_weapon_swap_to_h = 1;
		}
...
void borg_notice_swap_armour(void) {
	...
	for (i = 0; i < INVEN_PACK; i++) {
		...
		case TV_DRAG_ARMOR: {
			 ...
			 */
			if (item->flags1 & TR1_SLAY_ANIMAL)
				armour_swap_slay_animal = TRUE;
			if (item->flags1 & TR1_SLAY_EVIL)
				armour_swap_slay_evil = TRUE;
			if (item->flags1 & TR1_SLAY_UNDEAD)
				armour_swap_slay_undead = TRUE;
			if (item->flags1 & TR1_SLAY_DEMON)
				armour_swap_slay_demon = TRUE;
			if (item->flags1 & TR1_SLAY_GIANT)
				armour_swap_slay_giant = TRUE;
			if (item->flags1 & TR1_SLAY_DRAGON)
				armour_swap_slay_dragon = TRUE;
			if (item->flags1 & TR1_KILL_DRAGON)
				armour_swap_kill_dragon = TRUE;
			if (item->flags1 & TR1_IMPACT)
			...
			/* Affect infravision */
			if (item->flags1 & TR1_INFRA)
				armour_swap_see_infra += item->pval;
			/* Affect various skills */
			/* Affect speed */
			 ...
			 */
			damage =
				 (borg_items[INVEN_WIELD].dd * borg_items[INVEN_WIELD].ds * 35L);
			...
			if (!borg_skill[BI_WS_ANIMAL] && !borg_skill[BI_WB_ELEC] &&
				...
				v += (dam * 2) / 2;
			if (!borg_skill[BI_WS_EVIL] && armour_swap_slay_evil)
				v += (dam * 7) / 2;
			/* assume 3x base damage for x% of creatures */
			dam = damage * 3 * borg_skill[BI_BLOWS];

			if (!borg_skill[BI_WS_UNDEAD] && armour_swap_slay_undead)
				v += (dam * 5) / 2;
			if (!borg_skill[BI_WS_DEMON] && armour_swap_slay_demon)
				v += (dam * 3) / 2;
			if (!borg_skill[BI_WS_GIANT] && armour_swap_slay_giant)
				v += (dam * 4) / 2;
			if (!borg_skill[BI_WS_DRAGON] && !borg_skill[BI_WK_DRAGON] &&
				 armour_swap_slay_dragon)
				v += (dam * 6) / 2;
			if (!borg_skill[BI_WB_ACID] && armour_swap_brand_acid)
				v += (dam * 4) / 2;
			if (!borg_skill[BI_WB_ELEC] && armour_swap_brand_elec)
				v += (dam * 5) / 2;
			if (!borg_skill[BI_WB_FIRE] && armour_swap_brand_fire)
				v += (dam * 3) / 2;
			if (!borg_skill[BI_WB_COLD] && armour_swap_brand_cold)
				v += (dam * 3) / 2;

			/* assume 5x base damage for x% of creatures */
			dam = damage * 5 * borg_skill[BI_BLOWS];
			if (!borg_skill[BI_WK_DRAGON] && armour_swap_kill_dragon)
				v += (dam * 5) / 2;
			...
			if (!borg_skill[BI_SDIG] && armour_swap_slow_digest)
				v += 10L;
			if (armour_swap_aggravate)
				v -= 8000L;
			if (armour_swap_teleport)
				v -= 100000L;
			if (decurse_armour_swap != -1)
				v -= 5000L;
			if (!borg_skill[BI_REG] && armour_swap_regenerate)
				v += 2000L;
			if (!borg_skill[BI_ESP] && armour_swap_telepathy)
				v += 5000L;
			if (!borg_skill[BI_LITE] && armour_swap_lite)
				v += 2000L;
			if (!borg_skill[BI_SINV] && armour_swap_see_invis)
				v += 50000L;
			if (!borg_skill[BI_FEATH] && armour_swap_ffall)
				v += 10L;
			if (!borg_skill[BI_FRACT] && armour_swap_free_act)
				v += 10000L;
			if (!borg_skill[BI_HLIFE] && (borg_skill[BI_MAXCLEVEL] < 50) &&
				 armour_swap_hold_life)
				v += 2000L;
			if (!borg_skill[BI_IFIRE] && armour_swap_immune_fire)
				v += 70000L;
			if (!borg_skill[BI_IACID] && armour_swap_immune_acid)
				v += 30000L;
			if (!borg_skill[BI_ICOLD] && armour_swap_immune_cold)
				v += 50000L;
			if (!borg_skill[BI_IELEC] && armour_swap_immune_elec)
				v += 25000L;
			if (!borg_skill[BI_RFIRE] && armour_swap_resist_fire)
				v += 8000L;
			if (!borg_skill[BI_RACID] && armour_swap_resist_acid)
				v += 6000L;
			if (!borg_skill[BI_RCOLD] && armour_swap_resist_cold)
				v += 4000L;
			if (!borg_skill[BI_RELEC] && armour_swap_resist_elec)
				v += 3000L;
			/* extra bonus for getting all basic resist */
			if (!borg_skill[BI_RFIRE] && !borg_skill[BI_RELEC] &&
				 !borg_skill[BI_RCOLD] && !borg_skill[BI_RACID] &&
				 (armour_swap_resist_fire && armour_swap_resist_acid &&
				  armour_swap_resist_elec && armour_swap_resist_cold))
				v += 10000L;
			if (!borg_skill[BI_RPOIS] && armour_swap_resist_pois)
				v += 20000L;
			if (!borg_skill[BI_RCONF] && armour_swap_resist_conf)
				v += 5000L;
			if (!borg_skill[BI_RSND] && armour_swap_resist_sound)
				v += 2000L;
			if (!borg_skill[BI_RLITE] && armour_swap_resist_lite)
				v += 800L;
			if (!borg_skill[BI_RDARK] && armour_swap_resist_dark)
				v += 800L;
			if (!borg_skill[BI_RKAOS] && armour_swap_resist_chaos)
				v += 8000L;
			if (!borg_skill[BI_RDIS] && armour_swap_resist_disen)
				v += 5000L;
			if (!borg_skill[BI_RSHRD] && armour_swap_resist_shard)
				v += 500L;
			if (!borg_skill[BI_RNXUS] && armour_swap_resist_nexus)
				v += 100L;
			if (!borg_skill[BI_RBLIND] && armour_swap_resist_blind)
				v += 5000L;
			if (!borg_skill[BI_RNTHR] && armour_swap_resist_neth)
				v += 5500L;
			...
			{
				if (!borg_skill[BI_RSHRD] && armour_swap_resist_shard &&
			borg_skill[BI_MAXDEPTH] >= 65) v += 3000L;
			}
			*/
					...
					(!borg_skill[BI_IFIRE] && armour_swap_immune_fire)))
			{
				 ...
				 if (borg_race_death[TARRASQUE] == 0)
				 {
						if (!borg_skill[BI_ICOLD] && armour_swap_immune_cold) v  +=
			90000L;
						if (!borg_skill[BI_IFIRE] && armour_swap_immune_fire) v  +=
			90000L;
				 }
			}
			*/
...
void borg_notice(bool notice_swap) {
	...
	for (i = 0; i < 6; i++) {
		/* Cheat the exact number from the game.  This number is available to the
		 * player
	...
	borg_skill[BI_SRACID] = borg_skill[BI_RACID] || armour_swap_resist_acid ||
									weapon_swap_resist_acid;
	borg_skill[BI_SRELEC] = borg_skill[BI_RELEC] || armour_swap_resist_elec ||
									weapon_swap_resist_elec;
	borg_skill[BI_SRFIRE] = borg_skill[BI_RFIRE] || armour_swap_resist_fire ||
									weapon_swap_resist_fire;
	borg_skill[BI_SRCOLD] = borg_skill[BI_RCOLD] || armour_swap_resist_cold ||
									weapon_swap_resist_cold;
	borg_skill[BI_SRPOIS] = borg_skill[BI_RPOIS] || armour_swap_resist_pois ||
									weapon_swap_resist_pois;
	borg_skill[BI_SRFEAR] = borg_skill[BI_RFEAR] || armour_swap_resist_fear ||
									weapon_swap_resist_fear;
	borg_skill[BI_SRLITE] = borg_skill[BI_RLITE] || armour_swap_resist_lite ||
									weapon_swap_resist_lite;
	borg_skill[BI_SRDARK] = borg_skill[BI_RDARK] || armour_swap_resist_dark ||
									weapon_swap_resist_dark;
	borg_skill[BI_SRBLIND] = borg_skill[BI_RBLIND] || armour_swap_resist_blind ||
									 weapon_swap_resist_blind;
	borg_skill[BI_SRCONF] = borg_skill[BI_RCONF] || armour_swap_resist_conf ||
									weapon_swap_resist_conf;
	borg_skill[BI_SRSND] = borg_skill[BI_RSND] || armour_swap_resist_sound ||
								  weapon_swap_resist_sound;
	borg_skill[BI_SRSHRD] = borg_skill[BI_RSHRD] || armour_swap_resist_shard ||
									weapon_swap_resist_shard;
	borg_skill[BI_SRNXUS] = borg_skill[BI_RNXUS] || armour_swap_resist_nexus ||
									weapon_swap_resist_nexus;
	borg_skill[BI_SRNTHR] = borg_skill[BI_RNTHR] || armour_swap_resist_neth ||
									weapon_swap_resist_neth;
	borg_skill[BI_SRKAOS] = borg_skill[BI_RKAOS] || armour_swap_resist_chaos ||
									weapon_swap_resist_chaos;
	borg_skill[BI_SRDIS] = borg_skill[BI_RDIS] || armour_swap_resist_disen ||
								  weapon_swap_resist_disen;
	borg_skill[BI_SHLIFE] =
		 borg_skill[BI_HLIFE] || armour_swap_hold_life || weapon_swap_hold_life;
	borg_skill[BI_SFRACT] =
		 borg_skill[BI_FRACT] || armour_swap_free_act || weapon_swap_free_act;
}
...
static void borg_notice_home_dupe(borg_item *item, bool check_sval, int i) {
	...
	for (x = 0; x < i; x++) {
		...
		if ((item->tval == item2->tval) &&
			 ...
			 (item->name1 == item2->name1) && (item->name2 == item2->name2)) {
			dupe_count++;
		}
	}
...
static void borg_notice_home_aux2(borg_item *in_item, bool no_items) {
	...
	for (i = 0; i < (STORE_INVEN_MAX + (INVEN_TOTAL - INVEN_WIELD)); i++) {
		...
		if (item->flags2 & TR2_SUST_CON)
			num_sustain_str += item->iqty;
		if (item->flags2 & TR2_SUST_STR && item->flags2 & TR2_SUST_INT &&
			 item->flags2 & TR2_SUST_WIS && item->flags2 & TR2_SUST_DEX &&
		...
		case TV_GLOVES:
			...
			if (borg_skill[BI_INTMANA] && borg_skill[BI_MAXSP] > 3) {
				...
				if (item->iqty && (!(item->flags2 & TR2_FREE_ACT)) &&
		...
		case TV_HAFTED:
			/* case TV_DIGGING: */
			{
				...
				if (adj_str_hold[my_stat_ind[A_STR]] >= item->weight / 10) {
					...
					str_index =
						 (adj_stat[ADJ_STR_BLOW][my_stat_ind[A_STR]] * mul / div);
	...
	if (borg_skill[BI_SINT])
		num_fix_stat[A_INT] += 1000;
	if (borg_skill[BI_SWIS])
		num_fix_stat[A_WIS] += 1000;
	if (borg_skill[BI_SDEX])
		num_fix_stat[A_DEX] += 1000;
	if (borg_skill[BI_SCON])
		num_fix_stat[A_CON] += 1000;
	if (borg_skill[BI_SCHR])
		num_fix_stat[A_CHA] += 1000;
	...
	/* Immunity flags */
	if (f2 & (TR2_IM_FIRE))
		num_immune_fire = TRUE;
	if (f2 & (TR2_IM_ACID))
		num_immune_acid = TRUE;
	if (f2 & (TR2_IM_COLD))
		num_immune_cold = TRUE;
	if (f2 & (TR2_IM_ELEC))
		num_immune_elec = TRUE;

	/* Resistance flags */
	if (f2 & (TR2_RES_ACID))
		num_resist_acid = TRUE;
	if (f2 & (TR2_RES_ELEC))
		num_resist_elec = TRUE;
	if (f2 & (TR2_RES_FIRE))
		num_resist_fire = TRUE;
	if (f2 & (TR2_RES_COLD))
		num_resist_cold = TRUE;
	if (f2 & (TR2_RES_POIS))
		num_resist_pois = TRUE;
	if (f2 & (TR2_RES_LITE))
		num_resist_lite = TRUE;
	if (f2 & (TR2_RES_DARK))
		num_resist_dark = TRUE;
	if (f2 & (TR2_RES_BLIND))
		num_resist_blind = TRUE;
	if (f2 & (TR2_RES_CONF))
		num_resist_conf = TRUE;
	if (f2 & (TR2_RES_SOUND))
		num_resist_sound = TRUE;
	if (f2 & (TR2_RES_SHARDS))
		num_resist_shard = TRUE;
	if (f2 & (TR2_RES_NEXUS))
		num_resist_nexus = TRUE;
	if (f2 & (TR2_RES_NETHER))
		num_resist_neth = TRUE;
	if (f2 & (TR2_RES_CHAOS))
		num_resist_chaos = TRUE;
	if (f2 & (TR2_RES_DISEN))
		num_resist_disen = TRUE;

	/* Sustain flags */
	if (f2 & (TR2_SUST_STR))
		num_sustain_str = TRUE;
	if (f2 & (TR2_SUST_INT))
		num_sustain_int = TRUE;
	if (f2 & (TR2_SUST_WIS))
		num_sustain_wis = TRUE;
	if (f2 & (TR2_SUST_DEX))
		num_sustain_dex = TRUE;
	if (f2 & (TR2_SUST_CON))
		num_sustain_con = TRUE;
}
...
extern s32b borg_power_aux2(void) {
	...
	if (borg_skill[BI_INTMANA] && (((cur_wgt - max_wgt) / 10) > 0)) {
	...
	if (borg_worships_damage) {
		...
		value += borg_items[INVEN_WIELD].dd * borg_items[INVEN_WIELD].ds * 20L *
	...
	/* borg_worships_ac */
	if (borg_worships_ac)
		value += ((borg_skill[BI_ARMOR]) * 2500L);

	/* Fudge factors for normalizing the AC-Reward curve */
	if (borg_skill[BI_ARMOR] >= 16 && borg_skill[BI_ARMOR] <= 74)
		value += 28250L;
	if (borg_skill[BI_ARMOR] >= 75)
		value += 73750L;
	...
	for (i = INVEN_WIELD; i < INVEN_TOTAL; i++) {
		...
		multibonus =
			 (((item->flags2 & TR2_RES_POIS) != 0) +
			  ...
			  ((item->flags2 & TR2_RES_ACID) && (item->flags2 & TR2_RES_ELEC) &&
				(item->flags2 & TR2_RES_FIRE) && (item->flags2 & TR2_RES_COLD)) +
			  ((item->flags2 & TR2_SUST_STR) && (item->flags2 & TR2_SUST_INT) &&
				(item->flags2 & TR2_SUST_WIS) && (item->flags2 & TR2_SUST_DEX) &&
				(item->flags2 & TR2_SUST_CON)));

		if (multibonus >= 2)
			value += 500 * multibonus;
	}
		 ...
		 (!borg_skill[BI_IFIRE] && weapon_swap_immune_fire)))
	{
		 if (borg_race_death[TARRASQUE] == 0)
		 {
				if (!borg_skill[BI_ICOLD] && weapon_swap_immune_cold) value  +=
	90000L;
				if (!borg_skill[BI_IFIRE] && weapon_swap_immune_fire) value  +=
	90000L;
		 }
	}
	*/
	...
	if ((borg_skill[BI_ISCUT] || borg_skill[BI_ISPOISONED]) &&
		...
		value += 50000;
	if ((borg_skill[BI_ISCUT] || borg_skill[BI_ISPOISONED]) &&
		 borg_skill[BI_ACSW]) { /* usually takes more than one */
		k = 0;
		for (; k < 5 && k < borg_skill[BI_ACSW]; k++)
			value += 25000L;
	}
	if (borg_skill[BI_ISPOISONED] && borg_skill[BI_ACUREPOIS])
		value += 15000;
	if (borg_skill[BI_ISPOISONED] && amt_slow_poison)
		value += 5000;
	...
	if ((borg_class == CLASS_PRIEST || borg_class == CLASS_PALADIN) &&
		...
		k = 0;
		for (; k < 3 && k < borg_has[POTION_HEAL]; k++)
			value += 3000L;
	}
	...
	if (borg_skill[BI_MAXDEPTH] >= 99) {
		...
		if (borg_race_death[RACE_LILITH] == 0 || borg_race_death[RACE_LUCIFER] ==
		0)
		{
				for (k = 0; k < 99 && k < borg_has[POTION_HEAL]; k++) value +=
		8000L;
				for (k = 0; k < 99 && k < borg_skill[BI_AEZHEAL]; k++) value
		+=10000L;
		...
		*/
		...
		if (borg_race_death[RACE_LUCIFER] == 0) {
			...
			if ((num_heal_true + borg_has[POTION_HEAL] + num_ez_heal_true +
						borg_skill[BI_AEZHEAL] <
				  45) ||
			...
			/* I have enough, carry all */
				...
				for (k = 0; k < 99 && k < borg_has[POTION_HEAL]; k++)
					value += 8000L;
				for (k = 0; k < 99 && k < borg_skill[BI_AEZHEAL]; k++)
					value += 10000L;
				for (k = 0; k < 99 && k < borg_skill[BI_ASPEED]; k++)
					value += 7000L;
				/* Restore Mana */
				if (borg_class != CLASS_WARRIOR) {
					for (k = 0; k < 99 && k < borg_has[POTION_RES_MANA]; k++)
						value += 5000L;
				}
				/* Shrooms of Restoring */
				for (k = 0; k < 35 && k < amt_fix_stat[6]; k++)
					value += 5000L;
				/* No need to store extras in home */
				borg_scumming_pots = FALSE;
			}
	...
	if (borg_skill[BI_CDEPTH] <= 98) {
		k = 0;
		for (; k < 20 && k < borg_skill[BI_ASPEED]; k++)
			value += 5000L;
	}
	...
	if (borg_skill[BI_CLEVEL] < 15) {
		...
		for (; k < 5 && k < borg_skill[BI_ACSW]; k++)
			value += 250L;
		for (; k < 10 && k < borg_skill[BI_ACSW]; k++)
			value += 55L;
	}

	/* Hack -- Reward add stat */
	if (amt_add_stat[A_STR])
		value += 50000;
	if (amt_add_stat[A_INT])
		value += 20000;
	if (borg_skill[BI_INTMANA])
		if (amt_add_stat[A_INT])
			value += 75000;

	if (amt_add_stat[A_WIS])
		value += 20000;
	if (borg_skill[BI_WISMANA])
		if (amt_add_stat[A_WIS])
			value += 75000;
	if (amt_add_stat[A_DEX])
		value += 50000;
	if (amt_add_stat[A_CON])
		value += 50000;
	if (amt_add_stat[A_CHA])
		value += 10000;

	/* Hack -- Reward fix stat */
	if (amt_fix_stat[A_STR])
		value += 10000;
	if (amt_fix_stat[A_INT])
		value += 10000;
	if (amt_fix_stat[A_WIS])
		value += 10000;
	if (amt_fix_stat[A_DEX])
		value += 10000;
	if (amt_fix_stat[A_CON])
		value += 10000;
	if (amt_fix_stat[A_CHA])
		value += 10000;

	/* Reward Remove Curse */
	if (borg_wearing_cursed) {
		if (borg_has[SCROLL_STAR_CURSE])
			value += 90000;
		if (borg_has[SCROLL_CURSE])
			value += 90000;
	}
	...
	if (amt_enchant_to_a && (my_need_enchant_to_a || enchant_armour_swap_to_a))
		value += 514L;
	...
	if (amt_enchant_to_d && (my_need_enchant_to_d || enchant_weapon_swap_to_d))
		value += 109L;
	/* Reward *enchant weapon* to damage */
	if (amt_enchant_weapon)
		value += 5000L;
	/* Reward *enchant armour*  */
	if (amt_enchant_armor)
		value += 5000L;
	/* Reward carrying a shovel if low dlevel */
	if ((borg_skill[BI_MAXDEPTH] <= 54 || borg_class == CLASS_MYSTIC) &&
		 borg_items[INVEN_WIELD].tval != TV_DIGGING && borg_skill[BI_CDEPTH] &&
		 amt_digger == 1)
		value += 5000L;
	...
	for (realm = 0; realm < MAX_REALM + 1; realm++) {
		for (book = 0; book < 4; book++) {
			...
			if ((when > 5) && (when >= borg_skill[BI_MAXCLEVEL] + 2))
				continue;

			/* Reward the book */
			k = 0;
			for (; k < 1 && k < amt_book[realm][book]; k++)
				value += 500000L;
	...
	/* Hack -- Apply "encumbrance" penalty from weight if I don't have Feather.
	 * I can drown if encumbered.*/
...
extern s32b borg_power_aux3(void) {
	...
	if ((!borg_skill[BI_NO_MELEE] && borg_class != CLASS_MYSTIC) ||
		...
		if (damage) {
			...
			if (borg_skill[BI_MAXDEPTH] >= 75) {
				...
				value += item->dd * item->ds * 5L * 2 * borg_skill[BI_BLOWS];
			}
		}

		/* assume 2x base damage for x% of creatures */
		dam = damage * 2 * borg_skill[BI_BLOWS];
		if (borg_skill[BI_WS_ANIMAL])
			value += (dam * 2) / 2;
		if (borg_skill[BI_WB_POIS])
			value += (dam * 2) / 2;
		...
		if (borg_skill[BI_WS_DEMON])
			value += (dam * 6) / 2;
		if (borg_fighting_demon && borg_skill[BI_WS_DEMON])
			value += (dam * 2);
		if (borg_skill[BI_WS_DRAGON] && (!borg_skill[BI_WK_DRAGON]))
			value += (dam * 9) / 2;
		...
		if (borg_skill[BI_WB_COLD])
			value += (dam * 6) / 2;
		if (borg_skill[BI_WB_VORPAL])
			value += (dam * 6) / 2;
		if (borg_skill[BI_WB_CHAOTIC])
			value += (dam * 15) / 10;
		...
		if (borg_skill[BI_WK_DRAGON]) {
			...
			if (borg_fighting_dragon)
				value += 50000 + (dam * 5) / 2;
			if (borg_fighting_dragon && borg_fighting_questor)
				value += 150000 + (dam * 5) / 2;
			if (borg_fighting_dragon && borg_fighting_unique)
				value += 150000 + (dam * 5) / 2;
		}
		...
		if (borg_skill[BI_WB_VAMPIRIC])
			value += 5000 + (dam * 5) / 2;
		/* Even Greater (if Warrior) bonus to borg for the vamp weapons */
		if (borg_skill[BI_WB_VAMPIRIC] && borg_class == CLASS_WARRIOR)
			value += 5000 + (dam * 5) / 2;

		/* It is only on Grond */
		if (borg_skill[BI_W_IMPACT])
			value += 5000L;

		/* Bonus for some types based on the xtra1.c */
		switch (borg_class) {
		/* Warrior.  Can use 15 lb weapons without penalty at level 1, and 45 lb
		...
		/* Mage/High Mage.  Can use 6 lb weapons without penalty at level 1, and
		 * 16 lb weapons without penalty at 50th level. */
		case CLASS_MAGE:
		case CLASS_HIGH_MAGE: {
			value +=
				 (20 + (2 * borg_skill[BI_CLEVEL] / 3) - (item->weight / 3)) * 5;
			break;
		}

		/* Priest.  Can use 12 lb weapons without penalty at level 1, and 22 lb
		 * weapons without penalty at 50th level. */
		case CLASS_PRIEST: {
			value +=
				 (30 + (1 * borg_skill[BI_CLEVEL] / 2) - (item->weight / 4)) * 5;
			break;
		}
		...
		* weapons.  */
		case CLASS_ROGUE: {
			value +=
				 (33 + (2 * borg_skill[BI_CLEVEL] / 3) - (item->weight / 3)) + 5;
			break;
		}
		...
		*weapons without penalty at 50th level. */
		case CLASS_RANGER: {
			value +=
				 (25 + (1 * borg_skill[BI_CLEVEL] / 2) - (item->weight / 5)) * 5;
			break;
		}
	...
	} else if (borg_class == CLASS_MYSTIC &&
				  !borg_items[INVEN_WIELD].iqty) /* Martial Artists */
	{
		int ma = MAX_MA - 1;
	...
	if (borg_skill[BI_HEAVYWEPON])
		value -= 500000L;

	/* HACK -- Borg worships num_blow, even on broken swords.
	 * kind 47 is a broken sword usually 1d2 in damage
 * The no_melee types can accept using the broken items with a penalty for doing
 * so.
 * But, melee types will not tolerate using a broken item
 */
	if (borg_skill[BI_NO_MELEE] &&
		 (item->kind == K_BROKEN_A || item->kind == K_BROKEN_B ||
		  item->kind == K_BROKEN_C))
		value -= 5000L;
	if (!borg_skill[BI_NO_MELEE] &&
		 (item->kind == K_BROKEN_A || item->kind == K_BROKEN_B ||
		  item->kind == K_BROKEN_C))
	...
	if (!borg_skill[BI_NO_MELEE]) {
		...
		if (borg_skill[BI_CLEVEL] < 15)
			value += (borg_skill[BI_SHOTS] * damage * 200L);

		/* Certain guys do not need to carry a bow for damage reasons.  They carry
		 * one for abilities */
		if (!borg_skill[BI_NO_MELEE]) {
			/* slings are very important for level 1 guys.  Slings are cheap and
			 * ammo is cheap  */
			if (item->sval == SV_SLING && my_stat_ind[A_STR] >= 10 &&
				 borg_skill[BI_CLEVEL] == 1) {
				value = 80000L;
			}

			/* slings force you to carry heavy ammo.  Penalty for that unles you
			 * have lots of str  */
			if (item->sval == SV_SLING && !item->name1 && my_stat_ind[A_STR] < 9 &&
			...
			if (borg_class == CLASS_RANGER && my_ammo_tval == TV_ARROW)
				value += 30000L;
			if (borg_class == CLASS_RANGER &&
				 (my_ammo_tval == TV_SHOT || my_ammo_tval == TV_BOLT))
				value -= 300L;

			/* Prefer Slings due to skill bonuses */
			if ((borg_class == CLASS_MYSTIC || borg_class == CLASS_ROGUE) &&
				 my_ammo_tval == TV_SHOT)
				value += 300L;
		}
	...
	for (i = INVEN_LEFT; i <= INVEN_RIGHT; i++) {
		...
		if (item->kind == RING_FLAMES && !borg_skill[BI_IFIRE])
			value += 1000;
		if (item->kind == RING_ACID && !borg_skill[BI_IACID])
			value += 1000;
		if (item->kind == RING_ICE && !borg_skill[BI_ICOLD])
			value += 1000;
	}
	...
	/* Sometimes, we want the borg to wield an empty lantern.  It helps him when
	 * choosing fuel. */
	if (borg_items[INVEN_LITE].tval == TV_LITE &&
		 borg_items[INVEN_LITE].pval == 0) {
		value += 10;

		/* Now, when shopping, be sure to select the fuel instead of buying a new
		 * lantern */
		if (borg_skill[BI_AFUEL] >= 1)
			value += 2000200L;
	}
	...
	if ((borg_skill[BI_WISMANA]) && (my_stat_ind[A_WIS] <= 37)) {
		...
		/* Bonus for sp. */
		value +=
			 ((adj_stat[ADJ_MANA][my_stat_ind[A_WIS]] * borg_skill[BI_CLEVEL]) /
			  2) *
			 150L;
		...
		if (borg_class == CLASS_PRIEST || borg_class == CLASS_ORPHIC) {
			...
			if (borg_spell_legal(REALM_MIRACLES, 2, 6))
				value += my_stat_ind[A_WIS] * 35000L;
	...
	if (my_stat_ind[A_CON] <= 37) {
		int bonus_hp = (((adj_stat[ADJ_HP][my_stat_ind[A_CON]] - 128) *
		...
		if (borg_worships_hp) {
			...
			/*         This is a bit wierd because we are not really giving a bonus
			 * for */
	...
	if ((borg_class == CLASS_WARRIOR || borg_class == CLASS_CHAOS_KNIGHT) &&
		 borg_skill[BI_RFEAR])
		value += 2000L;
	...
	if (borg_skill[BI_RFIRE])
		value += 8000L;
	/* extra bonus for getting all basic resist */
	if (borg_skill[BI_RFIRE] && borg_skill[BI_RACID] && borg_skill[BI_RELEC] &&
		 borg_skill[BI_RCOLD])
		value += 10000L;
	...
	if (borg_skill[BI_RDIS] && borg_skill[BI_MAXDEPTH] >= 50)
		value += 5000L;
	if (borg_skill[BI_RSHRD] && borg_skill[BI_MAXDEPTH] >= 50)
		value += 3000L;

	/* TODO Make sure there is no Hellband equivalent */
	/* Special concern for Cyberdemons */

	if (borg_skill[BI_RNXUS] && borg_skill[BI_MAXDEPTH] >= 50)
		value += 100L;
	if (borg_skill[BI_RBLIND])
		value += 5000L;
	if (borg_skill[BI_RNTHR] && borg_skill[BI_MAXDEPTH] >= 50)
		value += 5500L;
	if (borg_skill[BI_REFLECT])
		value += 8000L;
	...
	if (borg_skill[BI_SSTR] && borg_skill[BI_SINT] && borg_skill[BI_SWIS] &&
		 borg_skill[BI_SDEX] && borg_skill[BI_SCON])
		value += 1000L;

	/* Interesting Flags */
	if (borg_skill[BI_FIRESH])
		value += 500;
	if (borg_skill[BI_ELECSH])
		value += 300;
	if (borg_skill[BI_WRAITH])
		value += 500;
	...
	if ((borg_skill[BI_SINV] || borg_skill[BI_ESP] || borg_skill[BI_SSINV] ||
		  borg_skill[BI_SESP]) &&
		 (borg_skill[BI_MAXDEPTH] + 1 >= 10))
		value += 100000L;

	/* Mega-Hack -- Free action (level 20) */
	if (borg_skill[BI_FRACT] && (borg_skill[BI_MAXDEPTH] + 1 >= 20))
		value += 100000L;

	/*  Mega-Hack -- resists (level 25) */
	if (borg_skill[BI_RFIRE] && (borg_skill[BI_MAXDEPTH] + 1 >= 25))
		value += 100000L;

	/*  Mega-Hack -- resists (level 40) */
	if (borg_skill[BI_RPOIS] && (borg_skill[BI_MAXDEPTH] + 1 >= 40))
		value += 100000L;
	if (borg_skill[BI_RELEC] && (borg_skill[BI_MAXDEPTH] + 1 >= 40))
		value += 100000L;
	if (borg_skill[BI_RACID] && (borg_skill[BI_MAXDEPTH] + 1 >= 40))
		value += 100000L;
	if (borg_skill[BI_RCOLD] && (borg_skill[BI_MAXDEPTH] + 1 >= 40))
		value += 100000L;
	if (borg_skill[BI_ESP] && (borg_skill[BI_MAXDEPTH] + 1 >= 40))
		value += 100000L;
	...
	if ((borg_skill[BI_HLIFE] && (borg_skill[BI_MAXDEPTH] + 1 >= 46) &&
		...
		value += 100000L;
	if ((borg_skill[BI_SPEED] >= 115) && (borg_skill[BI_MAXDEPTH] + 1 >= 46))
		value += 100000L;
	if (borg_skill[BI_RCONF] && (borg_skill[BI_MAXDEPTH] + 1 >= 46))
		value += 100000L;

	/*  Mega-Hack -- resist Nether is -very- nice to have at level 50 */
	if (borg_skill[BI_RNTHR] && (borg_skill[BI_MAXDEPTH] + 1 >= 50))
		value += 55000L;

	/*  Mega-Hack -- resist Sound to avoid being KO'd */
	if (borg_skill[BI_RSND] && (borg_skill[BI_MAXDEPTH] + 1 >= 50))
		value += 100000L;
	...
	if (borg_skill[BI_RDIS] && (borg_skill[BI_MAXDEPTH] + 1 >= 60))
		value += 90000L;
	if ((borg_skill[BI_SPEED] >= 120) && (borg_skill[BI_MAXDEPTH] + 1 >= 60))
		value += 100000L;

	/*  Must have +20 speed (level 80) */
	if ((borg_skill[BI_SPEED] >= 130) && (borg_skill[BI_MAXDEPTH] + 1 >= 80))
		value += 100000L;
	 ...
	 */
	if (borg_skill[BI_RDARK] && (borg_skill[BI_MAXDEPTH] + 1 >= 80))
		value += 25000L;
	...
	if (borg_worships_ac) {
		...
		if (borg_skill[BI_ARMOR] >= 15 && borg_skill[BI_ARMOR] < 75)
			value += ((borg_skill[BI_ARMOR]) * 2000L) + 28250L;
		if (borg_skill[BI_ARMOR] >= 75)
			value += ((borg_skill[BI_ARMOR]) * 1500L) + 73750L;
	} else if (!borg_skill[BI_NO_MELEE]) {
		if (borg_skill[BI_ARMOR] < 15)
			value += ((borg_skill[BI_ARMOR]) * 2000L);
		if (borg_skill[BI_ARMOR] >= 15 && borg_skill[BI_ARMOR] < 75)
			value += ((borg_skill[BI_ARMOR]) * 500L) + 28350L;
		if (borg_skill[BI_ARMOR] >= 75)
			value += ((borg_skill[BI_ARMOR]) * 100L) + 60000L;
	} else if (borg_skill[BI_NO_MELEE]) {
		if (borg_skill[BI_ARMOR] < 15)
			value += ((borg_skill[BI_ARMOR]) * 2000L);
		if (borg_skill[BI_ARMOR] >= 15 && borg_skill[BI_ARMOR] < 75)
			value += ((borg_skill[BI_ARMOR]) * 500L) + 14175L;
		if (borg_skill[BI_ARMOR] >= 75)
			value += ((borg_skill[BI_ARMOR]) * 100L) + 44175L;
	}

	/*** Penalize various things ***/

	/* Penalize various flags */
	if (borg_skill[BI_CRSTELE])
		value -= 100000L;
	if (borg_skill[BI_CRSAGRV])
		value -= 800000L;
	if (borg_skill[BI_CRSNOTELE])
		value -= 900000L;
	if (borg_skill[BI_CRSNOMAGIC] && borg_class != CLASS_WARRIOR)
		value -= 900000L;
	...
	if ((borg_skill[BI_MAXDEPTH] + 1 == 100 || borg_skill[BI_CDEPTH] == 100) &&
		...
		if (borg_skill[BI_INTMANA] && borg_skill[BI_SINT])
			value += 45000L;
		/* extra bonus for spell casters */
		if (borg_skill[BI_WISMANA] && borg_skill[BI_SWIS])
			value += 35000L;
		if (borg_skill[BI_SCON])
			value += 55000L;
		if (borg_skill[BI_SDEX])
		...
		if (borg_skill[BI_RPOIS])
			value += 100000L;
		if (borg_skill[BI_RFIRE] && borg_skill[BI_RACID] &&
			 borg_skill[BI_RELEC] && borg_skill[BI_RCOLD])
			value += 100000L;
	}
	...
	for (i = INVEN_WIELD; i < INVEN_TOTAL; i++) {
		...
		/* Bonus for the activation rings.  Not too big a bonus.  The warriors are
		 * given
		...
		if (item->name1 == ART_IRONFIST || item->name1 == ART_DAGGER_INFERNO ||
			 item->xtra2 == ACT_BO_FIRE_1) {
			value += (500 + (9 * (8 + 1) / 2));
		}
		/* Artifact -- Nimthanc- frost bolt 6d8*/
		else if (item->name1 == ART_COCYTUS || item->name1 == ART_GHOULS ||
					item->xtra2 == ACT_BO_COLD_1) {
			value += (500 + (6 * (8 + 1) / 2));
		}

		/* Artifact -- Dethanc- electric bolt 4d8*/
		else if (item->name1 == ART_DAGGER_FURCIFER ||
					item->name1 == ART_FURFICER || item->xtra2 == ACT_BO_ELEC_1) {
			value += (500 + (4 * (8 + 1) / 2));
		}
		...
		/* Fire ball 72*/
		else if (item->name1 == ART_RASHAVERAK ||
					item->name1 == ART_MORNINGSTAR || item->xtra2 == ACT_BA_FIRE_1) {
			value += (500 + 144);
		}
		...
		else if (item->name1 == ART_AMULET_RAPHAEL ||
					item->name1 == ART_AMULET_MICHAEL) {
			value += (500 + (50 * 5) + (50 * 5));
		}
		...
		else if (item->name1 == ART_JOSEPH || item->name1 == ART_SAMAEL ||
					item->name1 == ART_ABADDON || item->name1 == ART_BLOOD_MICHAEL) {
			value += (500 + (50 * 5));
		...
		/* Other randart activations not already considered in borg_power_4() */
			...
			value += (50000);
					  ...
					  !borg_spell_legal(REALM_DEATH, 3, 7)) {
			value += 30000;
		} else if (item->xtra2 == ACT_DRAIN_1 || item->xtra2 == ACT_BA_FIRE_2 ||
					  item->xtra2 == ACT_BA_COLD_3 || item->xtra2 == ACT_BA_ELEC_3 ||
			...
			value += (500 + 300);
					  ...
					  item->xtra2 == ACT_CALL_CHAOS ||
					  item->xtra2 == ACT_BA_MISS_3 || item->xtra2 == ACT_GENOCIDE ||
					  item->xtra2 == ACT_MASS_GENO) {
			value += (500 + 800);
		}

		/* Artifact -- Thingol recharge ---- already looked at in borg_notice()
		 * and borg_power_4() */
 ...
 * In here, we must subtract out the bonus granted from certain
	...
	/* Reward fuel */
	k = 0;
	for (; k < 5 && k < borg_skill[BI_AFUEL]; k++)
		value += 60000L;
	for (; k < 10 && k < borg_skill[BI_AFUEL]; k++)
		value += 6000L;
	...
	if ((borg_skill[BI_ISHUNGRY] || borg_skill[BI_ISWEAK]) &&
		...
		value += 100000;
	k = 0;
	for (k = 0; k < 7 && k < borg_skill[BI_FOOD]; k++)
		value += 50000L;
	for (k = 0; k < 10 && k < borg_skill[BI_FOOD]; k++)
		value += 200L;
	if (borg_skill[BI_REG] && !borg_skill[BI_SDIG]) {
		k = 0;
		for (; k < 20 && k < borg_skill[BI_FOOD]; k++)
			value += 500L;
	}
	/* Prefere to buy HiCalorie foods over LowCalorie */
	if (amt_food_hical <= 5)
	...
	if (borg_class == CLASS_WARRIOR && borg_skill[BI_CLEVEL] > 40) {
		k = 0;
		for (; k < 4 && k < borg_skill[BI_ARESALL]; k++)
			value += 550L;
	}
	...
	if (borg_skill[BI_CLEVEL] >= 12) {
		for (k = 0; k < 10 && k < borg_skill[BI_AID]; k++)
			value += 600L;
		for (k = 0; k < 15 && k < borg_skill[BI_AID]; k++)
			value += 100L;
	}
	...
	if (my_need_id) {
		k = 0;
		for (; k < my_need_id && k < borg_skill[BI_AID]; k++)
			value += 6000L;
	}
...
*/
	...
	if (borg_munchkin_start && borg_skill[BI_MAXCLEVEL] <= borg_munchkin_level) {
		...
		for (k = 0; k < 10 && k < borg_skill[BI_APFE]; k++)
			value += 10000L;
		for (k = 0; k < 25 && k < borg_skill[BI_APFE]; k++)
			value += 2000L;
	}

	/*  Reward Glyph- Rune of Protection-  carry lots of these*/
	k = 0;
	for (k = 0; k < 10 && k < borg_skill[BI_AGLYPH]; k++)
		value += 10000L;
	for (k = 0; k < 25 && k < borg_skill[BI_AGLYPH]; k++)
		value += 2000L;

	/* Reward recall */
	k = 0;
	if (borg_skill[BI_CLEVEL] >= 25) {
		for (k = 0; k < 4 && k < borg_skill[BI_RECALL]; k++)
			value += 50000L;
		for (k = 0; k < 7 && k < borg_skill[BI_RECALL]; k++)
			value += 5000L;
	...
	if (borg_skill[BI_MAXDEPTH] >= 50 && borg_has[ROD_RECALL])
		value += 12000;
	if (borg_skill[BI_MAXDEPTH] >= 50 && borg_has[ROD_RECALL] >= 2)
		value += 12000;
	...
	if (borg_munchkin_start && borg_skill[BI_MAXCLEVEL] <= borg_munchkin_level) {
		...
		for (k = 0; k < 5 && k < borg_skill[BI_AESCAPE]; k++)
			value += 10000L;
	}
	if (borg_skill[BI_CDEPTH] > 90) {
		for (k = 0; k < 15 && k < borg_skill[BI_AESCAPE]; k++)
			value += 10000L;
	}
	...
	if (borg_munchkin_start && borg_skill[BI_MAXCLEVEL] <= borg_munchkin_level) {
		...
		if (borg_skill[BI_CLEVEL] >= 3) {
			if (borg_skill[BI_ATELEPORT])
				value += 10000L;
		}
		if (borg_skill[BI_CLEVEL] >= 7) {
			for (; k < 3 && k < borg_skill[BI_ATELEPORT]; k++)
				value += 10000L;
		}
		if (borg_skill[BI_CLEVEL] >= 30) {
			for (; k < 10 && k < borg_skill[BI_ATELEPORT]; k++)
				value += 8000L;
		}
	}
	...
	if (borg_munchkin_start && borg_skill[BI_MAXCLEVEL] <= borg_munchkin_level) {
		...
		for (; k < 5 && k < borg_skill[BI_ATELEPORTLVL]; k++)
			value += 5000L;
	}
			...
			*/
			...
			if (borg_race_death[RACE_LUCIFER] == 0) {
				...
				/* I have enough, carry all pots */
					...
					k = 0;
					for (; k < 99 && k < borg_has[POTION_HEAL]; k++)
						value += 8000L;
					k = 0;
					for (; k < 99 && k < borg_skill[BI_AEZHEAL]; k++)
						value += 10000L;
					k = 0;
					for (; k < 99 && k < borg_skill[BI_ALIFE]; k++)
						value += 10000L;
					k = 0;
					for (; k < 99 && k < borg_skill[BI_ASPEED]; k++)
						value += 8000L;
					/* Restore Mana */
					k = 0;
					if (borg_class != CLASS_WARRIOR) {
						for (; k < 99 && k < borg_has[POTION_RES_MANA]; k++)
							value += 5000L;
					}
					/* Shrooms of Restoring */
					k = 0;
					for (; k < 35 && k < amt_fix_stat[6]; k++)
						value += 5000L;
				}
			}
		}
	...
	if (borg_skill[BI_MAXSP] > 100) {
		k = 0;
		for (; k < 10 && k < borg_has[POTION_RES_MANA]; k++)
			value += 4000L;
		k = 0;
		for (; k < 100 && k < borg_skill[BI_ASTFMAGI]; k++)
			value += 4000L;
	}
	...
	if (borg_skill[BI_CLEVEL] < 35 && borg_skill[BI_CLEVEL] >= 10) {
		k = 0;
		for (; k < 7 && k < borg_skill[BI_ACSW]; k++)
			value += 550L;
		for (; k < 12 && k < borg_skill[BI_ACSW]; k++)
			value += 55L;
	}

	/* Reward light serious -- Low Level Characters */
	if (borg_skill[BI_CLEVEL] < 5 && borg_skill[BI_ACSW] <= 5 &&
		 !borg_munchkin_start) {
		k = 0;
		for (; k < 6 && k < borg_skill[BI_ACLW]; k++)
			value += 550L;
	}
	...
	if (borg_munchkin_start && borg_skill[BI_MAXCLEVEL] <= 10) {
		...
		if (!borg_skill[BI_RPOIS]) {
			k = 0;
			for (; k < 5 && k < borg_skill[BI_ACUREPOIS]; k++)
				value += 250L;
		}
	}
	...
	if (borg_munchkin_start && borg_skill[BI_MAXCLEVEL] <= borg_munchkin_level) {
		...
		k = 0;
		for (; k < 1 && k < borg_skill[BI_ADETTRAP]; k++)
			value += 4000L;

		/* Reward detect door */
		k = 0;
		for (; k < 1 && k < borg_skill[BI_ADETDOOR]; k++)
			value += 2000L;

		/* Reward detect evil */
		if (!borg_skill[BI_ESP]) {
			k = 0;
			for (; k < 1 && k < borg_skill[BI_ADETEVIL]; k++)
				value += 1000L;
		}
	}
	...
	if (borg_munchkin_start && borg_skill[BI_MAXCLEVEL] <= borg_munchkin_level) {
		...
		for (; k < 1 && k < borg_skill[BI_AMAGICMAP]; k++)
			value += 4000L;
	}

	/* Reward Call Light */
	k = 0;
	for (; k < 1 && k < borg_skill[BI_ALITE]; k++)
		value += 1000L;

	/* Genocide scrolls. Just scrolls, mainly used for Morgoth */
	if (borg_skill[BI_MAXDEPTH] >= 98) {
		k = 0;
		for (; k < 10 && k < borg_has[SCROLL_GENOCIDE]; k++)
			value += 10000L;
		k = 0;
		for (; k < 25 && k < borg_has[SCROLL_GENOCIDE]; k++)
			value += 2000L;
	}
	...
	if (borg_skill[BI_MAXDEPTH] >= 98) {
		k = 0;
		for (; k < 10 && k < borg_has[SCROLL_MGENOCIDE]; k++)
			value += 10000L;
		for (; k < 25 && k < borg_has[SCROLL_MGENOCIDE]; k++)
			value += 2000L;
	}
	...
	if (borg_skill[BI_MAXDEPTH] >= 98) {
		k = 0;
		for (; k < 15 && k < borg_has[POTION_INVULN]; k++)
			value += 10000L;
		k = 0;
		for (; k < 99 && k < borg_has[POTION_INVULN]; k++)
			value += 2000L;
	}

	/* Reward Recharge ability */
	if ((borg_skill[BI_CLEVEL] <= 15 &&
		  (borg_has[WAND_MM] || borg_has[WAND_SCLOUD])) ||
		 borg_skill[BI_ARECHARGE] >= 1000) {
		/* Reward the first one */
		if (borg_skill[BI_ARECHARGE])
			value += 20000;
		...
		if (borg_munchkin_start &&
			...
			k = 0;
			for (; k < 5 && k < borg_skill[BI_ARECHARGE]; k++)
				value += 10000L;
		}
	...
	if (borg_class == CLASS_RANGER) {
		if (borg_skill[BI_CLEVEL] < 5) {
			k = 0;
			for (; k < 20 && k < borg_skill[BI_AMISSILES]; k++)
				value += 100000L;
		}
		k = 0;
		for (; k < 30 && k < borg_skill[BI_AMISSILES]; k++)
			value += 1000L;
		k = 0;
		for (; k < 80 && k < borg_skill[BI_AMISSILES]; k++)
			value += 100L;
	} else if (!borg_skill[BI_NO_MELEE]) {
		k = 0;
		if (borg_skill[BI_CLEVEL] < 5) {
		...
		for (; k < 20 && k < borg_skill[BI_AMISSILES]; k++)
			value += 1000L;
		k = 0;
		for (; k < 50 && k < borg_skill[BI_AMISSILES]; k++)
			value += 100L;
	}
	...
	if (borg_skill[BI_ASTFDEST])
		value += 5000L;
	k = 0;
	for (; k < 3 && k < borg_skill[BI_ASTFDEST]; k++)
		value += 200L;

	/* Wands of attack are good to carry for low level guys */
	if (borg_skill[BI_CLEVEL] <= 15) {
		if (borg_has[WAND_MM])
			value += 6500L; /* Magic Missile */
	}
	if (borg_skill[BI_CLEVEL] >= 15 && borg_skill[BI_CLEVEL] < 25) {
		if (borg_has[WAND_SCLOUD])
			value += 5500L; /* Stinking Cloud */
	}

	/* Stone to Mud if really important for avoiding summoners */
	k = 0;
	for (; k < 25 && k < borg_skill[BI_ASTONE2MUD]; k++)
		value += 1500L;
	...
	if (borg_skill[BI_STR] < 18) {
		...
		if (borg_skill[BI_MAXDEPTH] <= 30) {
			for (; k < 5 && k < borg_skill[BI_AROD1]; k++)
				value += 8000;
		}
		k = 0;
		for (; k < 5 && k < borg_skill[BI_AROD2]; k++)
			value += 12000;
	}
	if (borg_skill[BI_STR] >= 18) {
		k = 0;
		...
		k = 0;
		for (; k < 10 && k < borg_skill[BI_AROD2]; k++)
			value += 12000;
	}
	...
	/* Reward enchant weapon to hit */
	if (amt_enchant_to_h && (my_need_enchant_to_h || enchant_weapon_swap_to_h) &&
		 !borg_skill[BI_NO_MELEE] && borg_skill[BI_CLEVEL] > 35)
		value += 240L;
	...
	/* Reward *enchant weapon* to damage */
	if (amt_enchant_weapon &&
		 (my_need_enchant_to_h && my_need_enchant_to_d &&
		  enchant_weapon_swap_to_d && enchant_weapon_swap_to_h) &&
		 !borg_skill[BI_NO_MELEE] && borg_skill[BI_CLEVEL] > 35)
		value += 5000L;

	/* Reward *enchant armour*  */
	if (amt_enchant_armor && (my_need_enchant_to_a || enchant_armour_swap_to_a))
		value += 5000L;
	...
	if (borg_skill[BI_CLEVEL] == 1) {
		k = 0;
		for (; k < 10 && k < borg_has[POTION_HEAL]; k++)
			value -= 20000L;
	}
...
static s32b borg_power_home_aux2(void) {
	...
	for (k = 0; k < 100 && k < num_genocide; k++)
		value += 5000L - k * 10L;

	/* Reward Mass Genocide scrolls. Just scrolls, mainly used for Morgoth */
	for (k = 0; k < 100 && k < num_mass_genocide; k++)
		value += 5000L - k * 10L;

	/* Collect recall */
	if (borg_gold < 25000 || !borg_equips_rod(SV_ROD_RECALL)) {
		for (k = 0; k < 10 && k < num_recall; k++)
			value += 3000L - k * 10L;
	}

	/* Collect escape */
	for (k = 0; k < 10 && k < num_escape; k++)
		value += 2000L - k * 10L;

	/* Collect teleport */
	for (k = 0; k < 15 && k < num_teleport; k++)
		value += 400L - k * 8L;

	/* Collect teleport level scrolls*/
	for (k = 0; k < 20 && k < num_teleport_level; k++)
		value += 1000L - k * 8L;

	/* Collect Speed */
	for (k = 0; k < 99 && k < num_speed; k++)
		value += 5000L - k * 10L;

	/* Collect Invuln Potions */
	for (k = 0; k < 99 && k < num_goi_pot; k++)
		value += 5000L - k * 10L;

	/*** Healing ***/

	/* collect heal/mana/ */
	for (k = 0; k < 99 && k < num_heal; k++)
		value += 3000L - k * 8L;
	for (k = 0; k < 99 && k < num_ez_heal; k++)
		value += 8000L - k * 8L;
	if (borg_skill[BI_MAXSP] > 1) {
		for (k = 0; k < 99 && k < num_mana; k++)
			value += 6000L - k * 8L;
	}

	/* Collect New Life */
	for (k = 0; k < 50 && k < num_new_life; k++)
		value += 5000L - k * 10L;
 ...
 * Mindcrafters get a Pot of Restore Mana. It is better
	...
	for (k = 0; k < 99 && k < num_fix_stat[6]; k++)
		value += 5000L;
...
static int borg_danger_aux1(int i, int y, int x, bool mon_v_mon) {
	...
	if ((borg_prot_from_evil) && (r_ptr->flags3 & RF3_EVIL) &&
		 ((borg_skill[BI_CLEVEL]) >= r_ptr->level)) {
		pfe = 1;
	}

	/* Mega-Hack -- unknown monsters */
	if (kill->r_idx >= MAX_R_IDX)
		return (1000);
	...
	for (k = 0; k < 4; k++) {
		...
		case RBE_HURT:
			...
			if (b_ptr->method == RBM_KICK)
				n += (borg_goi || borg_wraith)
							? 400 * 20
							: 400; /* If GOI is on, take that into account */
			power = 60;
			break;
		...
		case RBE_POISON:
			...
			power = 5;
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags3 & RF3_IM_POIS)
					z /= 9;
				break;
			}
			if (borg_skill[BI_RPOIS])
		...
		case RBE_EAT_GOLD:
			...
			if ((borg_goi || borg_wraith) && !borg_attacking)
				z = 0;
			if (100 <=
				 adj_stat[ADJ_DODGE][my_stat_ind[A_DEX]] + borg_skill[BI_CLEVEL])
				break;
			if (borg_gold < 100)
				break;
			if (borg_gold > 100000)
		...
		case RBE_ACID:
			...
			z = (b_ptr->d_dice * b_ptr->d_side);
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags3 & RF3_IM_ACID)
					z /= 9;
				break;
			}
			if (borg_skill[BI_VACID])
				z *= 2;
		...
		case RBE_ELEC:
			...
			power = 10;
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags3 & RF3_IM_ELEC)
					z /= 9;
				break;
			}
			/* if invulnurable, no damage */
		...
		case RBE_FIRE:
			...
			power = 10;
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags3 & RF3_IM_FIRE)
					z /= 9;
				break;
			}
			/* if invulnurable, no damage */
		...
		case RBE_COLD:
			...
			power = 10;
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags3 & RF3_IM_COLD)
					z /= 9;
				break;
			}
			/* if invulnurable, no damage */
		...
		case RBE_BLIND:
			...
			if ((borg_goi || borg_wraith) && !borg_attacking)
				z = 0;
			if (borg_skill[BI_RBLIND] || amt_cure_blind >= 2)
				break;
			if (!borg_full_damage)
				z += 10;
			if ((pfe) && !borg_attacking)
		...
		case RBE_CONFUSE:
			...
			power = 10;
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags3 & RF3_NO_CONF)
					z /= 9;
				break;
			}
			/* if invulnurable, no damage */
			if ((borg_goi || borg_wraith) && !borg_attacking)
				z = 0;
			if (borg_skill[BI_RCONF] || amt_cure_confusion >= 2)
				break;
		...
		case RBE_TERRIFY:
			...
			power = 10;
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags3 & RF3_NO_FEAR)
					z /= 9;
				break;
			}
			/* if invulnurable, no damage */
...
static int borg_danger_aux2(int i, int y, int x, bool average, bool mon_v_mon) {
	...
	for (q = 0; q < kill->ranged_attack; q++) {
		...
		case 96 + 3: /* RF4_ROCKETS */
			...
			if (z > 600)
				z = 600;
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags4 & RF4_BR_SHAR)
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 13: /* RF4_BR_NETH */
			...
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_NETH) ||
									  (r_info[target->r_idx].flags3 & RF3_UNDEAD)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 14: /* RF4_BR_LITE */
			...
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_LITE) ||
									  (!(r_info[target->r_idx].flags3 & RF3_HURT_LITE))))
					z /= 9;
				break;
			}
			/* Vamps hurt more */
		...
		case 96 + 15: /* RF4_BR_DARK */
			...
			if (z > 350)
				z = 350;
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags4 & RF4_BR_DARK)
					z /= 9;
				break;
			}
			if (borg_goi) {
		...
		case 96 + 17: /* RF4_BR_SOUN */
			...
			if (z > 350)
				z = 350;
			if (mon_v_mon) {
				if (ag->kill && r_info[target->r_idx].flags4 & RF4_BR_SOUN)
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 18: /* RF4_BR_CHAO */
			...
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_CHAO) ||
									  (r_info[target->r_idx].flags3 & RF3_DEMON)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 19: /* RF4_BR_DISE */
			...
			if (z > 400)
				z = 400;
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_DISE)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 20: /* RF4_BR_NEXU */
			...
			if (z > 250)
				z = 250;
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_NEXU)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 21: /* RF4_BR_TIME */
			...
			if (z > 150)
				z = 150;
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_TIME)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 22: /* RF4_BR_INER */
			...
			if (z > 200)
				z = 200;
			if (mon_v_mon) {
				if (ag->kill && (r_info[target->r_idx].flags4 & RF4_BR_INER))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 23: /* RF4_BR_GRAV */
			...
			if (z > 200)
				z = 200;
			if (mon_v_mon) {
				if (ag->kill && (r_info[target->r_idx].flags4 & RF4_BR_GRAV))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 25: /* RF4_BR_PLAS */
			...
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_PLAS) ||
									  (r_info[target->r_idx].flags3 & RF3_RES_PLAS)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 26: /* RF4_BR_WALL */
			...
			if (z > 200)
				z = 200;
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_WALL)))
					z /= 9;
				break;
			}
			if (borg_skill[BI_RSND])
		...
		case 96 + 27: /* RF4_MANA */
			...
			if (z > 250)
				z = 250;
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_MANA)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 28: /* RF4_BA_SLIM */
			...
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_SLIM) ||
									  (r_info[target->r_idx].flags3 & RF3_IM_POIS)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 96 + 31: /* RF4_BA_DISI */
			...
			if (z >= 300)
				z = 300;
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_DISI)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 128 + 0: /* RF5_BA_ACID */
			...
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_ACID) ||
									  (r_info[target->r_idx].flags3 & RF3_IM_ACID)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 128 + 1: /* RF5_BA_ELEC */
			...
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_ELEC) ||
									  (r_info[target->r_idx].flags3 & RF3_IM_ELEC)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 128 + 2: /* RF5_BA_FIRE */
			...
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_FIRE) ||
									  (r_info[target->r_idx].flags3 & RF3_IM_FIRE)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 128 + 3: /* RF5_BA_COLD */
			...
			if (mon_v_mon) {
				if (ag->kill && ((r_info[target->r_idx].flags4 & RF4_BR_COLD) ||
									  (r_info[target->r_idx].flags3 & RF3_IM_COLD)))
					z /= 9;
				break;
			}
			if (borg_goi || borg_wraith) {
		...
		case 160 + 5: /* RF6_TPORT */
			/* Monsters with this spell can follow the player.  Thats very
			 * dangerous */
			if (!(r_ptr->flags3 & RF3_RES_TELE)) {
				/*
				 * These guys can follow the borg through a phase and teleport so if
				 * they get too close,
				 * then the borg should be very nervous and either tport them away
				 * or tport himself.
				 */
				if (kill->dist <= 3)
					p += hp;
			...
			for (ii = 0; i < 8; i++) {
				...
				if (ag->feat == FEAT_GLYPH || ag->kill ||
					 ((ag->feat >= FEAT_DOOR_HEAD) &&
					  (ag->feat <= FEAT_PERM_SOLID))) { /* track the safe areas for
																		calculating danger */
					spot_safe++;
					...
					if (spot_safe == 0)
						spot_safe = 1;
					if ((borg_position &
						  (POSITION_SEA | POSITION_SUMM | POSITION_BORE)))
						spot_safe = 1000;
				}
			}
		...
		case 160 + 18: /* RF6_S_MONSTER */
			...
			if ((borg_position & (POSITION_SEA | POSITION_SUMM | POSITION_BORE)))
				spot_safe = 1000;
			if (borg_goi || borg_wraith || pfe || glyph || borg_create_door ||
		...
		if (bolt == TRUE) {
			if (!mon_v_mon && (borg_skill[BI_REFLECT] ||
									 !borg_projectable_pure(kill->y, kill->x, y, x)))
				z = 0;
			if (mon_v_mon && (!borg_projectable_pure(kill->y, kill->x, y, x)))
				z = 0;
		}
		/* Some borgs are concerned with the 'effects' of an attack.  ie, cold
		 * attacks shatter potions,
	 ...
	 * that spell, since the danger of the others (like fire, cold) will be
	 * greatly
	 * reduced by the proposed defence spell.  The result will be the borg will
	 * not cast
	 * the resistance spell eventhough it may be a very good idea.
	 *
	 * Example: a monster has three breath attacks (Fire, Ice, Disenchant) and
	 * each hits for
	 * 500 pts of damage.  The borg currently resists all three, so the danger
	 * would be 500.
	 * if the borg were to use a Res Heat Potion that would decrease the danger
	 * to:
	 * Fire:  333
	 * Ice:   500
	 * Disen: 500
	 * Now the Average is 444.  Not really worth it, nominal change.  But if the
	 * resistance
	 * spell was both Fire and Ice, then it would be:
	 * Fire:  333
	 * Ice:   333
	 * Disen: 500
	 * With an average of 388. Probably worth it, but the borg would see that the
	 * Disen attack
	 * is a quite dangerous and would return the result of 500.
	 *
	 * To fix this, the flag 'average' is added to the borg_danger() to skip this
	 * check
	 * and return the average damage.  If the flag is FALSE then the formula
	 * below is SKIPPED
	 * and the value returned with be the average.  If the flag is TRUE, then the
	 * formula below
	 * will be used to determine the returned value.  Currently the elemental
	 * resistance spells
	 * and PFE have the flag set as FALSE.  If any other defence spells switch to
	 * using
	 * 'false' make sure the p1 value is reset in that particular
	 * borg_defend_aux_foo.
	 *
	 */
	if (!average)
...
int borg_danger_aux(int y, int x, int c, int i, bool average, bool mon_v_mon) {
	...
	/*
	 * Skip certain monster indexes. These are used in the Swap Position
	 * mutation.
	 */
	if (borg_swap_pos) {
	...
	if ((r_ptr->flags2 & RF2_MULTIPLY) &&
		 (borg_skill[BI_CLEVEL] < 20)) { /* extra 50% */
		v1 = v1 + (v1 * 15 / 10);
	}
	...
	if (borg_sleep_spell_ii) {
		if ((d == 1) && (kill->awake) && (!(r_ptr->flags3 & RF3_NO_SLEEP)) &&
			 (!kill->unique) && (kill->level <= (borg_skill[BI_CLEVEL] - 15))) {
	...
	if (borg_sleep_spell) {
		if (kill->awake && (!(r_ptr->flags3 & RF3_NO_SLEEP)) && (!kill->unique) &&
			...
			else if ((borg_cave_floor_bold(y - 1, x) &&
						 borg_cave_floor_bold(y + 1, x) &&
	...
	if (kill->confused) {
		...
		if (borg_skill[BI_MAXCLEVEL] < 15 || borg_skill[BI_CURHP] < 20)
			v1 = 0;
	}
	if (kill->stunned) {
		v1 = v1 * 10 / 13;
	...
	if (borg_confuse_spell) {
		if (kill->awake && !kill->confused && (!(r_ptr->flags3 & RF3_NO_CONF)) &&
			 (!kill->unique)) {
			/* Under special circumstances force the damage to 0 */
			if ((borg_skill[BI_CLEVEL] < 20 &&
				  borg_skill[BI_CURHP] < borg_skill[BI_MAXHP] / 2) ||
	...
	if (!mon_v_mon) {
		 ...
		 * in the same round.  We should consider monsters that have a speed 2 or
		 * 3 classes
		 * higher than ours, but most times, the borg will only encounter monsters
		 * with a single
		 * catagory higher speed.
		 */
		if (q > 10 && d != 1 && v1 && !(r_ptr->flags1 & RF1_NEVER_MOVE) &&
			 !borg_munchkin_mode) {
			b_v1 = 0;
			...
			for (ii = 0; ii < 8; ii++) {
				...
				/* Check for legality */
				if (!in_bounds(y_temp, x_temp))
					continue;

				/* Cannot occupy another monster's grid unless it can push past or
				 * kill it */
				...
				if (borg_grids[y_temp][x_temp].feat >= FEAT_DOOR_HEAD &&
					 borg_grids[y_temp][x_temp].feat <= FEAT_DOOR_TAIL)
					continue;
				...
				if (borg_grids[y_temp][x_temp].feat == FEAT_WATER &&
					 ...
					 !(r_ptr->flags7 & RF7_AQUATIC) && !(r_ptr->flags7 & RF7_FLIGHT))
					continue;
				...
				*/
				...
				if (distance(y_temp, x_temp, y, x) > 1)
					continue;
		...
		/* A low level borg might be able to flee better after drinking that speed
		 * pot */
	...
	if (borg_threat_conf)
		v1_borg_threat_conf = TRUE;
	if (borg_threat_blind)
		v1_borg_threat_blind = TRUE;
	if (borg_threat_para)
		v1_borg_threat_para = TRUE;
	if (borg_threat_invis)
		v1_borg_threat_invis = TRUE;
	...
	/* Hack -- verify line of sight (both ways) but allow attack to pass through
 * closed
 * doors if the danger is significant.  These should keep the borg from opening
 * a
 * door to a vault full of summoners
 */
	if (!borg_projectable(y9, x9, y, x, FALSE, TRUE) &&
		 !borg_projectable(y, x, y9, x9, FALSE, TRUE)) {
		v2 = 0;
	}
	/* Hack -- verify line of sight (both ways) for monsters who can only move >
	 *1 grid.
	 * Some fast monsters can take a move action and range attack in the same
	 *round.
	 * Basically, we see how many grids the monster can move and check LOS from
	 *each of
	 * those grids to determine the relative danger.  We need to make sure that
	 *the monster
	 * is not passing through walls unless he has that ability.
			 * Consider a fast monster who can move and cast a spell in the same
	 *round.
		 * This is important for a borg looking for a safe grid from a ranged
	 *attacker.
		 * If the attacker is fast then he might be able to move to a grid which
	 *does have LOS
	...
	*/
		...
		for (ii = 0; ii < 8; ii++) {
			...
			if ((borg_grids[y_temp][x_temp].feat >= FEAT_WALL_EXTRA &&
				...
				if (r_ptr->flags2 & RF2_PASS_WALL) {
					if (borg_projectable(y_temp, x_temp, y, x, FALSE, TRUE))
						b_v2 = v2 * q / 10;
				}
			...
			else if (borg_projectable(y_temp, x_temp, y, x, FALSE, TRUE))
				b_v2 = v2 * q / 10;
		}
	...
	if (borg_sleep_spell_ii) {
		if ((d == 1) && (kill->awake) && (!(r_ptr->flags3 & RF3_NO_SLEEP)) &&
			 (!kill->unique) &&
			 (kill->level <=
			  ((borg_skill[BI_CLEVEL] < 15)
					 ? borg_skill[BI_CLEVEL]
					 : (((borg_skill[BI_CLEVEL] - 10) / 4) * 3) + 10))) {
			v2 = v2 / 3;
		}
	}
	...
	if (v2 && !(r_ptr->flags2 & RF2_SMART)) {
		...
		for (i = 1; i < borg_kills_nxt; i++) {
			...
			if (!borg_projectable(kill->y, kill->x, y9, x9, FALSE, TRUE))
				continue;

			/* track closest one */
			if (b_dx > distance(y9, x9, kill->y, kill->x))
				continue;
	...
	if (v2 == 0) {
		...
		if (borg_threat_conf)
			v2_borg_threat_conf = TRUE;
		if (borg_threat_blind)
			v2_borg_threat_blind = TRUE;
		if (borg_threat_para)
			v2_borg_threat_para = TRUE;
		if (borg_threat_invis)
			v2_borg_threat_invis = TRUE;
	}

	/* Differentiate between the v1 and v2 immediate threats */
	if (v1_borg_threat_conf || v2_borg_threat_conf)
		borg_threat_conf = TRUE;
	if (v1_borg_threat_blind || v2_borg_threat_blind)
		borg_threat_blind = TRUE;
	if (v1_borg_threat_para || v2_borg_threat_para)
		borg_threat_para = TRUE;
	if (v1_borg_threat_invis || v2_borg_threat_invis)
		borg_threat_invis = TRUE;
...
int borg_danger(int y, int x, int c, bool average) {
	 ...
	 * floor grid */
	if ((!(cave[y][x].info & (CAVE_ICKY)) && borg_skill[BI_CDEPTH] <= 70 &&
	...
	if (borg_skill[BI_CDEPTH] == 100 && p >= 300)
		p = 300;

	/* Added danger (from a lot of monsters).
		 * But do not add it if we have been sitting on
		 * this panel for too long, or monster's in a vault.  The
	 * fear_monsters[][]
		 * can induce some bouncy behavior.
		 */
	if (time_this_panel <= 200 && !(cave[y][x].info & (CAVE_ICKY)))
...
cptr borg_restock(int depth) {
	...
	if (!borg_skill[BI_LITE] && borg_skill[BI_CUR_LITE] < 1)
		return ("rs my_cur_lite");

	/* Must have "fuel" */
	if (borg_skill[BI_AFUEL] < 1 && !borg_skill[BI_LITE])
		return ("rs amt_fuel");

	/* Must have "food" */
	if (borg_skill[BI_FOOD] < 1 && !borg_skill[BI_VAMPIRE])
		return ("rs amt_food");
	...
	/* Must have good lite */
	if (!borg_skill[BI_LITE] && borg_skill[BI_CUR_LITE] < 2)
		return ("rs lite+1");

	/* Must have "fuel" */
	if (borg_skill[BI_AFUEL] < 3 && !borg_skill[BI_LITE])
		return ("rs fuel+2");

	/* Must have "food" */
	if (borg_skill[BI_FOOD] < 2 && !borg_skill[BI_VAMPIRE])
		return ("rs food+2");
	...
	/* Must have "recall" */
	if (borg_skill[BI_RECALL] < 1)
		return ("rs recall");

	/* Potions of Critical Wounds */
	if ((borg_skill[BI_MAXCLEVEL] < 15) &&
		 borg_skill[BI_ACSW] + borg_skill[BI_ACCW] < 1 &&
	...
	/* Must have "cure" */
	if ((borg_skill[BI_MAXCLEVEL] < 25) &&
		 borg_skill[BI_ACSW] + borg_skill[BI_ACCW] < 4)
		return ("rs 4 cure");

	/* Must have "teleport" */
	if (borg_skill[BI_ATELEPORT] + borg_skill[BI_AESCAPE] +
			  borg_skill[BI_ATELEPORTLVL] <
		 2)
		return ("rs 2 teleport");
	...
	/* Must have "Heal" */
	if (borg_skill[BI_AHEAL] + borg_has[ROD_HEAL] + borg_skill[BI_AEZHEAL] < 1)
		return ("rs heal");
	...
	if (borg_t - borg_began < 10 && borg_skill[BI_AEZHEAL] < 15)
		return ("rs *heal*");
...
static cptr borg_prepared_aux(int depth) {
	...
	static char ret_string
		 [2000]; /* hack.  big ass string to return 'why things are bad' string */
	...
	for (iEntry = 0; iEntry < nEntrys; iEntry++) {
		...
		if (Req_item->item == -1) {
			if (!borg_calc_formula(formula[Req_item->number])) {
				...
				if (borg_slow_return)
					sprintf(ret_string, "Formula [%s] failed",
		...
		if (borg_has[Req_item->item] < Req_item->number) {
			...
			sprintf(ret_string, "Number of %s < %d", borg_prt_item(Req_item->item),
					  Req_item->number);
			return ret_string;
		}
	}
...
static cptr borg_prepared_aux2(int depth) {
	...
	/* Require food */
	if (borg_skill[BI_FOOD] < 3 && !borg_skill[BI_VAMPIRE])
		return ("3 Food");
	...
	/* Require fuel */
	if (borg_skill[BI_AFUEL] < 5 && !borg_skill[BI_LITE])
		return ("5 Fuel");

	if (!borg_plays_risky) {
		/* Require 30 hp */
		if (borg_skill[BI_MAXHP] < 30)
			return ("30 hp");
	}
	...
	/* Require recall */
	if (borg_skill[BI_RECALL] < 1 && borg_skill[BI_CLEVEL] <= 10)
		return ("1 recall");
	...
	if (!borg_plays_risky) {
		...
		switch (borg_class) {
		case CLASS_WARRIOR:
			if (borg_skill[BI_MAXHP] < 50)
				return ("50 hp");
			if (borg_skill[BI_MAXCLEVEL] < 4)
				return ("4 clevel");
			break;
		case CLASS_ROGUE:
			if (borg_skill[BI_MAXHP] < 50)
				return ("50 hp");
			if (borg_skill[BI_MAXCLEVEL] < 8)
				return ("8 clevel");
			break;
		case CLASS_PRIEST:
			if (borg_skill[BI_MAXHP] < 40)
				return ("40 hp");
			if (borg_skill[BI_MAXCLEVEL] < 9)
				return ("9 level");
			break;
		case CLASS_PALADIN:
			if (borg_skill[BI_MAXHP] < 50)
		...
		case CLASS_RANGER:
			...
			break;
		case CLASS_MAGE:
			if (borg_skill[BI_MAXHP] < 60)
				return ("60 hp");
			if (borg_skill[BI_MAXCLEVEL] < 11)
				return ("11 clevel");
			break;
		default:
			if (borg_skill[BI_MAXHP] < 50)
	...
	if ((borg_skill[BI_MAXCLEVEL] < 10) &&
		 borg_skill[BI_ACLW] + borg_skill[BI_ACSW] + borg_skill[BI_ACCW] < 2)
		return ("2 cure");

	/* Require food */
	if (borg_skill[BI_FOOD] < 5 && !borg_skill[BI_VAMPIRE])
		return ("5 Food");
	...
	if ((borg_skill[BI_MAXCLEVEL] < 20) &&
		 borg_skill[BI_ACLW] + borg_skill[BI_ACSW] + borg_skill[BI_ACCW] < 5)
		return ("5 cures");
	...
	if (!borg_plays_risky) {
		...
		case CLASS_WARRIOR:
			...
			break;
		case CLASS_ROGUE:
			if (borg_skill[BI_MAXCLEVEL] < depth && depth <= 19)
	...
	if ((borg_skill[BI_MAXCLEVEL] < 30) && borg_skill[BI_ACCW] < 5)
		return ("cure crit5");

	/* See invisible or telepathy */
	if ((!borg_skill[BI_SINV] && !borg_skill[BI_ESP] && !borg_skill[BI_SSINV] &&
		  !borg_skill[BI_SESP]) &&
		 borg_skill[BI_CLEVEL] < 30)
	...
	if (!borg_skill[BI_SRFIRE]) {
		int basics =
			 borg_skill[BI_SRACID] + borg_skill[BI_SRCOLD] + borg_skill[BI_SRELEC];

		if (basics < 2)
			return ("RF + resist2");
	}
	/* have some minimal stats */
	if (borg_stat[A_STR] < 7)
		return ("low STR");

	if (borg_skill[BI_INTMANA]) {
		if (borg_stat[A_INT] < 7)
			return ("low INT");
	}
	if (borg_skill[BI_WISMANA]) {
		if (borg_stat[A_WIS] < 7)
			return ("low WIS");
	}
	if (borg_stat[A_DEX] < 7)
		return ("low DEX");
	if (borg_stat[A_CON] < 7)
		return ("low CON");
	...
	if (!borg_plays_risky) {
		...
		case CLASS_PRIEST:
			...
			break;
		case CLASS_PALADIN:
			if (borg_skill[BI_MAXCLEVEL] < (depth + 2) &&
				 borg_skill[BI_MAXCLEVEL] <= 40)
				return ("dlevel + 2 >= clevel");
			break;
		case CLASS_RANGER:
			if (borg_skill[BI_MAXCLEVEL] < (depth + 3) &&
				 borg_skill[BI_MAXCLEVEL] <= 41 && borg_skill[BI_MAXCLEVEL] > 28)
		...
		case CLASS_MAGE:
			...
			if (((borg_skill[BI_MAXCLEVEL] - 38) * 2 + 30) < depth &&
				 borg_skill[BI_MAXCLEVEL] <= 44 && borg_skill[BI_MAXCLEVEL] > 38)
	...
	if (!borg_skill[BI_SRCOLD])
		return ("RCold");
	if (!borg_skill[BI_SRELEC])
		return ("RElec");
	if (!borg_skill[BI_SRACID])
		return ("RAcid");
	if (!borg_skill[BI_SRPOIS])
		return ("RPois");

	/* Escape and Teleport */
	if (borg_skill[BI_ADIMDOOR] + borg_skill[BI_ATELEPORT] +
			  borg_skill[BI_AESCAPE] + borg_skill[BI_ATELEPORTLVL] <
		 6)
		return ("tell&esc6");

	/* Cure Critical Wounds */
	if ((borg_skill[BI_MAXCLEVEL] < 30) &&
		 (borg_skill[BI_ACCW] + borg_skill[BI_ACSW]) < 10)
		return ("cure10");
	...
	if ((borg_skill[BI_REALM1] >= REALM_SORCERY ||
		  borg_skill[BI_REALM2] >= REALM_SORCERY)) {
		if (borg_stat[A_INT] < 16)
			return ("low INT");
	}
	if ((borg_skill[BI_REALM1] == REALM_MIRACLES ||
		  borg_skill[BI_REALM2] == REALM_MIRACLES)) {
		if (borg_stat[A_WIS] < 16)
			return ("low WIS");
	}
	if (borg_stat[A_DEX] < 16)
		return ("low DEX");
	if (borg_stat[A_CON] < 16)
		return ("low CON");

	if (!borg_plays_risky) {
		/* Minimal level */
		if (borg_skill[BI_MAXCLEVEL] < 45)
			return ("level45");
	}

	/* Sure fire escape */
	if (borg_skill[BI_AESCAPE] + borg_skill[BI_ATELEPORTLVL] < 2)
		return ("escape2");

	/* Telepathy, better have it by now */
	if (!borg_skill[BI_ESP] && !borg_skill[BI_SESP])
		return ("ESP");
	...
	if (borg_skill[BI_SPEED] < 115 && !borg_skill[BI_ASPEED])
		return ("+5 speed");

	/* Potions of heal */
	if (borg_skill[BI_AHEAL] < 1 && (borg_skill[BI_AEZHEAL] < 1))
		return ("1heal");
	...
	/* Potions of heal */
	if (borg_skill[BI_AHEAL] < 2 && borg_skill[BI_AEZHEAL] < 1)
		return ("2heal");
	...
	if (borg_skill[BI_AXGOI]) {
		...
		if (!borg_skill[BI_SRKAOS])
			return ("RChaos");
	}
	if (!borg_skill[BI_SRDIS])
		return ("RDisen");
	...
	if (!borg_skill[BI_KING]) {
		if ((borg_skill[BI_MAXSP] > 100) && (borg_has[POTION_RES_MANA] < 15))
			return ("15ResMana");
		...
		if (borg_has[POTION_HEAL] < 15 &&
			...
			return ("15Heal");
		else if (borg_has[POTION_HEAL] < 25)
			return ("25Heal");

		/* must have lots of ez-heal */
		if (borg_skill[BI_AEZHEAL] < 15)
			return ("15EZHeal");

		/* must have lots of speed */
		if (borg_skill[BI_ASPEED] < 15)
			return ("15Speed");
	}
...
cptr borg_prep(int depth) {
	...
	if (!borg_uses_calcs) {
		if ((reason = borg_prepared_aux2(depth)))
			return (reason);
	}
	...
	if (borg_skill[BI_MAXDEPTH] <= 98) {
		...
		for (i = 1; i < MAX_R_IDX - 1; i++) {
			...
			numb_live_unique++;
			if (i < living_unique_index || living_unique_index == 0)
		...
		if (borg_scums_uniques && borg_skill[BI_MAXCLEVEL] >= 35)
			auto_scum = TRUE;
#if 0
 ...
 */
void borg_init_4(void) { /* Do nothing? */ }
