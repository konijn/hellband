...
static void borg_follow_take(int i)
{
	...
	if (ag->kill && !(r_ptr->flags2 & RF2_TAKE_ITEM) &&
		!(r_ptr->flags2 & RF2_KILL_ITEM)) return;
...
static int borg_new_take(int k_idx, int y, int x)
{
	...
	else borg_note(format("# Creating an object '%s' at (%d,%d) valued at %d",
                     (k_name + k_info[take->k_idx].name),
                     take->y, take->x, take->value));
    /* Wipe goals */
    if (goal == GOAL_TAKE) goal = 0;

	/* Unknown grids are considered on a floor (but sometimes it is a water or lava) */
	if (borg_grids[take->y][take->x].feat == FEAT_INVIS ||
		borg_grids[take->y][take->x].feat == FEAT_NONE) borg_grids[take->y][take->x].feat = FEAT_FLOOR;
...
static bool observe_take_move(int y, int x, int d, byte a, char c)
{
    ...
    for (i = 1; i < borg_takes_nxt; i++)
    {
        ...
        if (!borg_skill[BI_ISIMAGE] && !borg_wraith && !borg_goi && a != k_ptr->d_attr)
        {
...
static int borg_guess_race(byte a, char c, bool multi, int y, int x)
{
    ...
    /* Message */
    if (borg_verbose) borg_note(format("# Assuming player ghost (char %d, attr %d)", c, a));
...
static int borg_guess_race_name(cptr who)
{
    ...
    {
        /* Stop when done */
        if (!streq(who, DEPTH_NORMAL_text[k])) break;
    ...
    /* Message */
    if (borg_verbose) borg_note(format("# Assuming player ghost (%s)", who));
...
static void borg_fear_grid(cptr who, int y, int x, int k)  /* 8-8, this was uint */
{
	...
	if (cave[y][x].info & CAVE_ICKY) return;
...
/*
 * Increase the "region danger."  this is induced from a non-seen monster.  An invisible
...
static void borg_fear_regional(cptr who, int y, int x, int k, bool seen_guy) /* 8-8 , had been uint */
{
	...
	if (prefix(who, "Your ")) return;

	/* Messages */
    borg_note(format("# Fearing regional (%d,%d) value %d because of a non-LOS %s",
                        y, x, k, who));
    /* Shift the panel */
...
static void borg_update_kill_new(int i)
{
	...
	if (kill->pet && kill->dist < 5) borg_pets = TRUE;
    ...
    for (k = 0; k < 32; k++)
    {
        if (r_ptr->flags4 & (1L << k)) kill->spell[num++] = k + 32 * 3;
    }

    /* Extract the "normal" spells */
    for (k = 0; k < 32; k++)
    {
        if (r_ptr->flags5 & (1L << k)) kill->spell[num++] = k + 32 * 4;
    }

    /* Extract the "bizarre" spells */
    for (k = 0; k < 32; k++)
    {
        if (r_ptr->flags6 & (1L << k)) kill->spell[num++] = k + 32 * 5;
    }
	...
	/* monster is Unique */
	kill->unique = FALSE;
    if (r_ptr->flags1 & RF1_UNIQUE) kill->unique = TRUE;
    if (r_ptr->flags1 & RF1_QUESTOR) kill->questor = TRUE;

	/* Monster is a summoner type */
	kill->summoner = FALSE;
	if ((r_info[kill->r_idx].flags6 & RF6_S_KIN) ||
		(r_info[kill->r_idx].flags6 & RF6_S_CYBER) ||
		...
		(r_info[kill->r_idx].flags6 & RF6_S_AMBERITES) ||
		(r_info[kill->r_idx].flags6 & RF6_S_UNIQUE) ) kill->summoner = TRUE;
	...
	/* Some monsters are a waste of our resources and we should not try to fight them.  
	 * 1.  We do not have an effective attack against them
	 * 2.  Their HP are really high, they are dangerous, and the treasure drop sucks
	 * 
	 */
	...
	if (!kill->friendly && !kill->pet && rounds >= 5)
	{
 		kill->cautious = TRUE;
    ...
    if (kill->r_idx == RACE_MORGOTH ||
		((kill->unique || kill->questor)   &&
		 (r_ptr->flags2 & RF2_KILL_WALL)))
    {
	    j = ((kill->y - PANEL_HGT / 2) / PANEL_HGT) * PANEL_HGT;
	    if (j < 0) j = 0;
	    if (j > MAX_HGT - SCREEN_HGT) j = MAX_HGT - SCREEN_HGT;
	    questor_panel_y = j;

	    j = ((kill->x - PANEL_WID / 2) / PANEL_WID) * PANEL_WID;
	    if (j < 0) j = 0;
	    if (j > MAX_WID - SCREEN_WID) j = MAX_WID - SCREEN_WID;
	    questor_panel_x = j;
	}
    ...
    if ((borg_grids[kill->y][kill->x].feat == FEAT_NONE ||
		borg_grids[kill->y][kill->x].feat == FEAT_INVIS) && !(r_ptr->flags2 & RF2_PASS_WALL))
    {
...
void borg_delete_kill(int i)
{
    ...
    /* Note */
    if (borg_verbose) borg_note(format("# Forgetting a monster '%s' at (%d,%d)",
    ...
    if (r_info[kill->r_idx].flags2 & RF2_MULTIPLY && !kill->pet)
	...
	if (quest_number(borg_skill[BI_CDEPTH])) (borg_depth |= DEPTH_QUEST);
	else (borg_depth &= ~DEPTH_QUEST);
...
static void borg_follow_kill(int i)
{
    ...
    for (j = 0; j < 8; j++)
    {
        ...
        if (ag->kill && 
			(!(r_info[kill->r_idx].flags2 & RF2_MOVE_BODY) &&
			 !(r_info[kill->r_idx].flags2 & RF2_KILL_BODY))) continue;
    ...
    /* Clear goals */
    if (((!borg_skill[BI_ESP] && !borg_esp) && goal == GOAL_KILL && (borg_flow_y[0] == kill->y && borg_flow_x[0] == kill->x))
...
static int borg_new_kill(int r_idx, int y, int x)
{
    ...
    /* Note */
    if (kill->r_idx != 0) (format("# Creating a monster '%s' at (%d,%d) danger %d at distance %d.",
                     (r_name + r_info[kill->r_idx].name),
                     kill->y, kill->x, p, kill->dist));

    /* Recalculate danger */
    borg_danger_wipe = TRUE;

    /* Wipe goals if this new monster can see me. */
    if (kill->los && borg_skill[BI_CDEPTH]) goal = 0;
    ...
    if (r_ptr->flags2 & RF2_PASS_WALL)
    {
		if (borg_grids[kill->y][kill->x].feat == FEAT_INVIS ||
			borg_grids[kill->y][kill->x].feat == FEAT_NONE) borg_grids[kill->y][kill->x].feat = FEAT_WALL_EXTRA;
	}
	...
	for (i = 0; i < borg_nasties_num; i++)
	{
		...
		if (r_info[kill->r_idx].d_char == borg_nasties[i])
		{
			...
			if (borg_nasties[i] == 'j'  && !strstr(r_name + r_info[kill->r_idx].name, "Shoggoth"))  borg_nasties_count[i] --;
			if (borg_nasties[i] == 'B' && !strstr(r_name + r_info[kill->r_idx].name, "Lord"))  borg_nasties_count[i] --;
			if (borg_nasties[i] == 'v' && !strstr(r_name + r_info[kill->r_idx].name, "Lloigor"))  borg_nasties_count[i] --;
		}
	}
...
static bool observe_kill_diff(int y, int x, byte a, char c)
{
	...
	/* no new monsters if hallucinations Unless the borg is just starting on this level like a screensaver restart. */
 ...
 */
static bool observe_kill_move(int wank_num, int y, int x, int d, byte a, char c, bool flag)
{
    int i, z, ox, oy;
    int r_idx;
	int num;
    ...
    for (i = 1; i < borg_kills_nxt; i++)
    {
		...
		if (kill->m_idx != c_ptr->m_idx) continue;
        ...
        if (a != r_ptr->d_attr && !borg_goi && !borg_wraith && !borg_skill[BI_ISIMAGE])
        {
				...
				(r_ptr->flags2 & (RF2_ATTR_ANY | RF2_SHAPECHANGER)))
			{
                ...
                /* Clear goals */
            if (!borg_skill[BI_ESP] && !borg_esp && goal == GOAL_TAKE) goal = 0;
            }
        }
        ...
        if (c != r_ptr->d_char && !borg_skill[BI_ISIMAGE])
		{
			...
			if (!(r_ptr->flags2 & (RF2_ATTR_ANY | RF2_SHAPECHANGER))) continue;
		}

		/* Verify that the wank-monster we are looking at is not a known monster
		 * which has not moved but happens to be standing next to a similar monster.
		 */
        if (z)
		{
			...
			/* Note */
			if (!kill->friendly && !kill->pet) borg_note(format("# Tracking a monster '%s' at (%d,%d) from (%d,%d)",
							 (r_name + r_ptr->name),
							 kill->y, kill->x, oy, ox));
...
static int borg_fear_spell(int i)
{
	...
	/* Minimal ouch */
	if (ouch < borg_skill[BI_CDEPTH]) ouch = borg_skill[BI_CDEPTH];
...
static int borg_locate_kill(cptr who, int y, int x, int r)
{
    ...
    if (b_i >= 0)
    {
        ...
        /* Note */
        if (borg_verbose) borg_note(format("# Converting an object '%s' at (%d,%d)",
                         (k_name + k_ptr->name),
                         take->y, take->x));
    ...
    for (i = 1; i < borg_kills_nxt; i++)
    {
		...
		/* Allow for greater range for uniques so the borg does not create multiple uniques per level */
		if (kill->unique) r *= 3;
    ...
    if (b_i >= 0)
    {
        ...
        /* Note */
        if (borg_verbose) borg_note(format("# Converting a monster '%s' at (%d,%d)",
                         (r_name + r_info[kill->r_idx].name),
                         kill->y, kill->x));
    ...
    for (i = 1; i < borg_kills_nxt; i++)
    {
        ...
        if (!borg_projectable(kill->y,kill->x,y,x,FALSE, TRUE)) continue;

		/* Is it in a dark zone? */
		if (!kill->los) borg_force_grid_los(kill->y, kill->x);
    ...
    if (b_i >= 0)
    {
        ...
        /* Note */
        if (borg_verbose) borg_note(format("# Matched a monster '%s' at (%d,%d)",
                         (r_name + r_info[kill->r_idx].name),
                         kill->y, kill->x));
    ...
    /* Note */
    if (borg_verbose) borg_note(format("# Ignoring a monster '%s' near (%d,%d)",
                     (r_name + r_ptr->name),
                     y, x));
...
static void borg_count_death(int i)
{
    ...
    if (borg_race_death[kill->r_idx] < MAX_SHORT) borg_race_death[kill->r_idx]++;

    /* if it was a unique then remove the unique_on_level flag */
    if (kill->unique) unique_on_level = 0;
...
static bool borg_handle_self(cptr str)
{
    ...
    else if (prefix(str, "lite"))
    {
		...
		if (!borg_skill[BI_CUR_LITE])
		{
	        ...
	        for (y = c_y - ((borg_skill[BI_CLEVEL] / 10) + 1); y <= c_y + ((borg_skill[BI_CLEVEL] / 10) + 1); y++)
	        {
	            /* Scan the "local" grids (5x5) */
	            for (x = c_x - ((borg_skill[BI_CLEVEL] / 10) + 1); x <= c_x + ((borg_skill[BI_CLEVEL] / 10) + 1); x++)
	            {
					/* Bounds check */
    ...
    {
        /* Message */
        borg_note(format("# Detected traps, doors, evil, & walls (%d,%d to %d,%d)",
                         q_y, q_x, q_y+1, q_x+1));
        ...
        for (dx = 0; dx < SCREEN_WID; dx++)
        {
           ...
           if ((a_trans != 0) || (c_trans != 0))
           {
            ...
            if (t_c == '@')
            {
                ...
                /* Steal my feat */
				if (!borg_skill[BI_ISBLIND]) t_c = f_info[c_ptr->feat].d_char;
				else if (ag->feat != FEAT_NONE)
                    t_c = f_info[ag->feat].d_char;
                else
                    t_c = f_info[FEAT_FLOOR].d_char;
				/* Sometimes the borg is tricked into thinking he is standing on a broken door so that he wont loop on spells */
				if (ag->feat == FEAT_BROKEN) t_c = f_info[FEAT_BROKEN].d_char;
            ...
            switch (Get_f_info_number[t_c])
			{
                ...
                case FEAT_NONE:
                {
					...
					/* Normal */
					if (!(ag->info & BORG_MARK)) ag->feat = FEAT_NONE;

					/* Wiz_lite helps us know the entire dungeon */
					if (when_wizard_lite >= 1001) ag->feat = FEAT_FLOOR;
					...
					if (borg_detect_wall[(w_y / PANEL_HGT)+0][(w_x / PANEL_WID)+0] == TRUE &&
				...
				case FEAT_BLDG_HEAD:
				case FEAT_BLDG_HEAD + 1:
				case FEAT_BLDG_HEAD + 2:
				case FEAT_BLDG_HEAD + 3:
				case FEAT_BLDG_HEAD + 4:
				case FEAT_BLDG_HEAD + 5:
				case FEAT_BLDG_HEAD + 6:
				case FEAT_BLDG_HEAD + 7:
				case FEAT_BLDG_HEAD + 8:
				case FEAT_BLDG_HEAD + 9:
				case FEAT_BLDG_HEAD + 10:
				case FEAT_BLDG_HEAD + 11:
				case FEAT_BLDG_HEAD + 12:
				case FEAT_BLDG_HEAD + 13:
				case FEAT_BLDG_HEAD + 14:
				case FEAT_BLDG_HEAD + 15:
				case FEAT_BLDG_HEAD + 16:
				case FEAT_BLDG_HEAD + 17:
				case FEAT_BLDG_HEAD + 18:
				case FEAT_BLDG_HEAD + 19:
				case FEAT_BLDG_HEAD + 20:
				case FEAT_BLDG_HEAD + 21:
				case FEAT_BLDG_HEAD + 22:
				case FEAT_BLDG_HEAD + 23:
				case FEAT_BLDG_HEAD + 24:
				case FEAT_BLDG_HEAD + 25:
				case FEAT_BLDG_HEAD + 26:
				case FEAT_BLDG_HEAD + 27:
				case FEAT_BLDG_HEAD + 28:
				case FEAT_BLDG_HEAD + 29:
				case FEAT_BLDG_HEAD + 30:
				case FEAT_BLDG_TAIL:
				{
					ag->feat = FEAT_FLOOR;
					break;
                ...
                case FEAT_DOOR_TAIL:
                {
					...
					/* Town Shop doors */
					if ((c_ptr->feat >= FEAT_BLDG_HEAD && c_ptr->feat <= FEAT_BLDG_TAIL) ||
					    (c_ptr->feat >= FEAT_SHOP_HEAD && c_ptr->feat <= FEAT_SHOP_TAIL))	ag->feat = c_ptr->feat;
					else
                    /* Assume easy */
            ...
            if (old_wall != new_wall)
            {
                ...
                if (new_wall && goal != GOAL_DIGGING) borg_data_flow->data[y][x] = 255;
			...
			for (y=0; y < MAX_HGT-1 ; y++)
			{
				for(x=0; x < MAX_WID -1; x++)
				{
					    ...
					    cave[y][x].feat <= FEAT_BLDG_TAIL)
					{
						...
						{
							if (cave[y][x].feat == FEAT_BLDG_HEAD + i)
							{
								/* bldg Location */
								track_bldg_x[i] = x;
								track_bldg_y[i] = y;
...
void borg_update(void)
{
    ...
    for (i = 1; i < borg_kills_nxt; i++)
    {
        ...
        if (borg_t - kill->when < 1000) continue;
    ...
    for (i = 0; i < borg_msg_num; i++)
    {
        ...
        else if (prefix(msg, "KILL:"))
        {
            ...
            if (successful_target < 0) successful_target = 1;
    ...
    if (successful_target < 0)
    {
        ...
        else if (successful_target > -10)
        {
			...
			if (borg_target_unknown_wall(g_y, g_x))	successful_target = 1;
        }
    }
    ...
    if (old_depth != borg_skill[BI_CDEPTH] ||
    	borg_respawning >= 3 ||
		old_town != borg_skill[BI_TOWN_NUM])
    {
		...
		for (i = 0; i < 9; i++)
		{
			if (borg_skill[BI_CDEPTH])
			{
				...
				if (when_waypoint[i]) continue;
				if (!when_waypoint[i+1]) break;
			}
			/* Not in a move */
			when_waypoint[i] = 0;
		}
		...
		for (i = 0; i < borg_nasties_num; i++)
		{
			...
			if (borg_nasties[i] == 'Z' && borg_skill[BI_CDEPTH] >= 15) borg_nasties_count[i] = 25; /* Assume some on level */
		}
    ...
    else
    {
		 ...
		 */
		if ((borg_skill[BI_CDEPTH] >= MORGOTH_DEPTH && !borg_skill[BI_KING]) ||
			(unique_on_level == RACE_MORGOTH))
		{
			/* We assume Morgoth is on this level */
			borg_depth |= (DEPTH_SUMMONER | DEPTH_BORER | DEPTH_QUEST | DEPTH_UNIQUE); 
		}
		...
		{
			/* complete no show */
			if (unique_on_level != RACE_MORGOTH) borg_depth &= ~(DEPTH_SUMMONER | DEPTH_BORER);

			/* has not been seen in a long time */
			if (unique_on_level == RACE_MORGOTH && (borg_t - borg_t_questor > 500))
	        {
		...
		for (i = 1; i < borg_kills_nxt; i++)
		{
			...
			if (time_this_panel > 500)
			{
				...
				if (kill->dist > r_ptr->aaf + 10 && kill->when < borg_t - 30) continue;

				/* Skip those that are too far away */
				if (kill->dist > r_ptr->aaf && kill->when < borg_t - 200) continue;
						
				/* Skip ones that havent been seen in a while and they near enough */
				if (kill->dist < r_ptr->aaf && kill->when < borg_t - 500) continue;
			}
			...
			if (r_ptr->flags2 & (RF2_KILL_WALL | RF2_PASS_WALL))
			{
				...
				/* Can this tunnelling monster get to me?  Is it surrounded by PERMA_grids? 
				 * Scan the nearby grids to see if they are PERMA.  If there are, then assume
				 * the borer cannot reach the borg.
				 */
				for (ii = 0; ii < 8; ii++)
				{
					/* check for adjacent PERM_ grid */
					if (borg_grids[kill->y + ddy[ii]][kill->x + ddx[ii]].feat >= FEAT_PERM_EXTRA &&
						borg_grids[kill->y + ddy[ii]][kill->x + ddx[ii]].feat <= FEAT_PERM_SOLID) perm_adj = TRUE;
	...
	if (borg_skill[BI_CHEAPGLYPH] && (borg_depth & (DEPTH_BORER & DEPTH_SUMMONER)))
	{
    ...
    for (j = 0; j < 24; j++)
    {
		...
		if (j >= 8 && ag->feat <= FEAT_MORE && (ag->info & BORG_VIEW)) floor_grid += 2; 
		/* if (j >= 8 && ag->feat <= FEAT_MORE && ag->info & BORG_VIEW) floor_grid++; */
	}
    ...
    for (i = borg_wank_num - 1; i >= 0; i--)
    {
        ...
        if (wank->is_kill &&
            observe_kill_move(i, wank->y, wank->x, 0, wank->t_a, wank->t_c, FALSE))
        {
    ...
    for (i = borg_wank_num - 1; i >= 0; i--)
    {
        ...
        if (wank->is_kill &&
            observe_kill_move(i, wank->y, wank->x, 1, wank->t_a, wank->t_c, FALSE))
        {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    ...
    for (i = borg_wank_num - 1; i >= 0; i--)
    {
        ...
        /* Track moving monsters */
        if (wank->is_kill &&
            observe_kill_move(i, wank->y, wank->x, 2, wank->t_a, wank->t_c, FALSE))
        {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    ...
    for (i = borg_wank_num - 1; i >= 0; i--)
    {
        ...
        /* Track moving monsters */
        if (wank->is_kill &&
            observe_kill_move(i, wank->y, wank->x, 3, wank->t_a, wank->t_c, FALSE))
        {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
    ...
    for (i = borg_wank_num - 1; i >= 0; i--)
    {
        ...
        /* Track moving monsters */
        if (wank->is_kill &&
            observe_kill_move(i, wank->y, wank->x, 7, wank->t_a, wank->t_c, TRUE))
        {
            /* Hack -- excise the entry */
            borg_wanks[i] = borg_wanks[--borg_wank_num];
        }
		...
		else if (prefix(msg, "STATE__QUEST:"))
        {
			...
			for (i = 1; i < max_r_idx-1; i++)
			{
				...
				if (strstr(msg, r_name + r_ptr->name))
				{
					...
					if (r_ptr->level > (borg_skill[BI_MAXCLEVEL] * 12 / 10) && borg_skill[BI_MAXCLEVEL] <= 35)
					{
						borg_note("# Quest race may be too dangerous.  Fleeing level.");
						goal_fleeing = TRUE;
					}
    ...
    for (i = 0; i < borg_msg_num; i++)
    {
        ...
        {
            borg_fear_regional(what, c_y, c_x, 2 * borg_fear_spell(atoi(msg+6)), FALSE);
            borg_msg_use[i] = 5;
        }
    }
    /* Display messages */
    ...
    for (i = 1; i < borg_kills_nxt; i++)
    {
	    ...
	    if (!kill->friendly && !kill->pet) borg_fear_grid(r_name + r_info[kill->r_idx].name, kill->y, kill->x, p);
	}
...
void borg_init_5(void)
{
    ...
    /* Save the entries */
    for (i = 0; i < size; i++) DEPTH_UNIQUE_text[i] = text[i];
    for (i = 0; i < size; i++) DEPTH_UNIQUE_what[i] = what[i];
    ...
    /* Save the entries */
    for (i = 0; i < size; i++) DEPTH_NORMAL_text[i] = text[i];
    for (i = 0; i < size; i++) DEPTH_NORMAL_what[i] = what[i];
